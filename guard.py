...
class RepositoryGuard:
    ...
    def __init__(self, repo_root: Path):
        ...
        self.folders = {
            ...
            'myhibachi-ai-backend': {
                'type': 'AI Backend',
                'forbidden_env': ['STRIPE_SECRET_KEY',
    ...
    def check_secrets(self, file_path: Path, content: str,
                      lines: List[str]) -> None:
        ...
        for pattern, description in self.secret_patterns:
            ...
            for match in matches:
                ...
                self.violations.append(Violation(
                    relative_path, 'HARDCODED_SECRET',
                    f'{description} found in code',
                    line_num, match.group()
                ))
                self.stats['security_violations'] += 1

    def check_placeholders(self, file_path: Path, content: str,
                           lines: List[str]) -> None:
        ...
        for pattern, description in self.placeholder_patterns:
            ...
            for match in matches:
                line_num = content[:match.start()].count('\n') + 1
                self.violations.append(Violation(
                    relative_path, 'PLACEHOLDER_CONTENT',
                    f'{description} found',
                    line_num, match.group()
                ))
                self.stats['placeholder_violations'] += 1

    def check_folder_rules(self, file_path: Path, content: str,
                           lines: List[str]) -> None:
        """Check folder-specific rules"""
        relative_path = str(file_path.relative_to(self.repo_root))
    ...
    def check_frontend_rules(self, file_path: Path, content: str, lines: List[str]) -> None:
        ...
        if not '/api/' in relative_path and file_path.suffix in ['.ts', '.tsx', '.js', '.jsx']:
            # Check for forbidden imports
            forbidden_patterns = [
                (r'import.*from.*["\']stripe["\']', 'Server-side Stripe import in frontend'),
                (r'process\.env\.(?!NEXT_PUBLIC_)', 'Non-public environment variable'),
                (r'import.*from.*["\']\.\.\/.*backend', 'Cross-boundary import to backend'),
            ]
            ...
            for pattern, description in forbidden_patterns:
                ...
                for match in matches:
                    line_num = content[:match.start()].count('\n') + 1
                    self.violations.append(Violation(
                        relative_path, 'FRONTEND_VIOLATION',
                        description,
                        line_num, match.group()
                    ))
    ...
    def check_legacy_backend_rules(self, file_path: Path, content: str, lines: List[str]) -> None:
        ...
        if '/api/stripe/' in content:
            self.violations.append(Violation(
                relative_path, 'LEGACY_VIOLATION',
                'Stripe routes forbidden in legacy backend',
                0, '/api/stripe/ routes detected'
            ))
            self.stats['separation_violations'] += 1
    ...
    def check_ai_backend_rules(self, file_path: Path, content: str, lines: List[str]) -> None:
        ...
        for env_var in forbidden_env_vars:
            if env_var in content:
                line_num = next((i+1 for i, line in enumerate(lines) if env_var in line), 0)
                self.violations.append(Violation(
                    relative_path, 'AI_BACKEND_VIOLATION',
            ...
            if other_folder != current_folder:
                ...
                for pattern in patterns:
                    ...
                    for match in matches:
                        line_num = content[:match.start()].count('\n') + 1
                        self.violations.append(Violation(
                            relative_path, 'CROSS_IMPORT_VIOLATION',
    ...
    def check_port_collisions(self) -> None:
        ...
        expected_ports = {folder: config['dev_port'] for folder, config in self.folders.items() if 'dev_port' in config}
        ...
        for folder_name, expected_port in expected_ports.items():
            ...
            config_files = [
                ...
                'main.py', 'app.py', 'server.py', '.env.example'
            ]
    ...
    def check_port_in_file(self, file_path: Path, content: str, expected_port: int, folder_name: str) -> None:
        ...
        for pattern in port_patterns:
            ...
            for match in matches:
                ...
                if found_port != expected_port and found_port in [3000, 8000, 8001, 8002]:
                    line_num = content[:match.start()].count('\n') + 1
                    self.violations.append(Violation(
                        relative_path, 'PORT_COLLISION',
    ...
    def scan_repository(self) -> None:
        ...
        print(f"{Fore.CYAN}🛡️ Repository Guard - Scanning {self.repo_root}{Style.RESET_ALL}")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Repository Guard - Security & Quality Enforcement')
    ...
    if not any((repo_root / folder).exists() for folder in ['myhibachi-frontend', 'myhibachi-backend-fastapi']):
