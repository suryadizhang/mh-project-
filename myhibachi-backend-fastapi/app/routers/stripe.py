...
from app.models.stripe_models import (
    Customer, Payment, Invoice, WebhookEvent, Refund, Product, Price
)
from app.schemas.stripe_schemas import (
    CustomerCreate, CustomerUpdate, Customer as CustomerSchema,
    CreateCheckoutSession, CreatePaymentIntent, RefundCreate,
    CheckoutSessionResponse, PaymentIntentResponse, CustomerPortalResponse,
    WebhookResponse, Payment as PaymentSchema, Invoice as InvoiceSchema,
    PaymentAnalytics, SuccessResponse, ErrorResponse
)
...
async def create_checkout_session(
    ...
    try:
        ...
        success_url = data.success_url or f"{settings.app_url}/checkout/success?session_id={{CHECKOUT_SESSION_ID}}"
    ...
    try:
        ...
        try:
            ...
            if event_type == 'payment_intent.succeeded':
                ...
                await handle_payment_cancellation(
...
async def stripe_webhook_v1(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f'Webhook signature verification failed: {error_message}'
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            if event_type == 'payment_intent.succeeded':
                await handle_payment_success_simple(
                    event_data, stripe_service, db
                )
            elif event_type == 'customer.created':
                await handle_customer_created_simple(
                    event_data, stripe_service, db
                )
            elif event_type == 'invoice.payment_succeeded':
                ...
                await handle_subscription_update_simple(
                    event_data, stripe_service, db
                )
            else:
                logger.info(f"Unhandled event type {event_type}")
...
async def handle_payment_success(
    ...
    try:
        ...
        if customer_id:
            await stripe_service.update_payment_preferences(customer_id, {
...
async def handle_checkout_completed(
    session: dict, stripe_service: StripeService, db: AsyncSession
):
...
async def handle_charge_refunded(
    charge: dict, stripe_service: StripeService, db: AsyncSession
):
...
async def handle_dispute_created(
    dispute: dict, stripe_service: StripeService, db: AsyncSession
):
...
async def handle_quote_accepted(
    quote: dict, stripe_service: StripeService, db: AsyncSession
):
