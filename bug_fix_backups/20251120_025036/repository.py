...
class BaseRepository(IRepository[TModel]):
    ...
    def __init__(
        self, db: AsyncSession, model: type[TModel], soft_delete: bool = False
    ):
        self.db = db
        self.model = model
    ...
    async def get_by_id(
        self, id: UUID, eager_load: list[str] | None = None
    ) -> TModel | None:
    ...
    async def bulk_update(
        self, filters: list[FilterCriteria], update_data: dict[str, Any]
    ) -> int:
    ...
    def _build_filter_conditions(self, filters: list[FilterCriteria]) -> list:
        ...
        for filter_criteria in filters:
            if not hasattr(self.model, filter_criteria.field):
                logger.warning(
                    f"Field {filter_criteria.field} not found in {self.model.__name__}"
                )
                continue
