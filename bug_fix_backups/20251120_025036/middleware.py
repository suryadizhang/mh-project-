...
class JWTBearer(HTTPBearer):
    ...
    async def __call__(
        ...
        if credentials:
            ...
            # Verify token
            payload = self.auth_service.verify_jwt_token(
...
class AuthenticatedUser:
    ...
    def __init__(
        self, user: StationUser, session: UserSession, permissions: set[str]
    ):
        self.user = user
...
class RateLimiter:
    ...
    def is_allowed(
        ...
        key_requests[:] = [
            req for req in key_requests if req[0] > now - window_seconds
        ]
    ...
    def _cleanup_old_entries(self, cutoff_time: float):
        ...
        for key in list(self.requests.keys()):
            self.requests[key] = [
                req for req in self.requests[key] if req[0] > cutoff_time
            ]
