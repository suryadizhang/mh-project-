...
from utils.validators import ValidationError, validate_email, validate_phone
...
class LeadService(BaseService, EventTrackingMixin):
    ...
    async def create_lead(
        ...
        try:
            ...
            if contact_info.get("email"):
                try:
                    contact_info["email"] = validate_email(
                        contact_info["email"]
                    )
            ...
            if contact_info.get("phone"):
                try:
                    contact_info["phone"] = validate_phone(
                        contact_info["phone"]
                    )
                except ValidationError as e:
                    logger.error(
            ...
            if context:
                lead_context = LeadContext(
                    ...
                    estimated_budget_cents=context.get(
            ...
            logger.info(
                f"Created lead {lead.id} from source {source.value} with score {score}"
            )
    ...
    async def create_lead_with_consent(
        ...
        try:
            ...
            lead = await self.create_lead(
                source, contact_info, context, utm_params
            )
            ...
            if consent_data and (
                consent_data.get("sms_consent")
                or consent_data.get("email_consent")
            ):
                # Validate SMS consent if provided
                if consent_data.get("sms_consent") and contact_info.get(
                    "phone"
                ):
                    is_valid, reason = self.compliance.validate_sms_consent(
                        phone=contact_info["phone"],
                        consent_text=consent_data.get("consent_text", ""),
                        consent_method=consent_data.get(
                ...
                consent_record = self.compliance.create_consent_record(
                    ...
                    email=(
                        ...
                        if consent_data.get("email_consent")
                        else None
                    ),
                    ip_address=consent_data.get("ip_address"),
                    user_agent=consent_data.get("user_agent"),
                ...
                await self.track_event(
                    ...
                    metadata={
                        "sms_consent": consent_data.get("sms_consent", False),
                        "email_consent": consent_data.get(
                            "email_consent", False
                        ),
                        "consent_method": consent_data.get("consent_method"),
                        "source": source.value,
                ...
                if consent_data.get("sms_consent") or consent_data.get(
                    "email_consent"
                ):
                    await self._subscribe_to_newsletter(
                        lead, consent_data, contact_info
                    )
    ...
    async def capture_failed_booking(
        ...
        """
        context = {
            "party_size_adults": booking_data.get(
                "party_size", booking_data.get("guest_count")
            ),
            "event_date": booking_data.get(
                "event_date", booking_data.get("booking_datetime")
            ),
        ...
        logger.info(
            f"Captured failed booking as lead {lead.id}: {failure_reason}"
        )
    ...
    async def capture_quote_request(
        ...
        try:
            ...
            logger.error(
                f"Invalid phone in capture_quote_request: {e}",
                extra={"phone": phone},
            )
            raise BusinessLogicException(
                message=f"Invalid phone number: {e}",
        ...
        if email:
            try:
                ...
                logger.error(
                    f"Invalid email in capture_quote_request: {e}",
                    extra={"email": email},
                )
                raise BusinessLogicException(
                    message=f"Invalid email address: {e}",
        ...
        if budget:
            try:
                ...
                if numbers:
                    ...
                    if len(numbers) >= 2:
                        budget_cents = int(
                            (int(numbers[0]) + int(numbers[1])) / 2 * 100
                        )
                    else:
    ...
    async def capture_social_inquiry(
        ...
        logger.info(
            f"Captured social inquiry from {platform} as lead {lead.id}"
        )
    ...
    async def capture_phone_inquiry(
        ...
        lead = await self.create_lead(
            source=source, contact_info=contact_info, context=context
        )
    ...
    async def update_lead_status(
        ...
        logger.info(
            f"Updated lead {lead_id} status: {old_status.value} → {status.value}"
        )
