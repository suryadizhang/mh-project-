...
from cqrs.base import (
    CommandHandler,  # Phase 2C: Updated from api.app.cqrs.base
)
...
from models.legacy_core import (
    CoreCustomer,  # Phase 2C: Updated from api.app.models.core
)
from models.legacy_events import (
    OutboxEntry,  # Phase 2C: Updated from api.app.models.events
)
...
class CreateLeadFromSocialHandler(CommandHandler[CreateLeadFromSocialCommand]):
    ...
    async def handle(
        ...
        try:
            ...
            if existing_lead:
                logger.info(
                    f"Lead already exists for {command.handle} on {command.source}"
                )
                return {
...
class SendSocialReplyHandler(CommandHandler[SendSocialReplyCommand]):
    ...
    async def handle(
        ...
        try:
            # Get thread details
            thread_stmt = select(SocialThread).where(
                SocialThread.id == command.thread_id
            )
            ...
            message_data = {
                ...
                "metadata": {
                    **(command.metadata or {}),
                    ...
                    "scheduled_send_at": (
                        command.schedule_send_at.isoformat()
                        if command.schedule_send_at
                        else None
                    ),
                },
                "sent_at": command.schedule_send_at or datetime.now(UTC),
            }
            ...
            return {
                "message_id": message.id,
                "thread_id": command.thread_id,
                "sent": not command.requires_approval
                and not command.schedule_send_at,
                "scheduled": command.schedule_send_at is not None,
                "requires_approval": command.requires_approval,
            }
...
class LinkSocialIdentityToCustomerHandler(
    CommandHandler[LinkSocialIdentityToCustomerCommand]
):
    """Handler for linking social identities to customers."""

    async def handle(
        self,
        command: LinkSocialIdentityToCustomerCommand,
        ...
        try:
            ...
            if not identity:
                raise ValueError(
                    f"Social identity {command.social_identity_id} not found"
                )

            # Get customer
            customer_stmt = select(CoreCustomer).where(
                CoreCustomer.id == command.customer_id
            )
...
class CreateSocialAccountHandler(CommandHandler[CreateSocialAccountCommand]):
    ...
    async def handle(
        ...
        try:
            ...
            logger.info(
                f"Created social account {account.id} for {command.platform}"
            )
            return {
                "account_id": account.id,
                "created": True,
