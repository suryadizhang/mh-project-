...
class Business(Base):
    ...
    primary_color: str = Column(
        String(7), nullable=True, default="#FF6B6B"
    )  # Coral red
    secondary_color: str = Column(
        String(7), nullable=True, default="#4ECDC4"
    )  # Teal
    ...
    subscription_tier: str = Column(
        String(50), nullable=False, default="self_hosted"
    )
    ...
    @classmethod
    async def get_by_id(
        cls, session: AsyncSession, business_id: UUID
    ) -> Optional["Business"]:
        """Get business by UUID"""
        result = await session.execute(
            select(cls).where(cls.id == business_id)
        )
