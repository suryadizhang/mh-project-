...
class GrowthTracker:
    ...
    def __init__(self):
        ...
        self.customer_threshold = int(
            os.getenv("CUSTOMER_COUNT_THRESHOLD", "1000")
        )
        self.logger = logger
    ...
    async def check_customer_count(self, db: AsyncSession) -> dict:
        ...
        if (
            customer_count >= self.customer_threshold
            and not self._growth_alert_sent
        ):
            # Send alert
        ...
        return {
            ...
            "percent_to_threshold": (customer_count / self.customer_threshold)
            * 100,
        }

    async def _send_growth_alert(
        self, customer_count: int, recent_growth: int, threshold: int
    ):
...
6. Switch to Neo4j
        ...
        await self.alert_manager.send_email_alert(
            subject=f"🔥 Customer Growth Alert: {customer_count:,} customers",
            body=message,
        )
    ...
    async def get_growth_summary(self, db: AsyncSession) -> dict:
        ...
        neo4j_recommended = total_customers >= (
            self.customer_threshold * 0.8
        )  # 80% of threshold

        return {
            "total_customers": total_customers,
            "threshold": self.customer_threshold,
            "threshold_percent": (total_customers / self.customer_threshold)
            * 100,
            "growth": growth,
