...
class CateringPayment(BaseModel):
    ...
    booking_id = Column(
        Integer,
        ForeignKey("catering_bookings.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    payment_method = Column(
        SQLEnum(PaymentProvider), nullable=False, index=True
    )
    status = Column(
        String(50), default="pending", index=True
    ...
    notification = relationship(
        "PaymentNotification", back_populates="payment", uselist=False
    )
...
class PaymentNotification(BaseModel):
    ...
    )  # Customer's phone from payment note
    sender_username = Column(String(100))  # Venmo username
    ...
    booking_id = Column(
        Integer,
        ForeignKey("catering_bookings.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    payment_id = Column(
    ...
    payment = relationship(
        "CateringPayment", back_populates="notification", uselist=False
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_notification_manual_review",
