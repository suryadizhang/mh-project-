...
class QRTrackingService:
    ...
    async def track_qr_scan(
        ...
        stmt = select(QRCode).where(
            QRCode.code == code.upper(), QRCode.is_active
        )
    ...
    async def get_qr_analytics(
        ...
        unique_ips = len(
            {scan.ip_address for scan in scans if scan.ip_address}
        )
        unique_sessions = len(
            {scan.session_id for scan in scans if scan.session_id}
        )
        ...
        # Recent scans
        recent_scans = sorted(scans, key=lambda s: s.scanned_at, reverse=True)[
            :10
        ]
        ...
        return {
            ...
            "metrics": {
                ...
                "conversion_rate": (
                    round((bookings / unique_sessions * 100), 2)
                    if unique_sessions > 0
                    else 0
                ),
                "lead_rate": (
                    round((leads / unique_sessions * 100), 2)
                    if unique_sessions > 0
                    else 0
                ),
            },
