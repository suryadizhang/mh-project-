...
class OutboxWorkerBase:
        ...
        try:
            while self.running:
                try:
                    ...
                    logger.exception(
                        f"Worker error in {self.__class__.__name__}: {e}"
                    )
                    await asyncio.sleep(self.config.poll_interval_seconds)
    ...
    async def _get_pending_events(self, db: AsyncSession) -> list[OutboxEntry]:
        ...
        query = (
            ...
            .where(
                and_(
                    ...
                    OutboxEntry.next_attempt_at <= datetime.now(UTC),
                )
            )
            .order_by(OutboxEntry.created_at)
            .limit(
                self.config.batch_size * 4
            )  # fetch some extra to filter in-Python
        )
        ...
        selected = [
            e
            for e in candidates
            if (e.payload or {}).get("event_type") in supported
        ]
    ...
    async def _mark_event_processed(self, event_id: str, db: AsyncSession):
        ...
        await db.execute(
            ...
            .values(completed_at=datetime.now(UTC), status="completed")
        )

    async def _handle_event_error(
        self, event: OutboxEntry, error_message: str, db: AsyncSession
    ):
        ...
        if retry_count >= self.config.max_retries:
            ...
            logger.error(
                f"Event {event.id} failed after {retry_count} retries: {error_message}"
            )
        else:
            # Schedule for retry
            await db.execute(
...
class SMSWorker(OutboxWorkerBase):
    ...
    def __init__(
        ...
        rc_config: dict[str, str] | None = None,
    ):
    ...
    async def _process_event(self, event: OutboxEntry, db: AsyncSession):
        ...
        logger.info(
            f"SMS sent successfully for event {event.id} to {phone_number[:6]}***"
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    raise Exception(
                        f"SMS API error: {response.status} - {error_text}"
                    )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(
                        f"Token refresh error: {response.status} - {error_text}"
                    )
...
class EmailWorker(OutboxWorkerBase):
    ...
    async def _process_event(self, event: OutboxEntry, db: AsyncSession):
        ...
        await self._send_email(
            email_address, subject, html_body, text_body, attachments
        )

        logger.info(
            f"Email sent successfully for event {event.id} to {email_address}"
        )
    ...
    async def _send_via_ses(
        ...
        ses = boto3.client(
            ...
            aws_secret_access_key=self.email_config.get(
...
class StripeWorker(OutboxWorkerBase):
    ...
    async def _process_event(self, event: OutboxEntry, db: AsyncSession):
        ...
        logger.info(
            f"Stripe event {event_type} processed successfully for event {event.id}"
        )

    async def _process_payment_intent(
        ...
        if event_data.get("email_encrypted", False):
            field_encryption = get_field_encryption()
            customer_email = decrypt_field(
                customer_email, field_encryption.key
            )
        ...
        try:
            ...
            logger.info(
                f"Created PaymentIntent {payment_intent.id} for booking {booking_id}"
            )
    ...
    async def _process_customer_create(
        self, event_data: dict[str, Any], db: AsyncSession
    ):
        ...
        try:
            ...
            logger.info(
                f"Created Stripe customer {customer.id} for {customer_email}"
            )
...
def create_outbox_processor_manager(
    ...
    if app_config.get("ringcentral", {}).get("enabled", False):
        ...
        if all(rc_config.get(key) for key in required_rc_keys):
            sms_worker = SMSWorker(
                config=WorkerConfig(
                    max_retries=app_config.get("sms_worker", {}).get(
                        "max_retries", 5
                    ),
                    batch_size=app_config.get("sms_worker", {}).get(
                        "batch_size", 10
                    ),
                ),
                rc_config=rc_config,
            ...
            logger.warning(
                "⚠️ SMS Worker disabled - incomplete RingCentral configuration"
            )
    ...
    if app_config.get("email", {}).get("enabled", False):
        ...
        if config_valid:
            email_worker = EmailWorker(
                config=WorkerConfig(
                    max_retries=app_config.get("email_worker", {}).get(
                        "max_retries", 3
                    ),
                    batch_size=app_config.get("email_worker", {}).get(
                        "batch_size", 20
                    ),
                ),
            ...
            logger.warning(
                f"⚠️ Email Worker disabled - incomplete {provider} configuration"
            )
    ...
    if app_config.get("stripe", {}).get("enabled", False):
        ...
        if stripe_config.get("secret_key") and stripe_config[
            "secret_key"
        ].startswith(("sk_test_", "sk_live_")):
            stripe_worker = StripeWorker(
                config=WorkerConfig(
                    max_retries=app_config.get("stripe_worker", {}).get(
                        "max_retries", 3
                    ),
                    batch_size=app_config.get("stripe_worker", {}).get(
                        "batch_size", 5
                    ),
                ),
            ...
            logger.warning(
                "⚠️ Stripe Worker disabled - invalid or missing secret key"
            )
