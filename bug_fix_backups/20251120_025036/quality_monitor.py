...
class QualityMonitor:
    ...
    def get_alerts(
        self, severity: str | None = None, limit: int = 10
    ) -> list[dict[str, Any]]:
    ...
    async def get_quality_comparison(
        ...
        # Calculate metrics
        similarities = [
            p.similarity_score for p in pairs if p.similarity_score
        ]
        teacher_times = [
            p.teacher_response_time_ms
            for p in pairs
            if p.teacher_response_time_ms
        ]
        student_times = [
            p.student_response_time_ms
            for p in pairs
            if p.student_response_time_ms
        ]
        ...
        comparison = {
            ...
            "similarity": {
                ...
                "count_high_quality": len(
                    [s for s in similarities if s >= 0.85]
                ),
            },
            "response_time": {
                "teacher_avg_ms": (
                    int(sum(teacher_times) / len(teacher_times))
                    if teacher_times
                    else 0
                ),
                "student_avg_ms": (
                    int(sum(student_times) / len(student_times))
                    if student_times
                    else 0
                ),
            ...
            "cost_analysis": {
                "avg_teacher_cost_usd": (
                    round(sum(costs) / len(costs), 4) if costs else 0
                ),
        ...
        if comparison["response_time"]["teacher_avg_ms"] > 0:
            ...
            comparison["response_time"]["student_faster_percent"] = int(
                speedup
            )
