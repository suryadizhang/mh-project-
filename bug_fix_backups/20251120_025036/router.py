...
class MessageRouter:
    ...
    async def route_message(
        ...
        try:
            ...
            if request.channel == MessageChannel.SMS and request.phone_number:
                ...
                if not tcpa_allowed:
                    raise ValueError(
                        f"TCPA compliance failed for {request.phone_number}"
                    )
    ...
    async def process_inbound_message(
        ...
        try:
            logger.info(
                f"Processing inbound {channel} message from {sender_info}"
            )

            # Handle TCPA commands for SMS
            if channel == MessageChannel.SMS and sender_info.get(
                "phone_number"
            ):
    ...
    async def _notify_websocket_clients(
        ...
        try:
            ...
            logger.info(
                f"Notifying {len(connections)} WebSocket clients of {event_type}"
            )
...
class TCPAHandler:
    ...
    async def check_tcpa_compliance(
        self, phone_number: str, channel: MessageChannel
    ) -> bool:
    ...
    async def handle_tcpa_command(
        ...
        if command in ["STOP", "START"]:
            # Update TCPA status
            new_status = (
                TCPAStatus.OPTED_OUT
                if command == "STOP"
                else TCPAStatus.OPTED_IN
            )

            await self._update_tcpa_status(
                normalized_phone, channel, new_status, "sms_reply"
            )
...
class SMSHandler:
    ...
    async def send_message(
        self, message: Message, request: SendMessageRequest
    ) -> dict[str, Any]:
