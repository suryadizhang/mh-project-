...
class E2EFlowTester:
    ...
    async def test_scenario_1_casual_booking(self):
        ...
        for turn_data in conversation:
            ...
            knowledge_result = await self.test_knowledge_for_scenario(
                turn_data
            )
            ...
            if turn_data.get("upsell_trigger"):
                ...
                if upsell_result["success"]:
                    print(
                        "   ✅ Upsell: Offered add-ons (not premium upgrades)"
                    )
                    passed += 1
                else:
            ...
            if turn_data.get("should_calculate_price"):
                if (
                    knowledge_result.get("calculated_price")
                    == turn_data["expected_price"]
                ):
                    print(
                        f"   ✅ Pricing: Correctly calculated ${turn_data['expected_price']}"
                    )
                    passed += 1
                else:
                    print(
                        f"   ❌ Pricing: Expected ${turn_data['expected_price']}"
                    )
                    failed += 1
        ...
        return {
            "passed": passed,
            "failed": failed,
            "scenario": "Casual Birthday Party",
        }
    ...
    async def test_scenario_2_formal_corporate(self):
        ...
        return {
            "passed": passed,
            "failed": failed,
            "scenario": "Formal Corporate Event",
        }
