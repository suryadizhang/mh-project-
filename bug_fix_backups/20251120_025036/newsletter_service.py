...
class SubscriberService(BaseService, EventTrackingMixin):
    ...
    async def subscribe(
        ...
        if not phone and not email:
            raise ValueError(
                "Either phone or email is required for Subscriber subscription"
            )
        ...
        try:
            ...
            logger.error(
                f"Validation error in subscribe: {e}",
        ...
        if existing:
            ...
            if existing.unsubscribed_at:
                ...
                await self._send_welcome_message(
                    existing, name=name, resubscribe=True
                )
        ...
        subscription = Subscriber(
            ...
            consent_updated_at=datetime.now(UTC),
        )
    ...
    async def unsubscribe(
        ...
        try:
            ...
            logger.error(
                f"Validation error in unsubscribe: {e}",
                extra={"phone": phone, "email": email},
            )
            raise
    ...
    async def process_stop_command(
        self, phone: str, channel: str = "sms"
    ) -> tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.error(
                f"Validation error in process_stop_command: {e}",
                extra={"phone": phone},
            )
            raise
    ...
    async def process_start_command(
        ...
        try:
            ...
            logger.error(
                f"Validation error in process_start_command: {e}",
                extra={"phone": phone},
            )
            raise
    ...
    async def process_help_command(self, phone: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.error(
                f"Validation error in process_help_command: {e}",
                extra={"phone": phone},
            )
            raise
    ...
    async def find_by_contact(
        ...
        for subscriber in all_subscribers:
            try:
                ...
                logger.warning(
                    f"Failed to decrypt subscriber {subscriber.id}: {e!s}",
    ...
    async def _send_unsubscribe_confirmation(self, subscription: Subscriber):
        ...
        if subscription.email:
            await self._send_email(
                subscription.email,
                "Unsubscribed from My Hibachi Chef",
                message,
            )
    ...
    async def _send_sms(self, phone: str, message: str):
        ...
        try:
            ...
            logger.exception(
                f"Failed to send Subscriber SMS: {e!s}", extra={"phone": phone}
            )
    ...
    async def _send_email(self, email: str, subject: str, message: str):
        ...
        try:
            ...
            logger.exception(
                f"Failed to send Subscriber email: {e!s}",
                extra={"email": email},
            )
