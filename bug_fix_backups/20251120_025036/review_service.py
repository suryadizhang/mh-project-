...
class ReviewService:
    ...
    async def send_review_sms(
        ...
        try:
            ...
            if response.success:
                ...
                review.sms_sent_at = datetime.now(UTC)
        ...
        try:
            ...
            if rating in ["okay", "could_be_better"]:
                ...
                if rating == "okay":
                    ...
                    review.ai_escalated_at = datetime.now(UTC)
                ...
                # "Could be better" - Immediate escalation, AI handles coupon
                    ...
                    review.ai_escalated_at = datetime.now(UTC)
                    review.admin_notified_at = datetime.now(UTC)
    ...
    async def track_external_review(
        ...
        try:
            ...
            review.external_review_date = datetime.now(UTC)
            await self.db.commit()
    ...
    async def escalate_to_human(
        ...
        try:
            ...
            review.metadata["escalated_at"] = datetime.now(UTC).isoformat()
        ...
        if active_only:
            query = query.where(
                and_(
                    ...
                    DiscountCoupon.valid_until >= datetime.now(UTC),
                )
            )
    ...
    async def _check_existing_active_coupon(
        ...
        result = await self.db.execute(
            ...
            .where(
                ...
                DiscountCoupon.valid_until >= datetime.now(UTC),  # Not expired
            )
