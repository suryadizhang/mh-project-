...
class AIMessage(Base):
    ...
    __table_args__ = (
        Index(
            "idx_ai_messages_conversation_timestamp",
            conversation_id,
            timestamp,
        ),
        ...
        Index(
            "idx_ai_messages_emotion_history",
            conversation_id,
            emotion_score,
            timestamp,
        ),
        ...
        Index(
            "idx_ai_messages_message_metadata",
            message_metadata,
...
class PostgreSQLMemory(MemoryBackend):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            async with get_db_context() as db:
                ...
                count = result.scalar()
                logger.info(
                    f"PostgreSQL memory initialized - {count} conversations in database"
                )
        ...
        try:
            async with get_db_context() as db:
                ...
                result = await db.execute(
                    ...
                    .where(AIConversation.is_active)
                )
            ...
            async with get_db_context() as db:
                ...
                if emotion_score is not None:
                    task = asyncio.create_task(
                        self._update_emotion_stats_background(
                            conversation_id, emotion_score
                        )
                    )
                ...
                logger.debug(
                    f"Stored message {message_id} in conversation {conversation_id}"
                )
            ...
            async with get_db_context() as db:
                ...
                if channels:
                    ...
                    conv_query = conv_query.where(
                        AIConversation.channel.in_(channel_values)
                    )
    ...
    async def get_user_conversations(
        ...
        try:
            async with get_db_context() as db:
                ...
                return [
                    self._db_conversation_to_metadata(conv)
    ...
    async def update_conversation_metadata(
        ...
        try:
            async with get_db_context() as db:
                ...
                if emotion_score is not None:
                    await self._update_emotion_stats(
                        db, conversation, emotion_score
                    )
                ...
                if (
                    ...
                    logger.warning(
                        f"Conversation {conversation_id} escalated to human agent"
                    )
    ...
    async def close_conversation(
        self, conversation_id: str, reason: str | None = None
    ) -> None:
    ...
    async def get_escalated_conversations(
        ...
        try:
            async with get_db_context() as db:
                ...
                query = select(AIConversation).where(
                    and_(
                        AIConversation.escalated,
                        AIConversation.escalated_at >= cutoff,
                    )
                )

                if channel:
                    query = query.where(
                        AIConversation.channel == channel.value
                    )
            ...
            raise MemoryBackendError(
                f"Failed to get escalated conversations: {e}"
            )
    ...
    async def get_context_window(
        ...
        try:
            ...
            async with get_db_context() as db:
                ...
                # Return in chronological order
                return [
                    self._db_message_to_model(msg) for msg in reversed(context)
                ]
    ...
    async def get_statistics(
        ...
        try:
            async with get_db_context() as db:
                ...
                if user_id:
                    conv_filter = and_(
                        conv_filter, AIConversation.user_id == user_id
                    )
                ...
                if user_id:
                    query = query.join(AIConversation).where(
                        AIConversation.user_id == user_id
                    )
                ...
                escalation_rate = (
                    escalated_count / total_conversations
    ...
    def _db_message_to_model(
        self, db_message: AIMessage
    ) -> ConversationMessage:
    ...
    def _db_conversation_to_metadata(
        self, db_conv: AIConversation
    ) -> ConversationMetadata:
