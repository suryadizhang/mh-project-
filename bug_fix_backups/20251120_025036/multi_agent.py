...
class MultiAgentConfig:
    ...
    enable_parallel_execution: bool = (
        False  # Execute independent steps in parallel
    )
...
class PlannerAgent:
    ...
    async def create_plan(
        self, analysis: Analysis, context: dict[str, Any]
    ) -> ExecutionPlan:
        """
...
Create a detailed execution plan."""
        ...
        try:
            ...
            plan = ExecutionPlan(
                goal=plan_data["goal"],
                steps=steps,
                estimated_duration_ms=plan_data.get(
            ...
            self.logger.info(
                f"Plan created: {len(steps)} steps, goal: {plan.goal}"
            )
...
class ExecutorAgent:
    ...
    async def execute_plan(
        ...
        for step in plan.steps:
            try:
                if step.tool_name and self.tool_registry:
                    ...
                    if tool:
                        self.logger.debug(
                            f"Step {step.step_number}: Calling {step.tool_name}"
                        )
                    ...
                    self.logger.debug(
                        f"Step {step.step_number}: Info gathering"
                    )
                    step.result = {
                        "status": "completed",
                        "type": "information_step",
                    }
                    step.completed = True

            except Exception as e:
                self.logger.error(
                    f"Step {step.step_number} failed: {e}", exc_info=True
                )
        ...
        self.logger.info(
            f"Execution complete: {completed_steps}/{len(plan.steps)} steps succeeded"
        )
            ...
            execution_summary.append(
                {
                    ...
                    "result_preview": (
                        str(step.result)[:200] if step.result else None
                    ),
                }
            )
...
Review the execution and provide your critique."""
        ...
        try:
            ...
            critique = Critique(
                ...
                missing_requirements=critique_data.get(
    ...
    async def process(
        ...
        try:
            ...
            self._log_message(
                AgentRole.ANALYZER,
                None,
                f"Analysis complete: {analysis.problem_statement}",
            )
            ...
            while critique_cycles < self.config.max_critique_cycles:
                ...
                # Phase 2: Planning
                self.logger.info(
                    f"Phase 2: Planning (cycle {critique_cycles})"
                )
                ...
                # Phase 3: Execution
                self.logger.info(
                    f"Phase 3: Executing plan (cycle {critique_cycles})"
                )
                ...
                self._log_message(
                    AgentRole.EXECUTOR,
                    None,
                    f"Execution complete: {completed}/{len(executed_plan.steps)} steps",
                )

                # Phase 4: Critique
                self.logger.info(
                    f"Phase 4: Reviewing results (cycle {critique_cycles})"
                )
                critique = await self.critic.critique(
                    analysis, executed_plan, context
                )
                self._log_message(
                    AgentRole.CRITIC,
                    None,
            ...
            self.logger.error(
                f"Multi-agent processing failed: {e}", exc_info=True
            )
            duration_ms = int(
                (datetime.now(UTC) - start_time).total_seconds() * 1000
            )
            ...
            return MultiAgentResult(
                success=False,
                response=f"I apologize, but I encountered an error while processing your complex request: {e!s}",
                analysis=Analysis(
                    problem_statement=query,
                    requirements=[],
                    constraints=[],
...
Generate a helpful, complete response for the customer."""
    ...
    def _log_message(
        self, from_agent: AgentRole, to_agent: AgentRole | None, content: str
    ):
