...
class TestSchedulerIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        logger.info("=" * 80)
        logger.info(
            "INTEGRATION TEST SUITE: Follow-Up Scheduler with AI Orchestrator"
        )
        logger.info("=" * 80)
    ...
    async def _cleanup_test_data(self):
        ...
        try:
            async with get_db_context() as db:
                ...
                await db.execute(
                    text(
                        "DELETE FROM scheduled_followups WHERE user_id LIKE :pattern"
                    ),
                ...
                await db.execute(
                    text(
                        "DELETE FROM ai_messages WHERE conversation_id LIKE :pattern"
                    ),
                ...
                await db.execute(
                    text(
                        "DELETE FROM ai_conversations WHERE id LIKE :pattern"
                    ),
                    {"pattern": f"{self.test_conv_id}%"},
                )
                await db.commit()
    ...
    def test_01_orchestrator_initialization(self):
        ...
        async def run_test():
            ...
            self.assertIsNotNone(
                self.orchestrator.scheduler,
                "Orchestrator should have scheduler",
            )
    ...
    def test_03_emotion_based_template_selection(self):
        ...
        async def run_test():
            ...
            # Verify templates
            jobs_high = await self.scheduler.get_scheduled_followups(
                user_id=user_high
            )
            jobs_low = await self.scheduler.get_scheduled_followups(
                user_id=user_low
            )
            ...
            self.assertEqual(
                jobs_high[0].template_id, "post_event_high_emotion"
            )
            self.assertEqual(jobs_low[0].template_id, "post_event_low_emotion")

            logger.info(
                f"✅ PASSED: High emotion → {jobs_high[0].template_id}"
            )
    ...
    def test_04_inactive_user_detection(self):
        ...
        async def run_test():
            ...
            inactive_date = datetime.now(UTC) - timedelta(days=31)
            ...
            inactive_users = await detect_inactive_users(
                inactive_threshold_days=30, limit=100
            )

            # Find our test user
            test_user = next(
                (u for u in inactive_users if u["user_id"] == user_id), None
            )
            self.assertIsNotNone(
                test_user, f"Should detect inactive user {user_id}"
            )
            ...
            self.assertGreater(
                scheduled_count,
                0,
                "Should schedule at least one re-engagement",
            )
            ...
            reengagement_jobs = [
                j for j in jobs if j.trigger_type == "reengagement"
            ]

            self.assertGreater(
                len(reengagement_jobs), 0, "Should have re-engagement job"
            )
            logger.info(
                f"✅ PASSED: Scheduled {len(reengagement_jobs)} re-engagement(s)"
            )
    ...
    def test_05_message_callback_integration(self):
        ...
        async def run_test():
            ...
            followup = ScheduledFollowUp(
                ...
                scheduled_at=datetime.now(UTC),  # Execute now
            ...
            async with get_db_context() as db:
                ...
                self.assertEqual(
                    updated_followup.status, FollowUpStatus.EXECUTED.value
                )
                self.assertIsNotNone(updated_followup.executed_at)

            logger.info(
                "✅ PASSED: Follow-up executed and marked as completed"
            )
