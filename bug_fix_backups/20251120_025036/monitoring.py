...
class MonitoringService:
        ...
        try:
            ...
            if hasattr(response, "cost") and response.cost:
                self._update_cost_metrics(
                    agent,
                    getattr(response, "model_used", "unknown"),
                    response.cost,
                )
            ...
            logger.info(
                f"Response logged: {message_id} processed in {processing_time:.3f}s"
            )
    ...
    def _update_cost_metrics(
        self, agent: str, model: str, cost: dict[str, float]
    ):
        """Update cost metrics."""
        ...
        try:
            ...
            return {
                "summary": {
                    ...
                    "avg_response_time": filtered_metrics.get(
                        "avg_response_time", 0.0
                    ),
    ...
    async def _get_system_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            avg_response_time = self.metrics["performance"][
                "avg_response_time"
            ]
            ...
            if (
                ...
                status = "critical"
                issues.append(
                    f"Critical response time: {avg_response_time * 1000:.0f}ms"
                )
            elif (
                avg_response_time * 1000
                ...
                if status != "critical":
                    status = "warning"
                issues.append(
                    f"Slow response time: {avg_response_time * 1000:.0f}ms"
                )
    ...
    async def get_audit_trail(
        ...
        try:
            ...
            if agent:
                filtered_logs = [
                    log for log in filtered_logs if log.get("agent") == agent
                ]
