...
class EscalationService:
    ...
    async def create_escalation(
        ...
        try:
            ...
            if existing:
                logger.warning(
                    f"Escalation already exists for conversation {conversation_id}"
                )
                return existing
            ...
            try:
                ...
                from api.v1.websockets.escalations import (
                ...
                logger.info(
                    f"✅ Broadcasted escalation_created event for {escalation.id}"
                )
            ...
            self.db.rollback()
            logger.error(f"Failed to create escalation: {e!s}")
            raise RuntimeError(f"Failed to create escalation: {e!s}")

    async def get_escalation(self, escalation_id: str) -> Escalation | None:
        """Get escalation by ID"""
        return (
            self.db.query(Escalation)
            .filter(Escalation.id == UUID(escalation_id))
            .first()
        )

    async def assign_escalation(
        self, escalation_id: str, admin_id: str, notes: str | None = None
    ) -> Escalation:
            ...
            asyncio.create_task(
                broadcast_escalation_event(
                    ...
                    escalation_data={
                        ...
                        "assigned_at": (
                            escalation.assigned_at.isoformat()
    ...
    async def update_status(
        self, escalation_id: str, status: str, notes: str | None = None
    ) -> Escalation:
        ...
        logger.info(
            f"Updated escalation {escalation_id} status: {old_status} -> {status}"
        )
        return escalation
    ...
    async def resolve_escalation(
        ...
        if resume_ai:
            ...
            logger.info(
                f"AI conversation resumed for escalation {escalation_id}"
            )
        ...
        logger.info(
            f"Resolved escalation {escalation_id} by admin {resolved_by_id}"
        )
        return escalation
    ...
    async def list_escalations(
        ...
        if priority:
            query = query.filter(
                Escalation.priority == EscalationPriority(priority)
            )
        if assigned_to_id:
            query = query.filter(
                Escalation.assigned_to_id == UUID(assigned_to_id)
            )
        if from_date:
    ...
    async def get_stats(self) -> dict:
        ...
        resolved_escalations = (
            self.db.query(
                func.avg(
                    func.extract(
                        "epoch", Escalation.resolved_at - Escalation.created_at
                    )
                    / 3600
                )
            )
