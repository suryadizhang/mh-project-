...
class AuditLog(Base):
    ...
    # Change tracking
    old_values: dict[str, Any] | None = Column(JSONB, nullable=True)
    new_values: dict[str, Any] | None = Column(JSONB, nullable=True)
    changed_fields: list[str] | None = Column(ARRAY(Text), nullable=True)
    ...
    # Security tracking
    ip_address: str | None = Column(INET, nullable=True)
    ...
    # Relationships
    user = relationship(
        "models.user.User", foreign_keys=[user_id], back_populates="audit_logs"
    )
...
class SecurityAuditLog(Base):
    ...
    target_user_id: UUID | None = Column(
        PGUUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    user = relationship(
        ...
        back_populates="security_audit_logs",
    )
    target_user = relationship(
        "models.user.User", foreign_keys=[target_user_id]
    )
