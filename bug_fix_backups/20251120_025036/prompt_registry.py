...
"""
            ...
            try:
                ...
                logger.exception(
                    f"Failed to create prompt file for {agent}: {e}"
                )

    async def get_agent_prompt(
        self, agent: str, context: dict[str, Any] | None = None
    ) -> str:
        """
        ...
        try:
            ...
            if not base_prompt:
                logger.warning(
                    f"No prompt found for agent: {agent}, using fallback"
                )
...
"""
        ...
        try:
            ...
            for filename in os.listdir(self.prompts_dir):
                if filename.startswith("system.") and filename.endswith(".md"):
                    agent = filename[
                        7:-3
                    ]  # Remove "system." prefix and ".md" suffix
                    agents.append(agent)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            validation_results = {}
            for agent in agents:
                validation_results[agent] = await self.validate_agent_prompt(
                    agent
                )
