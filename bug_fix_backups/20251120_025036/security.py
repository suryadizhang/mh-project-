...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if (
            content_length
            and int(content_length) > SecurityConfig.MAX_REQUEST_SIZE
        ):
            if PROMETHEUS_AVAILABLE and SECURITY_VIOLATIONS:
                SECURITY_VIOLATIONS.labels(
                    violation_type="large_request"
                ).inc()
        ...
        if not user_agent or len(user_agent) > 1000:
            if PROMETHEUS_AVAILABLE and SECURITY_VIOLATIONS:
                SECURITY_VIOLATIONS.labels(
                    violation_type="invalid_user_agent"
                ).inc()

            return JSONResponse(
                status_code=400, content={"error": "Invalid user agent"}
            )
...
class AuditLogger:
    ...
    def log_authentication(
        self, user_id: str, success: bool, ip: str, details: dict | None = None
    ):
    ...
    def log_authorization(
        self, user_id: str, action: str, resource: str, success: bool
    ):
...
def require_https(func):
    ...
    async def wrapper(request: Request, *args, **kwargs):
        if (
            ...
            raise HTTPException(
                status_code=400, detail="HTTPS required in production"
            )
...
def setup_security_middleware(
    ...
    if enable_rate_limiting:
        app.add_middleware(
            RateLimitByIPMiddleware, requests_per_minute=rate_limit_requests
        )
    ...
    if webhook_secret:
        app.add_middleware(
            WebhookSecurityMiddleware, webhook_secret=webhook_secret
        )
