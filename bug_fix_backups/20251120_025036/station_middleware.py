...
from core.database import (
    get_db_session,  # Phase 2C: Updated from api.app.database
)
...
class AuthenticatedUser:
    ...
    def can_perform_cross_station_action(
        ...
        return self.station_context.can_perform_cross_station_action(
            permission
        )
...
async def get_current_station_user(
    ...
    try:
        ...
        # Verify JWT token
        token_payload = auth_service.verify_jwt_token(
        ...
        station_context = auth_service.extract_station_context_from_token(
            token_payload
        )
        if not station_context:
            # Fallback: get fresh station context from database
            station_context = await auth_service.get_user_station_context(
                db, user
            )
...
def require_station_permission(
    ...
    async def permission_checker(
        ...
        if (
            allow_cross_station
            and target_station != auth_user.current_station_id
        ):
...
def require_station_role(
    ...
    async def role_checker(
        ...
        user_role = auth_user.station_context.get_role_for_station(
            target_station
        )
        if not user_role:
            raise HTTPException(
...
async def audit_log_action(
    ...
    try:
        ...
        await auth_service.log_station_action(
            ...
            details=details,
            ip_address=(
                auth_user.request.client.host
                if auth_user.request.client
                else None
            ),
