...
from core.database import get_async_session, get_db_session
...
def fetch_call_recording(self, recording_id: str):
    ...
    try:
        ...
        logger.info(
            f"Downloading recording {recording.rc_recording_id} from RingCentral"
        )
        ...
        filename = f"recording_{recording.id}_{recording.call_started_at.strftime('%Y%m%d_%H%M%S')}.{ext}"
        ...
        logger.error(f"Failed to download recording {recording_id}: {e!s}")
...
def fetch_recording_transcript(self, recording_id: str, call_session_id: str):
    ...
    try:
        ...
        recording.rc_transcript_confidence = transcript_data.get(
            "confidence", 0
        )
        ...
        logger.error(f"Failed to fetch transcript for {recording_id}: {e!s}")

        # Retry if not last attempt
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying transcript fetch (attempt {self.request.retries + 1})"
            )
            raise self.retry(exc=e, countdown=180 * (2**self.request.retries))
        else:
            logger.error(
                f"All transcript fetch retries exhausted for {recording_id}"
            )
            return {
                "status": "error",
                "recording_id": recording_id,
...
def link_recording_entities(self, recording_id: str):
    ...
    async def async_link():
        ...
        async for db in get_async_session():
            try:
                ...
                result = await linking_service.link_recording(
                    UUID(recording_id)
                )
                return result
            finally:
    ...
    try:
        ...
        if result.get("error"):
            logger.error(
                f"Entity linking failed for {recording_id}: {result['error']}"
            )
        ...
        logger.error(
            f"Failed to link recording entities for {recording_id}: {e!s}",
            exc_info=True,
        )
    ...
    try:
        ...
        for recording in expired_recordings:
            try:
                ...
                logger.error(
                    f"Failed to delete recording {recording.id}: {e!s}"
                )
                continue
        ...
        logger.error(f"Failed to cleanup expired recordings: {e!s}")
...
def process_recording_metadata(recording_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to process recording metadata: {e!s}")
        return {"status": "error", "message": str(e)}
    ...
    try:
        ...
        for recording in old_recordings:
            try:
                ...
                logger.error(
                    f"Failed to archive recording {recording.id}: {e!s}"
                )
                continue
        ...
        logger.error(f"Failed to archive recordings: {e!s}")
        return {"status": "error", "message": str(e)}
