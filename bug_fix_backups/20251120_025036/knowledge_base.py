...
class KnowledgeBaseService:
    ...
    def __init__(self):
        ...
        # CRITICAL: Add async lock for thread safety
    ...
    def _load_index(self):
        ...
        try:
            ...
            logging.exception(f"Failed to load FAISS index: {e!s}")
    ...
    def _save_index(self):
        ...
        try:
            ...
            logging.warning(f"Failed to save FAISS index: {e!s}")
            # Don't fail the operation, just log the save failure
    ...
    async def add_chunk(
        ...
        async with self._index_lock:  # CRITICAL: Prevent race conditions
            try:
                # Generate embedding
                ...
                # Add to database first (with transaction)
                db.add(chunk)
                await db.commit()
                ...
                logging.exception(f"Failed to add knowledge base chunk: {e!s}")
    ...
    async def search_chunks(
        ...
        try:
            ...
            logging.exception(f"Failed to search knowledge base chunks: {e!s}")
            # Return empty results but log the error
            return [], 0
    ...
    async def rebuild_index(self, db: AsyncSession) -> bool:
        ...
        try:
            ...
            logging.exception(f"Failed to rebuild FAISS index: {e!s}")
    ...
    def get_similar_chunks_text(self, query: str, limit: int = 3) -> list[str]:
        ...
        try:
            ...
            logging.exception(f"Failed to get similar chunks: {e!s}")
            return []
