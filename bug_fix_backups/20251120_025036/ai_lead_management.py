...
class AILeadManager:
    ...
    async def analyze_social_engagement(
        self, post: SocialMediaPost
    ) -> dict[str, Any]:
        ...
        try:
            response = await self.openai.chat_completion(
                ...
                model="gpt-4o",
                temperature=0.3,  # GPT-4o for social analysis
            )

            return json.loads(response["content"])

        except Exception as e:
            logger.exception(f"Social engagement analysis failed: {e}")
            return {"intent_level": 50, "budget_indicators": 50, "urgency": 50}

    async def score_lead_with_ai(
        self, lead: Lead, conversation_data: list[dict] | None = None
    ) -> float:
        """
        ...
        lead_data = {
            ...
            "context": {
                "party_size": (
                    lead.context.party_size_adults if lead.context else None
                ),
                "event_date": (
                    lead.context.event_date_pref.isoformat()
                    if lead.context and lead.context.event_date_pref
                    else None
                ),
                "zip_code": lead.context.zip_code if lead.context else None,
                "budget_range": (
                    lead.context.budget_range if lead.context else None
                ),
            },
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}],
                model="gpt-4o",
                temperature=0.2,  # GPT-4o for precise scoring
            )
            ...
            logger.exception(f"AI lead scoring failed: {e}")
    ...
    def _fallback_lead_scoring(self, lead: Lead) -> float:
        ...
        if lead.context:
            if lead.context.event_date_pref:
                score += 20
            if (
                lead.context.party_size_adults
                and lead.context.party_size_adults > 6
            ):
                score += 15
    ...
    async def determine_lead_quality(
        self, lead: Lead, ai_score: float
    ) -> LeadQuality:
    ...
    async def suggest_next_actions(self, lead: Lead) -> list[str]:
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}],
                model="gpt-4o-mini",  # Fast suggestions
                temperature=0.5,
            )
            ...
            logger.exception(f"Next action suggestions failed: {e}")
    ...
    def _fallback_next_actions(self, lead: Lead) -> list[str]:
        ...
        elif (datetime.now(UTC) - lead.last_contacted_at).days > 3:
...
class SocialMediaAI:
    ...
    async def analyze_mention(
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}],
                model="gpt-4o",  # GPT-4o, temperature=0.3
            )

            return json.loads(response["content"])

        except Exception as e:
            logger.exception(f"Mention analysis failed: {e}")
            return {
                "intent_type": "inquiry",
                "urgency": 5,
                "tone": "friendly",
    ...
    async def generate_response(
        self, mention_content: str, analysis: dict[str, Any]
    ) -> str:
    ...
    async def create_promotional_content(
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}],
                model="gpt-4o",  # GPT-4o, temperature=0.8
            )
...
class LeadNurtureAI:
    ...
    def __init__(
        self, openai_service: OpenAIService, lead_manager: AILeadManager
    ):
        self.openai = openai_service
    ...
    async def create_nurture_sequence(
        ...
        if (
            ...
            and (lead.context.event_date_pref - datetime.now(UTC).date()).days
            <= 30
        ):
            sequence.append(
                {
    ...
    async def personalize_message(self, template: str, lead: Lead) -> str:
        ...
        try:
            ...
            logger.exception(f"Message personalization failed: {e}")
            return template
