        ...
        try:
            ...
            for thread in result["threads"][: kwargs.get("limit", 10)]:
                ...
                summary = {
                    ...
                    "customer": thread.get("customer_name")
                    or thread.get("handle"),
            ...
            return SocialToolResult(
                success=False, message=f"Failed to get social inbox: {e!s}"
            )
...
class GetThreadDetailTool:
    ...
    async def execute(self, **kwargs) -> SocialToolResult:
        ...
        try:
            ...
            query = GetThreadDetailQuery(
                ...
                include_customer_profile=kwargs.get(
            ...
            if result.get("customer_profile"):
                ...
                customer_context = {
                    ...
                    "social_handle": customer_profile.get(
                        "social_identity", {}
                    ).get("handle"),
            ...
            return SocialToolResult(
                success=False, message=f"Failed to get thread detail: {e!s}"
            )
    ...
    def _generate_conversation_summary(
        ...
        business_messages = [
            m for m in messages if m["direction"] == "outbound"
        ]
        ...
        try:
            ...
            command = SendSocialReplyCommand(
                ...
                safety={
                    ...
                    "profanity_check": safety_context.get(
                        "profanity_checked", False
                    ),
                    "policy_compliant": safety_context.get(
            ...
            if result["requires_approval"]:
                response_message = (
                    "Reply created and queued for human approval"
                )
            ...
            return SocialToolResult(
                success=False, message=f"Failed to send social reply: {e!s}"
            )

    async def _calculate_safety_score(
        self, message: str, safety_context: dict[str, Any]
    ) -> float:
        """Calculate safety score for message."""
        base_score = 0.8

        # Check profanity
        if safety_context.get(
            "profanity_checked"
        ) and not self._contains_profanity(message):
            base_score += 0.1
        ...
        if len(message) < 500 and not any(
            ...
            for word in ["refund", "discount", "free", "money"]
        ):
            base_score += 0.05
...
class CreateLeadFromSocialTool:
    ...
    async def execute(self, **kwargs) -> SocialToolResult:
        ...
        try:
            thread_id = (
                UUID(kwargs["thread_id"]) if kwargs.get("thread_id") else None
            )
            ...
            return SocialToolResult(
                success=False, message=f"Failed to create social lead: {e!s}"
            )
    ...
    def _infer_interest_category(self, signals: list[str]) -> str:
        ...
        if any(
            word in signals_text
            for word in ["party", "group", "people", "birthday", "celebration"]
        ):
            return "private_hibachi"
        elif any(
            word in signals_text
            for word in ["catering", "event", "office", "wedding"]
        ):
            return "catering"
        elif any(
            word in signals_text
            for word in ["class", "lesson", "learn", "teach"]
        ):
...
class SearchSocialContentTool:
    ...
    parameters = {
        ...
        "properties": {
            ...
            "content_types": {
                ...
                "items": {
                    "type": "string",
                    "enum": ["message", "comment", "review", "mention"],
                },
    ...
    async def execute(self, **kwargs) -> SocialToolResult:
        ...
        try:
            ...
            return SocialToolResult(
                success=False,
                message=f"Failed to search social content: {e!s}",
            )
...
class UpdateThreadStatusTool:
    ...
    async def execute(self, **kwargs) -> SocialToolResult:
        ...
        try:
            ...
            return SocialToolResult(
                success=False, message=f"Failed to update thread status: {e!s}"
            )
...
class SocialMediaToolKit:
    ...
    async def execute_tool(self, tool_name: str, **kwargs) -> SocialToolResult:
        ...
        if not tool:
            return SocialToolResult(
                success=False, message=f"Unknown tool: {tool_name}"
            )
