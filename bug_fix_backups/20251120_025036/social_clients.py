...
class RateLimitedClient:
    ...
    async def can_make_request(self) -> bool:
        ...
        async with self._lock:
            ...
            if (
                not self.rate_limits.minute_reset_at
                or now >= self.rate_limits.minute_reset_at
            ):
            ...
            # Reset hour counter
            if (
                not self.rate_limits.hour_reset_at
                or now >= self.rate_limits.hour_reset_at
            ):
            ...
            # Reset day counter
            if (
                not self.rate_limits.day_reset_at
                or now >= self.rate_limits.day_reset_at
            ):
            ...
            # Check all limits
            return (
                self.rate_limits.current_minute
                < self.rate_limits.requests_per_minute
                and self.rate_limits.current_hour
                < self.rate_limits.requests_per_hour
                and self.rate_limits.current_day
...
class InstagramClient(RateLimitedClient):
    ...
    async def send_direct_message(
        ...
        if not await self.can_make_request():
            ...
            logger.info(
                f"Instagram rate limit hit, waiting {wait_time:.2f} seconds"
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Instagram DM sent successfully: {result.get('id')}"
                        )
                        return {
                            "success": True,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Instagram comments fetch error: {response.status}"
                        )
                        return []
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Facebook Messenger error: {response.status} - {error_text}"
                        )
                        return {"success": False, "error": error_text}
...
class GoogleBusinessClient(RateLimitedClient):
    ...
    async def reply_to_review(
        self, review_name: str, reply_text: str
    ) -> dict[str, Any]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Google review reply error: {response.status} - {error_text}"
                        )
                        return {"success": False, "error": error_text}
    ...
    async def get_reviews(self) -> list[dict[str, Any]]:
        ...
        url = (
            f"{self.base_url}/accounts/*/locations/{self.location_id}/reviews"
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Google reviews fetch error: {response.status}"
                        )
                        return []
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Yelp reviews fetch error: {response.status}"
                        )
                        return []
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Yelp business info fetch error: {response.status}"
                        )
                        return None
...
class SocialClientManager:
    ...
    def add_facebook_client(
        self, account_id: str, access_token: str, page_id: str
    ):
        """Add Facebook client for account."""
        self.clients[f"facebook_{account_id}"] = FacebookClient(
            access_token, page_id
        )

    def add_google_client(
        self, account_id: str, access_token: str, location_id: str
    ):
        """Add Google Business client for account."""
        self.clients[f"google_{account_id}"] = GoogleBusinessClient(
