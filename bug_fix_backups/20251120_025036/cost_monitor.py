...
class CostMonitor:
    ...
    def __init__(self):
        ...
        self.daily_threshold = float(
            os.getenv("API_COST_DAILY_THRESHOLD", DAILY_ALERT_THRESHOLD)
        )
        self.logger = logger
    ...
    async def check_monthly_spend(self, db: AsyncSession) -> dict:
        ...
        if (
            current_spend >= self.monthly_threshold
            and not self._monthly_alert_sent
        ):
            # Send alert
...
5. Gradual rollout (1 week)
        ...
        await self.alert_manager.send_email_alert(
            subject=f"🔥 API Spend Alert: ${current_spend:.2f}/month",
            body=message,
        )

        self.logger.info(
            f"Sent cost alert: ${current_spend:.2f} >= ${threshold:.2f}"
        )

    async def _send_daily_warning(
        self, daily_spend: float, threshold: float, breakdown: dict
    ):
...
Consider adding local Llama 3 for cost savings.
    ...
    async def get_cost_summary(self, db: AsyncSession) -> dict:
        ...
        llama3_recommended = current_spend >= (
            self.monthly_threshold * 0.8
        )  # 80% of threshold
        ...
        return {
            "current_month": {
                ...
                "threshold": self.monthly_threshold,
                "threshold_percent": (current_spend / self.monthly_threshold)
                * 100,
            ...
            "recommendations": {
                "llama3_upgrade": {
                    ...
                    "reason": (
                        "API costs approaching $500/month"
                        if llama3_recommended
                        else None
                    ),
                }
            },
