...
class ConversationService:
    ...
    async def get_conversation_history(
        ...
        if not self.enable_threading:
            ...
            self.logger.debug(
                f"Threading disabled - returning empty history for {conversation_id}"
            )
            return []

        # Phase 3: Full history retrieval (placeholder)
        self.logger.info(
            f"Would retrieve history for conversation {conversation_id} (Phase 3)"
        )
        return []

    async def create_conversation(
        self, customer_id: str | None, channel: str, message: str
    ) -> str:
        """
        ...
        conversation_id = f"conv_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    ...
    async def add_message(
        ...
        if not self.enable_threading:
            ...
            self.logger.debug(
                f"Threading disabled - not storing message for {conversation_id}"
            )
            return

        # Phase 3: Store message (placeholder)
        self.logger.info(
            f"Would store message in conversation {conversation_id} (Phase 3)",
            extra={"role": role},
        )
