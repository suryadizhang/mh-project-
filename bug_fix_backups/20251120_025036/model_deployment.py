...
class ModelDeployment:
    ...
    async def deploy_shadow(
        self, new_model_id: str, traffic_split: float = 0.1
    ) -> dict[str, Any]:
        """
        ...
        self.logger.info(
            f"🚀 Shadow deployment: {new_model_id}", extra=deployment
        )
    ...
    async def compare_performance(
        ...
        try:
            ...
            base_messages = await self._get_model_messages(
                db, self.base_model, since
            )
            candidate_messages = await self._get_model_messages(
                db, self.candidate_model, since
            )
            ...
            recommendation = self._get_recommendation(
                winner, candidate_metrics, base_metrics
            )
            ...
            self.logger.info(
                f"Performance comparison complete: {winner} wins",
                extra=comparison,
            )
            ...
            self.logger.error(
                f"Error comparing performance: {e!s}", exc_info=True
            )
            raise
    ...
    def _calculate_metrics(self, messages: list[Any]) -> dict[str, float]:
        ...
        escalated = sum(
            1 for m in messages if m.metadata and m.metadata.get("escalated")
        )
        ...
        bookings = sum(
            1
            for m in messages
            if m.metadata and m.metadata.get("led_to_booking")
        )
    ...
    def _determine_winner(
        self, base_metrics: dict, candidate_metrics: dict
    ) -> str:
        """
    ...
    def _get_recommendation(
        self, winner: str, candidate_metrics: dict, base_metrics: dict
    ) -> str:
    ...
    async def promote_to_production(self, model_id: str) -> dict[str, Any]:
        ...
        self.logger.info(
            f"✅ PROMOTED: {model_id} → Production", extra=promotion
        )
    ...
    async def rollback(self) -> dict[str, Any]:
        ...
        self.logger.info(
            f"⏮️  ROLLED BACK: Removed {old_candidate}", extra=rollback
        )
