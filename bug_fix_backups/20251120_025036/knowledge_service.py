...
class KnowledgeService:
    ...
    async def _get_pricing_info(self) -> dict:
        ...
        deposit_rule = (
            ...
            .filter(
                and_(
                    BusinessRule.rule_type == "deposit",
        ...
        travel_fee_rule = (
            ...
            .filter(
                and_(
                    BusinessRule.rule_type == "travel_fee",
                    BusinessRule.is_active == True,
                )
            )
            .first()
    ...
    async def _get_menu_upgrades(self) -> list[dict]:
        ...
        try:
            ...
            logger.warning(
                f"Could not query addon_items table: {e}. Using defaults."
            )
            return default_upgrades
    ...
    async def get_cancellation_policy(self) -> str:
        ...
        policy = (
            ...
            .filter(
                and_(
                    BusinessRule.rule_type == "cancellation",
                    BusinessRule.is_active == True,
                )
            )
            .first()
...
"""
    ...
    async def get_faq_answer(self, question: str) -> dict | None:
        """
        ...
        try:
            ...
            if similarity_score < 0.6:
                logger.info(
                    f"FAQ match too weak ({similarity_score:.2%}) for: {question[:50]}..."
                )
                return None
            ...
            matched_faq = next(
                (
                    faq
                    for faq in faqs
                    if faq.question == best_match["question"]
                ),
                None,
            )
            if matched_faq:
            ...
            logger.warning(
                f"⚠️ Semantic FAQ search failed, falling back to keyword: {e}"
            )
    ...
    async def _faq_keyword_fallback(
        ...
        for faq in faqs:
            score = self._calculate_relevance(
                question.lower(), faq.question.lower()
            )
            if score > max_score:
                max_score = score
                best_match = faq
    ...
    async def get_training_examples(
        self, tone: str, scenario: str, limit: int = 3
    ) -> list[dict]:
        """
        ...
        return [
            {
                ...
                "tags": ex.tags,
                "effectiveness_score": (
                    float(ex.effectiveness_score)
                    if ex.effectiveness_score
                    else None
                ),
            }
            for ex in examples
    ...
    async def get_upsell_suggestions(
                ...
                suggestions.append(
                    {
                        ...
                        "success_rate": (
                            float(upsell.success_rate)
                            if upsell.success_rate
                            else None
                        ),
                    }
                )
        ...
        return suggestions[
            :3
        ]  # Return top 3 for natural conversation (not overwhelming)

    async def check_availability(
        self, requested_date: date, time_slot: str
    ) -> dict:
        """
    ...
    async def get_customer_tone_preference(
    ...
    async def update_customer_tone_preference(
        ...
        if preference:
            ...
            preference.tone_confidence = (
                float(preference.tone_confidence) * old_weight
            ...
            preference.last_interaction = datetime.now(UTC)
        else:
