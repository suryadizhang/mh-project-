...
class ConfidencePredictor:
    ...
    async def predict_confidence(
        ...
        for feature_name, feature_value in features.items():
            if feature_name in self.feature_weights:
                confidence += (
                    self.feature_weights[feature_name] * feature_value
                )
        ...
        similar_adjustment = await self._get_similar_conversation_adjustment(
            db, message, intent
        )
    ...
    def _extract_features(
        ...
        features["has_numbers"] = (
            1.0 if any(c.isdigit() for c in message) else 0.0
        )
        ...
        features["has_tech_terms"] = float(
            any(term in message.lower() for term in tech_terms)
        )
    ...
    async def train(self, db: AsyncSession):
        ...
        try:
            ...
            feature_names = list(
                self._extract_features("", "faq", None).keys()
            )
    ...
    async def _should_retrain(self, db: AsyncSession) -> bool:
        ...
        hours_since_training = (
            datetime.now(UTC) - self.last_training
        ).total_seconds() / 3600

        return (
            hours_since_training
            >= settings.ML_PREDICTOR_RETRAIN_INTERVAL_HOURS
        )
