...
class CursorPaginator:
    ...
    async def paginate(
        ...
        if include_total:
            ...
            count_query = sql_select(func.count()).select_from(
            ...
            if self.secondary_order and "secondary_value" in cursor_data:
                ...
                if self.order_direction == "desc":
                    secondary_comparison = (
                        self.secondary_order < secondary_value
                    )
                else:
                    secondary_comparison = (
                        self.secondary_order > secondary_value
                ...
                comparison = or_(
                    comparison,
                    and_(self.order_by == cursor_value, secondary_comparison),
                )
    ...
    def _decode_cursor(self, cursor: str) -> dict | None:
        ...
        try:
            ...
            if isinstance(value, str) and "T" in value:
                try:
                    cursor_dict["value"] = datetime.fromisoformat(
...
async def paginate_query(
    ...
    return await paginator.paginate(
        db=db, cursor=cursor, limit=limit, include_total=include_total
    )
