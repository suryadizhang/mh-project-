...
class IntegrationTestSuite:
    ...
    async def test_multi_turn_conversation(self):
        ...
        conversation_id = f"test_conv_{datetime.now(UTC).timestamp()}"
    ...
    async def test_intent_transitions(self):
        ...
        conversation_id = f"test_transition_{datetime.now(UTC).timestamp()}"
    ...
    async def test_fallback_handling(self):
        ...
        for message in ambiguous_messages:
            ...
            response = await self.router.route_with_fallback(
                message=message,
                context={
                    "conversation_id": f"test_fallback_{datetime.now(UTC).timestamp()}"
                },
    ...
    async def test_tool_calling(self):
        ...
        for test_case in tool_test_cases:
            request = OrchestratorRequest(
                ...
                channel="webchat",
                conversation_id=f"test_tools_{datetime.now(UTC).timestamp()}",
                customer_context={},
            )
    ...
    async def _run_test_case(self, agent_name: str, test_case: dict):
        ...
        request = OrchestratorRequest(
            message=test_case["message"],
            channel="webchat",
            conversation_id=f"test_{agent_name}_{datetime.now(UTC).timestamp()}",
            customer_context={"email": "test@example.com"},
        )
