...
from services.email.base import (
    EmailResult,  # We'll reuse this structure for consistency
)
    ...
    async def _update_subscriber_metrics(
        ...
        try:
            ...
            if sent:
                ...
                subscriber.last_sms_sent_date = datetime.now(UTC)

            if delivered:
                subscriber.total_sms_delivered += 1
                subscriber.last_sms_delivered_date = datetime.now(UTC)
            ...
            try:
                from api.v1.websockets.compliance import (
