...
class TestConcurrentLoadHandling:
    ...
    async def test_concurrent_cursor_pagination_requests(
        ...
        successes = [
            r for r in results if isinstance(r, dict) and r["status"] == 200
        ]
        failures = [
            r
            for r in results
        ...
        avg_duration = sum(r["duration_ms"] for r in successes) / len(
            successes
        )

        print(
            f"✅ Concurrent pagination: {len(successes)}/50 succeeded ({success_rate:.1f}%)"
        )
    ...
    async def test_concurrent_cte_analytics_requests(
        ...
        total_duration = performance_tracker.get_duration(
        ...
        print(
            f"✅ Concurrent analytics: {len(successes)}/30 succeeded ({success_rate:.1f}%)"
        )
    ...
    async def test_mixed_concurrent_requests(
        ...
        assert (
            success_rate >= 90
        ), f"Mixed success rate {success_rate:.1f}% below 90%"

        print(
            f"✅ Mixed concurrent: {len(successes)}/60 succeeded ({success_rate:.1f}%)"
        )
...
class TestConnectionPoolManagement:
    ...
    async def test_connection_pool_no_exhaustion(
        ...
        assert (
            error_rate < 5
        ), f"Error rate {error_rate:.1f}% too high: {errors[:5]}"

        print(
            f"✅ Connection pool: {100 - len(errors)}/100 succeeded ({100 - error_rate:.1f}%)"
        )
    ...
    async def test_connection_pool_concurrent_stress(
        ...
        assert (
            success_rate >= 85
        ), f"Stress test success rate {success_rate:.1f}% too low"

        print(
            f"✅ Connection pool stress: {len(successes)}/100 succeeded ({success_rate:.1f}%)"
        )
...
class TestThroughputMeasurement:
    ...
    async def test_cursor_pagination_throughput(
        ...
        duration_seconds = (
            performance_tracker.get_duration("pagination_throughput") / 1000
        )
        ...
        print(
            f"   {len(successes)}/{request_count} succeeded in {duration_seconds:.2f}s"
        )
    ...
    async def test_cte_analytics_throughput(
        ...
        duration_seconds = (
            performance_tracker.get_duration("analytics_throughput") / 1000
        )
...
class TestErrorRateMonitoring:
    ...
    async def test_error_rate_under_normal_load(
        ...
        assert (
            error_rate < 5
        ), f"Error rate {error_rate:.1f}% too high under normal load"

        print(
            f"✅ Normal load error rate: {error_rate:.1f}% ({successes}/{len(all_results)} succeeded)"
        )
    ...
    async def test_error_rate_under_heavy_load(
        ...
        assert (
            error_rate < 20
        ), f"Error rate {error_rate:.1f}% too high under heavy load"

        print(
            f"✅ Heavy load error rate: {error_rate:.1f}% ({successes}/{len(results)} succeeded)"
        )
    ...
    async def test_error_recovery(
        ...
        print(
            f"✅ Error recovery: First={first_success_rate:.1f}%, Second={second_success_rate:.1f}%"
        )
...
class TestLongRunningLoad:
    ...
    async def test_sustained_load_stability(
        ...
        while (
            datetime.utcnow() - start_time
        ).total_seconds() < 30 and request_count < 200:
            try:
                response = await async_client.get("/api/bookings?limit=20")
                if response.status_code == 200:
                    success_count += 1
        ...
        assert (
            success_rate > 80
        ), f"Success rate {success_rate:.1f}% dropped below 80%"

        print(
            f"✅ Sustained load: {success_count}/{request_count} succeeded ({success_rate:.1f}%)"
        )
        print(
            f"   Duration: {duration_seconds:.1f}s, Throughput: {throughput:.1f} req/sec"
        )
...
class TestDatabaseQueryLoad:
    ...
    async def test_concurrent_cte_query_execution(
        ...
        assert (
            success_rate > 85
        ), f"CTE concurrency success rate {success_rate:.1f}% too low"

        print(
            f"✅ Concurrent CTEs: {successful_queries}/{total_queries} succeeded ({success_rate:.1f}%)"
        )
