...
class AITutorPair(Base):
    ...
    context = Column(
        Text, nullable=True, comment="Conversation context/history"
    )
    agent_type = Column(
        String(50), nullable=True, comment="Which AI agent type"
    )

    # Teacher response (OpenAI)
    teacher_model = Column(
        String(50), nullable=False, comment="Teacher model name"
    )
    teacher_response = Column(
        Text, nullable=False, comment="Teacher's response"
    )
    ...
    student_model = Column(
        String(50), nullable=False, comment="Student model name"
    )
    student_response = Column(
        Text, nullable=False, comment="Student's response"
    )
    ...
    quality_score = Column(
        Float, nullable=True, comment="Overall quality assessment (0-1)"
    )
...
class AIRLHFScore(Base):
    ...
    # Human feedback
    reviewer_id = Column(
        Integer, nullable=True, comment="Admin user who reviewed"
    )

    # Scoring dimensions (1-5 scale)
    accuracy_score = Column(
        Integer, nullable=False, comment="How accurate is the response? (1-5)"
    )
    helpfulness_score = Column(
        Integer, nullable=False, comment="How helpful is the response? (1-5)"
    )
    tone_score = Column(
        Integer, nullable=False, comment="Appropriate tone for brand? (1-5)"
    )
    ...
    overall_score = Column(
        Float, nullable=False, comment="Average of all scores (1-5)"
    )
...
class AIExportJob(Base):
    ...
    # Job details
    export_type = Column(
        String(50), nullable=False, comment="fine_tuning/evaluation/backup"
    )
    status = Column(
    ...
    min_rlhf_score = Column(
        Float, nullable=True, comment="Minimum RLHF score if available"
    )
    ...
    pairs_exported = Column(
        Integer, default=0, comment="Number of pairs exported"
    )
    output_file_path = Column(
        String(500), nullable=True, comment="Path to JSONL file"
    )
    ...
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
    created_by = Column(
        Integer, nullable=True, comment="Admin user who initiated"
    )
