...
from models.call_recording import CallDirection, CallRecording, CallStatus
...
class RingCentralVoiceService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.voice_ai_enabled = (
            os.getenv("ENABLE_VOICE_AI", "false").lower() == "true"
        )
        self.default_mode = VoiceAIMode(
            os.getenv("VOICE_AI_MODE", VoiceAIMode.TRIAGE.value)
        )
        self.max_call_duration = int(
            os.getenv("MAX_CALL_DURATION_SECONDS", "1800")
        )  # 30 min

        # Call routing
        self.business_hours_start = int(
            os.getenv("BUSINESS_HOURS_START", "9")
        )  # 9 AM
        self.business_hours_end = int(
            os.getenv("BUSINESS_HOURS_END", "17")
        )  # 5 PM
    ...
    async def handle_inbound_call(
        ...
        try:
            ...
            logger.info(
                f"📞 Inbound call: {from_number} → {to_number} (session: {session_id})"
            )
            ...
            if routing["action"] == "ai_handle":
                ...
                return await self._transfer_to_human(
                    call_record, routing["agent_number"]
                )
                ...
                return await self._ai_triage_then_transfer(
                    call_record, call_data
                )
        ...
        try:
            ...
            result = await self.db.execute(
                select(CallRecording).where(
                    CallRecording.rc_call_id == session_id
                )
            )
    ...
    async def _handle_with_ai(
        ...
        # Convert to speech
        audio_stream = self.speech_service.synthesize_speech(
            greeting, streaming=True
        )
    ...
    async def _transfer_to_human(
        ...
        call_record.transferred_at = datetime.now(UTC)
        await self.db.commit()
    ...
    async def _send_to_voicemail(
        self, call_record: CallRecording
    ) -> dict[str, Any]:
    ...
    async def _determine_call_routing(
        self, call_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if (
            self.voice_ai_enabled
            and self.default_mode == VoiceAIMode.FULL_AUTO
        ):
    ...
    async def _start_recording(self, call_record: CallRecording) -> None:
        ...
        call_record.recording_started_at = datetime.now(UTC)
        await self.db.commit()
    ...
    async def _handle_call_hangup(
        ...
        if call_record.answered_at:
            duration = (
                call_record.ended_at - call_record.answered_at
            ).total_seconds()
    ...
    async def get_call_statistics(
        ...
        result = await self.db.execute(
            select(CallRecording).where(
                CallRecording.started_at >= start_date,
                CallRecording.started_at <= end_date,
            )
        )
        ...
        transferred_calls = len(
            [c for c in calls if c.status == CallStatus.TRANSFERRED]
        )
        voicemails = len(
            [c for c in calls if c.status == CallStatus.VOICEMAIL]
        )

        total_duration = sum((c.duration_seconds or 0) for c in calls)
        avg_duration = (
            total_duration / answered_calls if answered_calls > 0 else 0
        )
        ...
        return {
            ...
            "answer_rate": (
                (answered_calls / total_calls * 100) if total_calls > 0 else 0
            ),
