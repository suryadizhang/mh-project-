...
async def create_memory_backend(
    ...
    if backend_type is None:
        backend_str = os.getenv(
            "MEMORY_BACKEND", DEFAULT_BACKEND.value
        ).lower()
        try:
    ...
    try:
        ...
        if backend_type == MemoryBackendType.POSTGRESQL:
            ...
            neo4j_user = kwargs.get("neo4j_user") or os.getenv(
                "NEO4J_USER", "neo4j"
            )
            neo4j_password = kwargs.get("neo4j_password") or os.getenv(
                "NEO4J_PASSWORD"
            )
            ...
            logger.warning(
                "Neo4j memory backend is a STUB - not yet implemented"
            )
            memory = Neo4jMemory(
                neo4j_uri=neo4j_uri,
        ...
        logger.info(
            f"✅ {backend_type.value.upper()} memory backend initialized successfully"
        )
...
def get_configured_backend() -> MemoryBackendType:
    ...
    try:
        ...
        logger.warning(
            f"Invalid MEMORY_BACKEND: {backend_str}, returning default"
        )
