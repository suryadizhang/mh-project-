...
class UnifiedNotificationService:
        ...
        if self.provider == "twilio" and TWILIO_AVAILABLE:
            try:
                ...
                if account_sid and auth_token:
                    ...
                    self.sms_from = os.getenv(
                        "TWILIO_SMS_NUMBER", "+19167408768"
                    )
                    logger.info("✅ Twilio WhatsApp service initialized")
                else:
                    logger.warning(
                        "⚠️ Twilio credentials missing - using mock mode"
                    )
                    self.provider = "mock"
                    self.twilio_client = None
            except Exception as e:
                logger.exception(
                    f"❌ Twilio initialization failed: {e} - using mock mode"
                )
                self.provider = "mock"
                self.twilio_client = None
        else:
            ...
            if self.provider != "mock":
                logger.warning(
                    f"⚠️ Provider '{self.provider}' not supported yet - using mock mode"
                )
                self.provider = "mock"

        logger.info(
            f"📱 Notification Service: Provider={self.provider}, Admin={self.admin_phone}"
        )
    ...
    def _is_quiet_hours(self) -> bool:
        ...
        ):  # e.g., 22:00 to 8:00 (crosses midnight)
            return (
                current_hour >= self.quiet_start
                or current_hour < self.quiet_end
            )
...
Check admin portal for details."""
    ...
    async def _send_message(
        ...
        if not to_phone:
            logger.warning(
                f"❌ No phone number provided for {notification_type}"
            )
            return {
                "success": False,
        ...
        if self.provider == "twilio" and self.twilio_client:
            ...
            # Fallback to SMS
            logger.info(
                f"📱 WhatsApp failed, trying SMS fallback for {notification_type}"
            )
            return await self._send_via_twilio_sms(to_phone, message)

        # No provider available
        logger.error(
            f"❌ No notification provider available for {notification_type}"
        )
        return {
            "success": False,
            "channel": NotificationChannel.FAILED,
    ...
    async def _send_mock(
        ...
        logger.info("=" * 60)
        logger.info(
            f"📱 MOCK NOTIFICATION - {notification_type.value.upper()}"
        )
    ...
    async def _send_via_twilio_whatsapp(
        ...
        try:
            ...
            logger.info(
                f"✅ WhatsApp sent to {to_phone} - SID: {response.sid}"
            )
...
async def notify_new_booking(
    customer_name: str, customer_phone: str, **kwargs
):
    """Send new booking notification"""
    service = get_notification_service()
    return await service.send_new_booking_notification(
        customer_name, customer_phone, **kwargs
    )
...
async def notify_booking_edit(
    ...
    service = get_notification_service()
    return await service.send_booking_edit_notification(
        customer_name, customer_phone, **kwargs
    )
...
async def notify_cancellation(
    ...
    service = get_notification_service()
    return await service.send_cancellation_notification(
        customer_name, customer_phone, **kwargs
    )
...
async def notify_payment(customer_name: str, customer_phone: str, **kwargs):
    ...
    service = get_notification_service()
    return await service.send_payment_confirmation(
        customer_name, customer_phone, **kwargs
    )
...
async def notify_complaint(customer_name: str, customer_phone: str, **kwargs):
    ...
    service = get_notification_service()
    return await service.send_complaint_notification(
        customer_name, customer_phone, **kwargs
    )
