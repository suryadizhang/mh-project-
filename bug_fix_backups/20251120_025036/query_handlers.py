            ...
            if query.include_messages and booking_data.get("message_threads"):
                for thread in booking_data["message_threads"]:
                    if thread.get("phone_number"):
                        thread["phone_number"] = self.encryption.decrypt(
                            thread["phone_number"]
                        )
    ...
    async def handle(self, query: GetBookingsQuery) -> QueryResult:
        ...
        try:
            ...
            if query.customer_email:
                ...
                conditions.append(
                    "bd.customer_email_encrypted = :customer_email"
                )
            ...
            if query.customer_phone:
                ...
                conditions.append(
                    "bd.customer_phone_encrypted = :customer_phone"
                )
            ...
            return QueryResult(
                success=True, data=bookings, total_count=total_count
            )
    ...
    async def handle(self, query: GetCustomer360Query) -> QueryResult:
        ...
        try:
            ...
            if query.customer_id:
                ...
                params = {
                    "identifier": self.encryption.encrypt(query.customer_email)
                }
            elif query.customer_phone:
                where_condition = "c360.customer_phone_encrypted = :identifier"
                params = {
                    "identifier": self.encryption.encrypt(query.customer_phone)
                }
            else:
                return QueryResult(
                    success=False,
        ...
        try:
            ...
            if query.include_customer_details and thread_data.get("customer"):
                ...
                if customer.get("name"):
                    customer["name"] = self.encryption.decrypt(
                        customer["name"]
                    )
                if customer.get("email"):
                    customer["email"] = self.encryption.decrypt(
                        customer["email"]
                    )
            ...
            if thread_data.get("messages"):
                for message in thread_data["messages"]:
                    if message.get("content_encrypted"):
                        message["content"] = self.encryption.decrypt(
    ...
    async def handle(self, query: GetDashboardStatsQuery) -> QueryResult:
        ...
        try:
            ...
            if query.include_bookings:
                booking_stats = await self._get_booking_stats(
