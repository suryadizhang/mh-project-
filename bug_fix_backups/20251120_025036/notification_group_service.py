...
class NotificationGroupService:
        ...
        if station_id:
            conditions.append(
                or_(
                    ...
                    NotificationGroup.station_id.is_(
                        None
                    ),  # Include all-stations groups
                )
            )
    ...
    async def remove_member(self, member_id: UUID) -> bool:
        ...
        query = select(NotificationGroupMember).where(
    ...
    async def remove_event_subscription(self, event_id: UUID) -> bool:
        ...
        query = select(NotificationGroupEvent).where(
            NotificationGroupEvent.id == event_id
        )
        result = await self.db.execute(query)
    ...
    async def get_recipients_for_event(
        ...
        query = (
            ...
            .join(
                NotificationGroupEvent,
                NotificationGroup.id == NotificationGroupEvent.group_id,
            )
            .join(
                NotificationGroupMember,
            ...
            .where(
                and_(
                    ...
                    or_(
                        ...
                        NotificationGroupEvent.event_type
        ...
        logger.info(
            f"📧 Found {len(recipients)} recipients for {event_type.value} event"
        )
    ...
    async def initialize_default_groups(
        ...
        for group_config in DEFAULT_GROUPS:
            ...
            query = select(NotificationGroup).where(
                NotificationGroup.name == group_config["name"]
            )
            result = await self.db.execute(query)
            ...
            if existing:
                logger.info(
                    f"⏭️  Group '{group_config['name']}' already exists"
                )
                continue
