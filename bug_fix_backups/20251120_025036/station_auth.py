...
class StationContext:
    ...
    def __init__(
        ...
        self.station_roles = {
            assignment["station_id"]: assignment["role"]
        ...
        if highest_role:
            self.highest_role = (
                StationRole(highest_role)
                if isinstance(highest_role, str)
                else highest_role
            )
        else:
    ...
    def is_admin(self) -> bool:
        ...
        return self.highest_role in {
            StationRole.SUPER_ADMIN,
            StationRole.ADMIN,
        }
    ...
    def can_access_station(self, station_id: UUID) -> bool:
        ...
        return can_access_station(
            self.highest_role, station_id, list(self.accessible_station_ids)
        )

    def has_permission_in_station(
        self, permission: StationPermission, station_id: UUID
    ) -> bool:
    ...
    def can_perform_cross_station_action(
        self, permission: StationPermission
    ) -> bool:
...
class StationAuthenticationService(BaseAuthenticationService):
    ...
    def __init__(
        ...
        self.station_token_lifetime = timedelta(
            hours=4
        )  # Shorter for station tokens
    ...
    async def get_user_station_context(
        ...
        try:
            ...
            stmt = (
                ...
                .where(
                    and_(
                        ...
                        UserStationAssignment.is_active,
                    )
                )
            ...
            for station_user, station in assignments:
                ...
                permissions = get_station_permissions(
                    role, station_user.additional_permissions
                )
    ...
    def create_station_aware_jwt_tokens(
        ...
        current_station_id = (
            target_station_id or station_context.current_station_id
        )
        station_role = station_context.get_role_for_station(current_station_id)
        station_permissions = station_context.get_permissions_for_station(
            current_station_id
        )
        ...
        access_payload = {
            ...
            "station_context": {
                ...
                "accessible_stations": [
                    str(sid) for sid in station_context.accessible_station_ids
                ],
    ...
    async def create_station_access_token(
        ...
        try:
            ...
            permissions = station_context.get_permissions_for_station(
                station_id
            )
            ...
            station_token = StationAccessToken(
                ...
                role=role.value,
                permissions=list(permissions),
                expires_at=datetime.now(UTC) + self.station_token_lifetime,
            )
        ...
        try:
            ...
            stmt = select(StationAccessToken).where(
                and_(
                    ...
                    StationAccessToken.expires_at > datetime.now(UTC),
                )
            )
    ...
    async def revoke_station_tokens(
        ...
        try:
            ...
            conditions = [
                StationAccessToken.user_id == user_id,
                not StationAccessToken.is_revoked,
            ]
    ...
    def extract_station_context_from_token(
        ...
        try:
            ...
            current_station_id = UUID(
                station_context_data["current_station_id"]
            )
            accessible_stations = [
                UUID(sid)
                for sid in station_context_data["accessible_stations"]
            ]
            ...
            context.station_permissions = {
                sid: set(station_context_data["station_permissions"])
                for sid in accessible_stations
            }
            context.highest_role = StationRole(
                station_context_data["highest_role"]
            )
    ...
    async def switch_station_context(
        ...
        try:
            ...
            if not current_station_context.can_access_station(
                target_station_id
            ):
                return None
            ...
            await self.log_station_action(
                ...
                details={
                    "from_station": str(
                        current_station_context.primary_station_id
                    ),
