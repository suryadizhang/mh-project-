...
from models.legacy_events import (
    IdempotencyKey,  # Phase 2C: Updated from api.app.models.events
)
...
class CreateBookingCommandHandler(CommandHandler):
    ...
    async def handle(self, command: CreateBookingCommand) -> CommandResult:
        ...
        try:
            ...
            booking = CoreBooking(
                ...
                status="confirmed",
                balance_due_cents=command.total_due_cents
                - command.deposit_due_cents,
            ...
            await self.outbox_processor.create_outbox_entries(
                ...
                targets=(
                    ["email", "stripe"]
                    if command.deposit_due_cents > 0
                    else ["email"]
                ),
            )
    ...
    async def _check_slot_availability(
        self, date, slot: str, party_size: int
    ) -> bool:
    ...
    async def _find_or_create_customer(
        self, email: str, name: str, phone: str
    ) -> Customer:
        """Find existing customer or create new one."""
        # Try to find by email first
        encrypted_email = self.encryption.encrypt(email)

        stmt = select(CoreCustomer).where(
            CoreCustomer.email_encrypted == encrypted_email
        )
        result = await self.session.execute(stmt)
    ...
    async def _check_idempotency(
        ...
        if idem:
            if idem.status == "completed":
                ...
                return CommandResult(
                    success=False, error="Command previously failed"
                )
            else:
                return CommandResult(
                    success=False, error="Command is still processing"
                )

        return None

    async def _save_idempotency(
        self, key: str, command_type: str, result_data: dict[str, Any]
    ):
        ...
        idem = IdempotencyKey(
            ...
            + timedelta(days=7),  # Keep for a week
        )
...
class RecordPaymentCommandHandler(CommandHandler):
    ...
    async def handle(self, command: RecordPaymentCommand) -> CommandResult:
        ...
        try:
            ...
            booking_stmt = select(CoreBooking).where(
            ...
            if not booking:
                return CommandResult(
                    success=False,
                    error=f"Booking {command.booking_id} not found",
                )

            if booking.status == "cancelled":
                return CommandResult(
                    success=False,
                    error="Cannot record payment for cancelled booking",
                )
            ...
            if new_balance == 0:
                ...
                total_paid + command.amount_cents >= booking.deposit_due_cents
            ):
        ...
        try:
            ...
            if not thread:
                ...
                thread_stmt = (
                    ...
                    .where(
                        MessageThread.phone_number_encrypted == encrypted_phone
                    )
            ...
            if not thread:
                ...
                thread = CoreMessageThread(
                    id=thread_id,
                    phone_number_encrypted=self.encryption.encrypt(
            ...
            return CommandResult(
                ...
                data={
                    "message_id": str(message_id),
                    "thread_id": str(thread.id),
                    "created_new_thread": thread.created_at
                    == thread.updated_at,
                },
                events=[event],
            )
