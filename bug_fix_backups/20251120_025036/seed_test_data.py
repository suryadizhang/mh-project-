...
class DataSeeder:
        ...
        for i in range(count):
            ...
            result = await self.session.execute(
                ...
                ),
                {
                    ...
                    "phone": fake.phone_number()[:20],
                    "address": (
                        fake.street_address() if random() > 0.3 else None
                    ),
    ...
    async def seed_bookings(self, count: int = 5000):
        ...
        for i in range(count):
            ...
            if booking_date < datetime.now(UTC) - timedelta(days=7):
                status = choice(
                    [
                        "completed",
                        "completed",
            ...
            elif booking_date > datetime.now(UTC):
    ...
    async def seed_payments(self, count: int = 3000):
        ...
        bookings_with_payments = sample(
            self.booking_ids, min(count, len(self.booking_ids))
        )
        ...
        for i, booking_id in enumerate(bookings_with_payments[:count]):
            ...
            booking_result = await self.session.execute(
                text(
                    "SELECT customer_id, total_amount, created_at FROM bookings WHERE id = :id"
                ),
                {"id": booking_id},
            )
...
async def main():
    ...
    async with async_session() as session:
        ...
        try:
            ...
            print(
                "✅ You can now run the backend server and test all endpoints.\n"
            )
