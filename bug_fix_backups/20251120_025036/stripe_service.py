...
from models.legacy_core import (
    CorePayment,  # Phase 2C: Updated from api.app.models.core
)
...
class StripeService:
    ...
    async def _handle_invoice_payment_succeeded(
        ...
        if invoice_record:
            ...
            invoice_record.paid_at = datetime.now(UTC)
            await self.db.commit()
    ...
    async def _handle_StripeCustomer_created(
        self, StripeCustomer: dict[str, Any]
    ) -> None:
    ...
    async def _update_payment_status(
        ...
        result = await self.db.execute(
            select(CorePayment).where(
                CorePayment.stripe_payment_intent_id == payment_intent_id
            )
        )
    ...
    async def _update_StripeCustomer_analytics(
        ...
        result = await self.db.execute(
            select(StripeCustomer).where(
                Customer.stripe_StripeCustomer_id == StripeCustomer_id
            )
        )
