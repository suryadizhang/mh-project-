...
class OrchestratorRequest(BaseModel):
    ...
    message: str = Field(
        ..., description="Customer inquiry message", min_length=1
    )
    channel: str = Field(
        default="email",
...
class OrchestratorConfig(BaseModel):
    ...
    max_tokens: int = Field(
        default=1500, ge=100, le=4000, description="Maximum tokens in response"
    )
    enable_rag: bool = Field(
        default=False, description="Enable RAG knowledge base (Phase 3)"
    )
    enable_voice: bool = Field(
        default=False, description="Enable voice AI (Phase 3)"
    )
    enable_threading: bool = Field(
        default=False, description="Enable conversation threading (Phase 3)"
    )
    enable_identity: bool = Field(
        default=False, description="Enable identity resolution (Phase 3)"
    )
