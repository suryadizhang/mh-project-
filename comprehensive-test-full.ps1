...
$aiTests = @(
    ...
    "Can I add lobster tail?",
    "I need help with my booking"
)

foreach ($msg in $aiTests) {
    $aiData = @{ message = $msg; user_id = "test-$(Get-Random)"; channel = "web" } | ConvertTo-Json
    $response = Test-Endpoint "AI: $($msg.Substring(0,20))..." "POST" "/api/v1/ai/chat" $aiData
    if ($response) {
        Write-Host "    Intent: $($response.intent) | Confidence: $($response.confidence)" -ForegroundColor Gray
    }
}
...
$quoteData = @{
    adults = 35; children = 3; filet_mignon = 10
    lobster_tail = 8; yakisoba_noodles = 5
    venue_address = "123 Main St, Sacramento, CA 95814"; zip_code = "95814"
} | ConvertTo-Json
$quoteResponse = Test-Endpoint "Quote Calculator" "POST" "/api/v1/public/quote/calculate" $quoteData
if ($quoteResponse) {
    Write-Host "    Total: `$$($quoteResponse.grand_total)" -ForegroundColor Gray
}

# SECTION 5: Payment Systems
Write-Host "`n=== Section 5: Payments ===" -ForegroundColor Yellow
$paymentData = @{ base_amount = 2000; tip_amount = 300; payment_method = "card" } | ConvertTo-Json
...
if ($headers) {
    Test-Endpoint "List Bookings" "GET" "/api/bookings/" -headers $headers
    Test-Endpoint "Weekly Stats" "GET" "/api/bookings/admin/weekly" -headers $headers
    Test-Endpoint "Monthly Stats" "GET" "/api/bookings/admin/monthly" -headers $headers
}
...
if ($headers) {
    ...
    if ($campaign -and $campaign.id) {
        ...
        Write-Host "  ℹ️  Campaign created but NOT sent (test mode)" -ForegroundColor Cyan
    }
    
    # AI Content Generation
    $aiContentData = @{ topic = "Holiday Specials"; tone = "friendly"; length = "medium" } | ConvertTo-Json
...
if ($headers) {
    Test-Endpoint "Admin Users" "GET" "/admin/users" -headers $headers
    Test-Endpoint "Admin Roles" "GET" "/admin/roles" -headers $headers
    Test-Endpoint "All Permissions" "GET" "/admin/roles/permissions/all" -headers $headers
}

# SECTION 9: CRM & Leads
Write-Host "`n=== Section 9: CRM & Leads ===" -ForegroundColor Yellow
if ($headers) {
    ...
    Test-Endpoint "CRM Dashboard" "GET" "/api/crm/crm/dashboard/stats" -headers $headers
}

# SECTION 10: Stripe Integration
Write-Host "`n=== Section 10: Stripe Payments ===" -ForegroundColor Yellow
if ($headers) {
    Test-Endpoint "Stripe Dashboard" "GET" "/api/stripe/v1/customers/dashboard" -headers $headers
    Test-Endpoint "Stripe Payments" "GET" "/api/stripe/payments" -headers $headers
    Test-Endpoint "Stripe Analytics" "GET" "/api/stripe/analytics/payments" -headers $headers
}
...
Write-Host "`nTested: Infrastructure, Auth, AI/ML, Payments, Bookings, Newsletter, Admin, CRM, Stripe" -ForegroundColor Gray
$completionTime = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
Write-Host "Completed: $completionTime" -ForegroundColor Gray
Write-Host ""
