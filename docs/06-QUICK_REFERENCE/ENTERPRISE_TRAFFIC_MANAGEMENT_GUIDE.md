...
async function searchBlog(query: string) {
  if (!rateLimiter.canMakeRequest('/api/blog', 'search')) {
    ...
    throw new Error(
      `Rate limit exceeded. Try again in ${Math.ceil(remaining / 1000)}s`
    );
  }
...
Your current implementation is **PERFECT** for 1,000-10,000 users.
Follow this roadmap as you scale:
