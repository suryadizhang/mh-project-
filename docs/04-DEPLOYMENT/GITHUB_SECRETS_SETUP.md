...
> **Last Updated:** February 2, 2026 **Purpose:** Required secrets for
> CI/CD workflows
...
For production deployments, configure a **production** environment
with approval:
...
- [ ] `CF_SSH_HOSTNAME` - SSH hostname (e.g.,
      `ssh.mhapi.mysticdatanode.net`)
- [ ] `VPS_USER` - SSH user (`root`)
- [ ] `VPS_STAGING_PATH` - Staging path
- [ ] `VPS_PRODUCTION_PATH` - Production path (can be same as staging
      if using same directory)
...
3. Click **"Run workflow"** → Select `main` branch → Click **"Run
   workflow"**
