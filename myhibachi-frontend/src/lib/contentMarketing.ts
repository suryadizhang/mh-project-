...
export const generateNewsletterContent = (featured: number[] = [], seasonal: number[] = []) => {
  const featuredPosts = featured.map(id => blogPosts.find(p => p.id === id)).filter(Boolean)
  const seasonalPosts = seasonal.map(id => blogPosts.find(p => p.id === id)).filter(Boolean)
...
function calculateSocialPotential(post: typeof blogPosts[0]): number {
  ...
  if (trendingEvents.some(event => post.eventType.includes(event))) score += 15
...
function calculateConversionPotential(post: typeof blogPosts[0]): number {
  ...
  if (highIntentEvents.some(event => post.eventType.includes(event))) score += 25
