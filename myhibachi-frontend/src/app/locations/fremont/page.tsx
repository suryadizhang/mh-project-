...
const locationData = generateLocationPage('Fremont')

export const metadata: Metadata = {
  title: locationData.title,
  description: locationData.metaDescription,
  keywords: 'Fremont hibachi catering, hibachi chef Fremont, East Bay hibachi, Fremont party catering, private hibachi chef Fremont, Fremont birthday hibachi, family hibachi Fremont, Fremont wedding hibachi',
  openGraph: {
    title: locationData.title,
    description: locationData.metaDescription,
    type: 'website',
  ...
  alternates: {
    canonical: '/locations/fremont'
  }
}

export default function FremontHibachiPage() {
  return (
    <div className="min-h-screen">
      {/* Schema Markup */}
      <LocalBusinessSchema
        businessName="MyHibachi Fremont"
        location="Fremont"
        description="Professional hibachi catering service in Fremont, California - East Bay's premier family hibachi entertainment"
      />
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center text-gray-900 mb-12">
            Fremont Hibachi Catering Services
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {locationData.content.services.map((service, index) => (
              <div key={index} className="bg-white p-6 rounded-lg shadow-sm text-center">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{service}</h3>
                <p className="text-gray-600 text-sm">Professional hibachi entertainment for your Fremont celebration</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4">
          <div className="prose prose-lg max-w-none"
               dangerouslySetInnerHTML={{ __html: locationContentBlocks.whyChooseUs('Fremont').replace(/\n/g, '<br />') }} />
        </div>
      </section>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4">
          <div className="prose prose-lg max-w-none"
               dangerouslySetInnerHTML={{ __html: locationContentBlocks.popularEvents('Fremont').replace(/\n/g, '<br />') }} />
        </div>
      </section>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4">
          <div className="prose prose-lg max-w-none"
               dangerouslySetInnerHTML={{ __html: locationContentBlocks.serviceAreas('Fremont').replace(/\n/g, '<br />') }} />
        </div>
      </section>
      ...
      <section className="py-16 bg-orange-600 text-white text-center">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-3xl font-bold mb-6">
            Ready to Book Your Fremont Hibachi Experience?
          </h2>
          <p className="text-xl mb-8">
            Contact us today for a free quote on your Fremont hibachi catering event!
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link
              href="/booking"
      ...
      <Assistant page="/locations/fremont" />
    </div>
  )
}
