...
export default function RelatedPosts({ currentPost, allPosts, maxPosts = 3 }: RelatedPostsProps) {
  const getRelatedPosts = (): BlogPost[] => {
    ...
    const scoredPosts = otherPosts.map(post => {
      ...
      const daysDiff = Math.abs((postDate.getTime() - currentPostDate.getTime()) / (1000 * 60 * 60 * 24))
      if (daysDiff <= 30) {
        score += 1
      }
    ...
    <div className="bg-gray-50 rounded-lg p-6 mt-8">
      ...
      <div className="grid gap-4">
        {relatedPosts.map((post) => (
          <article key={post.id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
