...
export default function ContentSeries({ posts, maxSeries = 6, className = '' }: ContentSeriesProps) {
  ...
  const generateSeries = (): ContentSeries[] => {
    ...
    if (weddingPosts.length >= 2) {
      series.push({
        ...
        description: 'Everything you need to plan the perfect hibachi wedding reception, from intimate ceremonies to grand celebrations.',
    ...
    const corporatePosts = posts.filter(post => 
      post.keywords?.some(keyword => 
    ...
    if (corporatePosts.length >= 2) {
      series.push({
        ...
        description: 'Professional guide to hosting successful corporate hibachi events, team building, and business entertainment.',
    ...
    if (familyPosts.length >= 3) {
      series.push({
        ...
        description: 'Create memorable family moments with hibachi catering for birthdays, anniversaries, and special gatherings.',
    ...
    const seasonalPosts = posts.filter(post => 
      post.seasonal || 
      post.keywords?.some(keyword => 
        ['spring', 'summer', 'fall', 'winter', 'holiday', 'christmas', 'thanksgiving'].includes(keyword.toLowerCase())
      )
    )
    if (seasonalPosts.length >= 3) {
      series.push({
        ...
        description: 'Year-round hibachi event planning with seasonal menus, decorations, and celebration ideas.',
    ...
    const bayAreaPosts = posts.filter(post => 
      post.serviceArea && !['All Areas', 'General'].includes(post.serviceArea)
    )
    if (bayAreaPosts.length >= 4) {
      series.push({
        id: 'bay-area-locations',
        title: 'Bay Area Hibachi Locations Guide',
        description: 'Location-specific hibachi catering guides for San Francisco, Silicon Valley, Peninsula, and surrounding areas.',
    ...
    if (techPosts.length >= 2) {
      series.push({
        ...
        description: 'Hibachi catering for tech companies, startups, and Silicon Valley corporate culture.',
      ...
      <div className="p-6">
        <div className="flex items-center space-x-2 mb-6">
          ...
          <span className="text-sm text-gray-500">
            ({contentSeries.length} collections)
          </span>
        </div>
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      {series.difficulty && (
                        <span className={`text-xs px-2 py-1 rounded ${
                          series.difficulty === 'Beginner' ? 'bg-green-100 text-green-800' :
                          series.difficulty === 'Intermediate' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="border-t border-gray-200 bg-gray-50 p-4">
                  <div className="grid gap-3">
                    {series.posts.map((post, index) => (
                      <div key={post.id} className="flex items-center space-x-3 bg-white p-3 rounded-lg border border-gray-200">
                        {/* Step Number */}
