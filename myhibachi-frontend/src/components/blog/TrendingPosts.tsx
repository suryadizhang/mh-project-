              ...
              {showTrend && (
                <div className="flex items-center space-x-1">
                  ...
                  <span className={`text-xs ${
                    post.trend === 'up' ? 'text-emerald-600' : 
                    post.trend === 'down' ? 'text-red-600' : 'text-slate-400'
                  }`}>
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-1 bg-slate-100 rounded-lg p-1">
            <button className={`px-2 py-1 text-xs rounded ${timeframe === 'week' ? 'bg-white shadow-sm' : ''}`}>
              Week
            </button>
            <button className={`px-2 py-1 text-xs rounded ${timeframe === 'month' ? 'bg-white shadow-sm' : ''}`}>
              Month
            </button>
            <button className={`px-2 py-1 text-xs rounded ${timeframe === 'all' ? 'bg-white shadow-sm' : ''}`}>
              All
            </button>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {trendingPosts.map((post, index) => (
          <div key={post.id} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-slate-50 transition-colors">
