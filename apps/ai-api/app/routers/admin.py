...
class AdminChatResponse(BaseModel):
    ...
    debug_info: Optional[dict] = Field(None, description="Debug information for admin")
...
async def test_admin_chat(
    ...
    try:
        ...
        } if request.test_mode else None
        ...
        logger.error(f"Admin chat test failed: {e}")
        raise HTTPException(
    ...
    try:
        ...
        for hour in range(24):
            hourly_activity.append({
                "hour": hour,
                "messages": 0  # Would need proper query implementation
            })
...
async def get_admin_conversations(
    ...
    try:
        ...
        logger.error(f"Error retrieving admin conversations: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
