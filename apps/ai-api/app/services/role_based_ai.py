...
class RoleBasedAIService:
    ...
    def __init__(self):
        ...
        self.customer_restrictions = [
            "admin", "management", "staff", "employee", "configure", 
            "settings", "analytics", "financial", "revenue", "profit",
            "user_data", "private", "internal", "backend", "database"
        ]
    ...
    def _filter_staff_response(self, response: str) -> str:
        ...
        sensitive_admin_content = ["financial", "profit", "revenue", "system_configuration", "database"]
        response_lower = response.lower()
...
"""
        ...
        if role == UserRole.CUSTOMER:
            ...
            for keyword in unauthorized_keywords:
                if keyword in message:
                    validation_result["warnings"].append(f"Customer attempted to access admin function: {keyword}")
    ...
    def get_booking_capabilities(self, role: UserRole) -> Dict[str, bool]:
        ...
        capabilities = {
            ...
            "can_manage_all_bookings": self.can_perform_action(role, AICapability.BOOKING_MANAGEMENT)
        }
    ...
    def get_management_capabilities(self, role: UserRole) -> Dict[str, bool]:
        ...
        capabilities = {
            ...
            "can_manage_operations": self.can_perform_action(role, AICapability.RESTAURANT_OPERATIONS),
            "can_configure_system": self.can_perform_action(role, AICapability.SYSTEM_CONFIGURATION)
        }
    ...
    def log_ai_interaction(self, role: UserRole, request: Dict[str, Any], response: str):
        ...
        if role == UserRole.CUSTOMER and any(word in request.get("message", "").lower() 
                                           for word in self.customer_restrictions):
            logger.warning(f"Customer attempted restricted access: {request.get('message', '')[:50]}")
...
def create_role_aware_response(role: UserRole, original_response: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    management_caps = role_based_ai.get_management_capabilities(role) if role != UserRole.CUSTOMER else {}
    
    return {
        "content": filtered_response,
        "role": role.value,
        "capabilities": {
            "booking": booking_caps,
