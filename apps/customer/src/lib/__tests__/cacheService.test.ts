...
describe('CacheService', () => {
  ...
  describe('Cache Strategies', () => {
    ...
    describe('network-first strategy', () => {
      ...
      it('should throw if network fails and no cache available', async () => {
        ...
        await expect(cacheService.networkFirst('test-key', 5000, fetcher)).rejects.toThrow(
          'Network error',
        );
      });
    });
  ...
  describe('Error Handling', () => {
    it('should handle fetcher errors gracefully', async () => {
      ...
      await expect(cacheService.cacheFirst('error-key', 5000, fetcher)).rejects.toThrow(
        'Fetch failed',
      );
    });
