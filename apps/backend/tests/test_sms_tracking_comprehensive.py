...
class TestSubscriberModel:
    ...
    def test_recalculate_engagement_score_no_activity(
        self, db_session, sample_subscriber
    ):
    ...
    def test_recalculate_engagement_score_high_delivery(
        self, db_session, sample_subscriber
    ):
    ...
    def test_recalculate_engagement_score_high_failure(
        self, db_session, sample_subscriber
    ):
    ...
    def test_recalculate_engagement_score_recent_activity(
        self, db_session, sample_subscriber
    ):
        ...
        # Set old activity
        sample_subscriber.last_sms_delivered_date = (
...
class TestCampaignModel:
    ...
    def test_update_cached_metrics_no_events(
        self, db_session, sample_campaign
    ):
    ...
    def test_update_cached_metrics_with_deliveries(
        self, db_session, sample_campaign, campaign_event
    ):
    ...
    def test_update_cached_metrics_with_failures(
        self, db_session, sample_campaign, campaign_event
    ):
...
class TestNewsletterSMSService:
    ...
    async def test_send_campaign_sms_success(
        ...
        delivery_event = (
            db_session.query(SMSDeliveryEvent)
            .filter_by(campaign_event_id=1)
            .first()
        )
    ...
    async def test_send_without_tracking(self, mock_ringcentral):
        ...
        result = await sms_service.send_campaign_sms(
            to_number="+15551234567", message="Test message"
        )
...
class TestRingCentralWebhook:
    ...
    async def test_handle_sms_delivery_status_delivered(
        ...
        payload = {"messageId": "msg-123", "messageStatus": "Delivered"}

        with patch(
            "api.v1.webhooks.ringcentral.get_db_session",
...
class TestCampaignMetricsTasks:
    ...
    def test_update_active_campaign_metrics(self, db_session, sample_campaign):
        ...
        sample_campaign.last_metrics_updated = datetime.utcnow() - timedelta(
            minutes=10
        )
        db_session.commit()

        with patch(
            "workers.campaign_metrics_tasks.get_db_session",
            return_value=iter([db_session]),
        ):
        ...
        assert (
            sample_campaign.last_metrics_updated
    ...
    def test_cleanup_completed_campaign_metrics(
        self, db_session, sample_campaign
    ):
...
class TestSMSTrackingIntegration:
    ...
    async def test_complete_sms_delivery_flow(
        ...
        payload = {"messageId": message_id, "messageStatus": "Delivered"}
        ...
        delivery_event = (
            db_session.query(SMSDeliveryEvent)
            .filter_by(ringcentral_message_id=message_id)
            .first()
        )
        assert delivery_event is not None
        assert delivery_event.status == SMSDeliveryStatus.DELIVERED
...
class TestTCPACompliance:
    ...
    def test_audit_trail_creation(self, db_session, campaign_event):
        ...
        event = (
            db_session.query(SMSDeliveryEvent)
            .filter_by(ringcentral_message_id="msg-audit-123")
            .first()
        )
