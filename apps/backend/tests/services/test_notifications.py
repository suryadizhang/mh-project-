...
async def test_with_details():
    ...
    # Test Data
    test_customer = {"name": "Suryadi Zhang", "phone": "+19167408768"}
    ...
    result = await service.send_booking_edit_notification(
        ...
        booking_id=test_booking["id"],
        changes={
            "event_date": {"old": "November 15, 2025", "new": "November 20, 2025"},
    ...
    print("\n🚀 Next Steps:")
    print("   1. Set META_PAGE_ACCESS_TOKEN and META_PHONE_NUMBER_ID in .env")
    print("   2. Complete Meta Business verification")
...
async def quick_test():
    ...
    await service.send_cancellation_notification(
        "Suryadi Zhang", "+19167408768", booking_id=123, event_date="Nov 15", refund_amount=150.00
    )
    ...
    await service.send_review_notification(
        "Suryadi Zhang", rating=5, review_text="Amazing service!", booking_id=123
    )

    await service.send_complaint_notification(
        "Suryadi Zhang",
        "+19167408768",
        complaint_text="Chef was late",
        booking_id=123,
        priority="medium",
    )
