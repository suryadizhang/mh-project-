...
class PrivacyTestSuite:
    ...
    def test(self, name: str, condition: bool, expected: str = "", actual: str = ""):
        ...
        if condition:
            ...
            self.results["failed"].append({"name": name, "expected": expected, "actual": actual})
            print(f"  [FAIL] {name}")
    ...
    async def test_initials_generation(self):
        ...
        for name, expected_initials in test_cases.items():
            ...
            self.test(
                f"Initials for '{name}'", actual == expected_initials, expected_initials, actual
            )
    ...
    async def test_display_name_logic(self):
        ...
        # Test full name mode
        review1 = CustomerReviewBlogPost(customer_name="John Doe", show_initials_only=False)
        self.test(
        ...
        # Test initials mode
        review2 = CustomerReviewBlogPost(customer_name="John Doe", show_initials_only=True)
        self.test(
        ...
        # Test empty name
        review3 = CustomerReviewBlogPost(customer_name="", show_initials_only=False)
        self.test(
    ...
    async def test_data_integrity(self):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                # Retrieve and verify
                result = await session.execute(
                    select(CustomerReviewBlogPost).filter(CustomerReviewBlogPost.id == review_id)
                )
                ...
                if retrieved:
                    ...
                    self.test(
                        "Store customer_email", retrieved.customer_email == "test@example.com"
                    )
                    ...
                    self.test(
                        "Data persistence", False, "Review saved and retrieved", "Review not found"
                    )
    ...
    async def test_api_privacy_filtering(self):
        ...
        self.test("Public API shows initials only", display_name == "AJ", "AJ", display_name)

        # Verify email/phone are not in display name
        self.test("Email not in display name", "alice@secret.com" not in display_name)
        self.test("Phone not in display name", "555-1234" not in display_name)
