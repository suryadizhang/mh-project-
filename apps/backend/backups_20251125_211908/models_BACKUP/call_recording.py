...
class CallRecording(Base):
    ...
    # Table configuration - remove schema as it doesn't exist
    ...
    # Business context - simplified (no foreign keys to avoid migration issues)
    # These are stored as UUIDs for reference but not enforced at DB level
    # RingCentral is external system, we just store metadata here
    booking_id = Column(UUID(as_uuid=True), nullable=True, index=True)
    customer_id = Column(UUID(as_uuid=True), nullable=True, index=True)
    escalation_id = Column(UUID(as_uuid=True), nullable=True, index=True)
    # agent_id removed - no User model exists
    # agent_id = Column(
    #     UUID(as_uuid=True),
    ...
    # last_accessed_by_id removed - no User model exists
    ...
    # Relationships - simplified, no actual DB relationships since RingCentral is external
    # These would need to be joined manually in queries if needed
    # booking = relationship("Booking", ...)  # Removed - external system
    # customer = relationship("Customer", ...)  # Removed - external system
    # escalation = relationship("Escalation", ...)  # Removed - external system
    ...
    def to_dict(self):
        return {
            ...
            # "agent_id": str(self.agent_id) if self.agent_id else None,  # Removed - no User model
            # "agent_name": self.agent.full_name if self.agent else None,  # Removed - no User model
    ...
    def record_access(self, user_id: UUID):
        ...
        # self.last_accessed_by_id = user_id  # Removed - no User model exists
