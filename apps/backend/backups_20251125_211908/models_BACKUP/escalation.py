...
class Escalation(Base):
    ...
    conversation_id = Column(
        UUID(as_uuid=True),
        nullable=True,  # no FK - Conversation model doesn't exist
        index=True,
    )
    customer_id = Column(
        UUID(as_uuid=True),
    ...
    # Relationships
    # conversation relationship removed - Conversation model doesn't exist
    customer = relationship("models.customer.Customer", back_populates="escalations", lazy="selectin")
    # assigned_to, resolved_by relationships removed - no User model exists
