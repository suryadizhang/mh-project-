...
class CateringPayment(BaseModel):
    ...
    # Removed notification_id - using PaymentNotification.payment_id instead (avoids circular FK)
...
class PaymentNotification(BaseModel):
    ...
    # Processing
    parsed_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))
    matched_at = Column(DateTime)
    confirmed_at = Column(DateTime)
    # reviewed_by removed - no User model exists
