...
class ThresholdMonitor:
    ...
    def __init__(
        ...
        self.metric_collector = metric_collector or get_metric_collector(redis_client)
        ...
        self._active_violations: Dict[int, RuleViolation] = {}  # rule_id -> violation
    ...
    async def _perform_check(self, check_critical_only: bool = False):
        ...
        try:
            ...
            # Evaluate rules
            timestamp = time.time()
            violations = self.rule_evaluator.evaluate_metrics(metrics, timestamp)
            
            # Update active violations
            self._update_active_violations(violations)
            
            # Check for violations ready to alert
            ready_violations = self.rule_evaluator.get_violations_ready_for_alert()
            
            # Create alerts
            alerts_created = await self._create_alerts_for_violations(ready_violations)
    ...
    async def _collect_single_metric(self, metric_name: str) -> Optional[float]:
        """Collect a single metric by name."""
        try:
            # Map metric names to collector methods
            if metric_name == "database_available":
                return 1.0 if self.metric_collector._check_database_available() else 0.0
            elif metric_name == "redis_available":
                return 1.0 if self.metric_collector._check_redis_available() else 0.0
            elif metric_name == "disk_percent":
                system_metrics = self.metric_collector._collect_system_metrics()
    ...
    def _on_metric_update(self, metric_name: str, value: float, timestamp: float):
        """
        ...
        try:
            ...
            violations = self.rule_evaluator.evaluate_metric(metric_name, value, timestamp)
            ...
            if violations:
                ...
                if ready_violations:
                    ...
                    asyncio.create_task(self._create_alerts_for_violations(ready_violations))
        
        except Exception as e:
            logger.error(f"Error in metric update callback: {e}", exc_info=True)
    ...
    def _update_active_violations(self, violations: List[RuleViolation]):
        ...
        resolved_rule_ids = set(self._active_violations.keys()) - active_rule_ids
    ...
    async def _create_alerts_for_violations(
        self,
        ready_violations: List[Tuple[AlertRule, RuleViolation]]
    ) -> int:
        """
        ...
        for rule, violation in ready_violations:
            try:
                ...
                # Start cooldown
                self.rule_evaluator.start_cooldown(rule.id, rule.cooldown_seconds)
                ...
                logger.error(f"Error creating alert for rule {rule.id}: {e}", exc_info=True)
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "subscriber_stats": self.subscriber.get_stats() if self.subscriber.is_running() else None,
            "rule_evaluator_violations": len(self.rule_evaluator.get_all_violations())
        }
