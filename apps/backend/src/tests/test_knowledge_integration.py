...
async def test_knowledge_context_injection(test_context, db_session):
    ...
    result = await router.route(
        message="What are your menu options?", context=test_context
    )

    # Verify knowledge_context was injected
    assert (
        "knowledge_context" in test_context
    ), "knowledge_context not injected into context"
    assert (
        len(test_context["knowledge_context"]) > 0
    ), "knowledge_context is empty"
    ...
    assert (
        "MENU" in context_upper
        or "PRICING" in context_upper
        or "PACKAGES" in context_upper
    )
    assert "CHARTER" in context_upper or "BUSINESS" in context_upper

    print(
        f"✅ Knowledge context injected: {len(knowledge_context)} characters"
    )
...
async def test_customer_care_agent_uses_knowledge(test_context):
    ...
    result = await router.route(
        message="What is your cancellation policy?", context=test_context
    )
    ...
    assert agent_type in [
        "customer_care",
        "knowledge",
    ], f"Expected customer_care or knowledge, got {agent_type}"

    # Verify response contains policy information
    content = result["content"].lower()
    assert any(
        word in content
        for word in ["cancel", "policy", "refund", "help", "assist"]
    )
...
async def test_operations_agent_uses_knowledge(test_context):
    ...
    result = await router.route(
        message="Do you serve the Orange County area?", context=test_context
    )
    ...
    content = result["content"].lower()
    assert any(
        word in content for word in ["area", "location", "service", "county"]
    )
...
async def test_knowledge_agent_uses_knowledge(test_context):
    ...
    result = await router.route(
        message="Tell me about My Hibachi", context=test_context
    )
    ...
    assert agent_type in [
        "knowledge",
        "lead_nurturing",
    ], f"Expected knowledge or lead_nurturing, got {agent_type}"

    # Verify response contains business information
    content = result["content"].lower()
    assert any(
        word in content
        for word in ["hibachi", "restaurant", "service", "chef", "catering"]
    )
...
async def test_fallback_routing_injects_knowledge(test_context):
    ...
    if "fallback" in result.get("routing", {}):
        print("✅ Fallback triggered, still got knowledge context")
    else:
        print("✅ No fallback needed, still got knowledge context")
...
async def test_knowledge_service_full_context():
    ...
    async for db in get_db():
        ...
        print("   Structure verified - sections present")
...
async def test_performance_overhead():
    ...
    result = await router.route(
        ...
        context={"conversation_id": "perf_test", "guest_count": 50},
    )
    ...
    assert (
        total_latency < 15000
    ), f"Total latency too high: {total_latency:.2f}ms"

    print(f"✅ Total latency: {total_latency:.2f}ms")
    print(
        f"   Classification: {result['routing']['classification_latency_ms']:.2f}ms"
    )
...
async def test_error_handling_empty_knowledge():
    ...
    print(
        f"✅ Agent handled empty knowledge_context: {result['content'][:100]}..."
    )
