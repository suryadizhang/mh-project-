...
from sqlalchemy import select, and_, or_, desc, func, update, delete, String
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.orm import selectinload
import logging

from models.email import EmailMessage, EmailThread, EmailSyncStatus
...
class EmailRepository:
    ...
    async def create_message(
        ...
        existing = await self.get_message_by_message_id(
    ...
    async def create_thread(
        ...
        existing = await self.get_thread_by_thread_id(
    ...
    async def update_thread_counts(
        self,
        thread_id: str
    ) -> Optional[EmailThread]:
        ...
        # Update thread
        return await self.update_thread(thread_id, {
    ...
    async def get_sync_status(
        ...
        result = await self.session.execute(
            select(EmailSyncStatus).where(
                and_(
                    EmailSyncStatus.inbox == inbox,
                    EmailSyncStatus.is_deleted == False
                )
            )
        )
        return result.scalar_one_or_none()
    ...
    async def bulk_add_label_to_messages(
        self,
        message_ids: List[str],
        label_slug: str
    ) -> int:
    ...
    async def bulk_remove_label_from_messages(
        self,
        message_ids: List[str],
        label_slug: str
    ) -> int:
