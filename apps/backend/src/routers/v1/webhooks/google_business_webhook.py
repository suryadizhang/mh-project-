...
async def process_google_business_message(
    message_data: dict[str, Any], db: Session
) -> dict[str, Any] | None:
    ...
    try:
        ...
        if "richCard" in message:
            ...
            attachments.append({"type": "rich_card", "content": rich_card})
        ...
        existing_lead = (
            ...
            .filter(
                Lead.social_handles.contains(
                    {"google_business": {"customer_id": customer_id, "name": customer_name}}
                )
            )
            .first()
        )
        ...
        if not existing_lead:
            ...
            lead_data = {
                ...
                "name": customer_name
                or f"Google Customer {customer_id[-4:] if customer_id else 'Unknown'}",
                "social_handles": {
                    "google_business": {
        ...
        logger.exception(f"Error processing Google Business message: {e}")
        db.rollback()
        return None
...
async def process_google_business_event(
    ...
    try:
        ...
        logger.exception(f"Error processing Google Business event: {e}")
        db.rollback()
        return None
    ...
    try:
        ...
        if settings.google_business_webhook_secret:
            ...
            if not verify_google_signature(
                payload, signature, settings.google_business_webhook_secret
            ):
        ...
        if webhook_type == "MESSAGE_RECEIVED":
            ...
            if result:
                ...
                background_tasks.add_task(
                    analyze_business_message, result["thread_id"], result["text"]
                )
                ...
                background_tasks.add_task(check_business_hours_response, result["conversation_id"])
        ...
        logger.exception(f"Error processing Google Business webhook: {e}")
...
async def analyze_business_message(thread_id: str, message_text: str):
    ...
    try:
        ...
        logger.exception(f"Error in business message AI analysis: {e}")
...
async def check_business_hours_response(conversation_id: str):
    ...
    try:
        ...
        if current_hour < 9 or current_hour >= 18:
            ...
            logger.info(
                f"Auto-response triggered for conversation {conversation_id} (outside business hours)"
            )

    except Exception as e:
        logger.exception(f"Error checking business hours response: {e}")
...
async def broadcast_thread_update(thread_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error broadcasting thread update: {e}")
...
async def send_business_message(conversation_id: str, message: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        logger.exception(f"Error sending Google Business message: {e}")
