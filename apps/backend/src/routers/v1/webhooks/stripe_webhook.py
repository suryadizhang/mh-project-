...
async def process_payment_event(stripe_event: dict[str, Any], db: Session) -> dict[str, Any] | None:
    ...
    try:
        ...
        customer_email = event_data.get("receipt_email") or event_data.get(
            "billing_details", {}
        ).get("email")
        ...
        if customer_id:
            try:
                ...
                logger.exception(f"Error retrieving Stripe customer: {e}")

        customer_name = ""
        if customer_info:
            customer_name = (
                customer_info.get("name") or customer_info.get("email", "").split("@")[0]
            )
        elif customer_email:
        ...
        if customer_id:
            existing_lead = (
                db.query(Lead)
                .filter(Lead.social_handles.contains({"stripe": {"customer_id": customer_id}}))
                .first()
            )

        # Try to find by email if not found by customer ID
        if not existing_lead and customer_email:
            existing_lead = db.query(Lead).filter(Lead.email == customer_email).first()
        ...
        if not existing_lead:
            ...
            lead_data = {
                ...
                "name": customer_name
                or f"Stripe Customer {customer_id[-4:] if customer_id else 'Unknown'}",
                "email": customer_email,
                "social_handles": {"stripe": {"customer_id": customer_id, "email": customer_email}},
        ...
        logger.exception(f"Error processing Stripe payment event: {e}")
        db.rollback()
        return None
...
async def process_dispute_event(stripe_event: dict[str, Any], db: Session) -> dict[str, Any] | None:
    ...
    try:
        ...
        try:
            ...
            logger.exception(f"Error retrieving charge for dispute: {e}")
            return None
        ...
        logger.exception(f"Error processing Stripe dispute: {e}")
        db.rollback()
        return None
...
async def stripe_webhook(
    ...
    try:
        ...
        if event_type.startswith(("payment_intent.", "invoice.")):
            # Payment events
            ...
            if result:
                background_tasks.add_task(
                    handle_subscription_change, result["thread_id"], event_type
                )
...
async def analyze_payment_event(thread_id: str, event_type: str, event_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error in payment event AI analysis: {e}")
...
async def handle_dispute_alert(thread_id: str, dispute_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error handling dispute alert: {e}")
...
async def handle_subscription_change(thread_id: str, event_type: str):
    ...
    try:
        ...
        logger.exception(f"Error handling subscription change: {e}")
...
async def broadcast_thread_update(thread_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error broadcasting Stripe thread update: {e}")
...
async def get_customer_payment_history(customer_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.exception(f"Error getting customer payment history: {e}")
