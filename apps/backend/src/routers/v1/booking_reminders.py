...
from db.models.booking_reminder import ReminderStatus
...
async def create_booking_reminder(
    booking_id: int, reminder_data: BookingReminderCreate, db: AsyncSession = Depends(get_db)
) -> BookingReminderResponse:
    """
...
async def list_booking_reminders(
    ...
    **Returns**: Paginated list of reminders
    ...
    reminders, total = await service.list_reminders(
        booking_id=booking_id, status=status_filter, page=page, page_size=page_size
    )
...
async def get_booking_reminder(
    booking_id: int, reminder_id: UUID, db: AsyncSession = Depends(get_db)
) -> BookingReminderResponse:
    """
