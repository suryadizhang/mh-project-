...
from utils.auth.roles import normalize_role
...
class SyncTriggerRequest(BaseModel):
    ...
    sources: Optional[list[str]] = Field(
        default=None, description="Specific sources to sync (menu, faqs, terms). If None, sync all."
    )
...
async def get_sync_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"message": "Failed to get sync status", "error": str(e)}
        )
...
async def get_sync_diff(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"message": "Failed to get sync diff", "error": str(e)}
        )
...
async def trigger_sync(
    ...
    """
    try:
        # Force sync requires superadmin - use normalize_role for case-insensitive comparison
        user_role = normalize_role(current_user.get("role"))
        if request.force and user_role != UserRole.SUPER_ADMIN:
            raise HTTPException(status_code=403, detail="Force sync requires superadmin role")

        sync_service = KnowledgeSyncService(session)

        # If specific sources requested
        if request.sources:
            result = {"overall_success": True, "timestamp": datetime.utcnow().isoformat()}
        ...
        raise HTTPException(
            status_code=500, detail={"message": "Failed to trigger sync", "error": str(e)}
        )
...
async def get_sync_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"message": "Failed to get sync history", "error": str(e)}
        )
    ...
    try:
        ...
        return SyncResultResponse(
            ...
            message=(
                ...
                if result.get("overall_success")
                else "Auto-sync completed with skipped conflicts"
            ),
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail={"message": "Failed to auto-sync", "error": str(e)}
        )
    ...
    try:
        ...
        return SyncResultResponse(
            ...
            message=(
                ...
                if result.get("overall_success")
                else "Force sync completed with errors"
            ),
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail={"message": "Failed to force sync", "error": str(e)}
        )
