...
async def get_metric_trends(
    ...
    try:
        ...
        entries = redis_client.zrangebyscore(history_key, cutoff_score, "+inf", withscores=True)
...
def _calculate_system_health(
    metrics: Dict[str, float], alert_service: AlertService
) -> SystemHealth:
    """
    ...
    return SystemHealth(
        status=status, score=round(score, 1), issues=issues, last_updated=datetime.utcnow()
    )
