...
async def register_webhooks(
    base_url: str, events: list[str] | None = None, sms_service: RingCentralSMSService | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        async with sms_service:
            ...
            sms_webhook_result = await sms_service.setup_webhook(
                webhook_urls["sms"], ["message-store"]
            )
            subscriptions["sms"] = {
        ...
        logger.info("Successfully registered SMS webhook")
        return {
            "status": "success",
        ...
        logger.exception(f"Failed to register webhooks: {e}")
        return {"status": "error", "error": str(e), "attempted_urls": webhook_urls}


async def list_webhooks(sms_service: RingCentralSMSService | None = None) -> list[dict[str, Any]]:
    """
...
async def cleanup_webhooks(sms_service: RingCentralSMSService | None = None) -> dict[str, Any]:
    """
...
def validate_configuration() -> dict[str, Any]:
    ...
    """
    validation_results = {"valid": True, "missing": [], "invalid": [], "warnings": []}
    ...
    jwt_token = getattr(settings, "ringcentral_jwt_token", None) or os.getenv(
    ...
    if not jwt_token and not (username and password):
        validation_results["missing"].append(
            "RINGCENTRAL_JWT_TOKEN or RINGCENTRAL_USERNAME/PASSWORD"
        )
        validation_results["valid"] = False
    ...
    phone_number = getattr(settings, "ringcentral_phone_number", None) or os.getenv(
        "RINGCENTRAL_PHONE_NUMBER"
    )
    if phone_number and not phone_number.startswith("+"):
        validation_results["invalid"].append(
            "RINGCENTRAL_PHONE_NUMBER should start with + (international format)"
        )
        validation_results["valid"] = False
...
async def test_connection(sms_service: RingCentralSMSService | None = None) -> dict[str, Any]:
    """
    ...
    try:
        async with sms_service:
            ...
            return {
                ...
                "capabilities": {
                    "sms_ready": bool(
                        getattr(settings, "ringcentral_phone_number", None)
                        or getattr(settings, "ringcentral_from_number", None)
                    ),
                    "phone_number": getattr(settings, "ringcentral_phone_number", None)
                    or getattr(settings, "ringcentral_from_number", None),
                },
...
async def send_test_sms(
    to_number: str, message: str | None = None, sms_service: RingCentralSMSService | None = None
) -> dict[str, Any]:
    """
    ...
    checklist = {
        ...
        "steps": [
            ...
            },
            {
                ...
                "completed": bool(
                    getattr(settings, "ringcentral_jwt_token", None)
                    or (
                        getattr(settings, "ringcentral_username", None)
                        and getattr(settings, "ringcentral_password", None)
                    )
                ),
                "required": True,
            },
