...
def superadmin_required(user=Depends(get_current_user_oauth)):
    ...
    if user.get("role") != "superadmin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Superadmin privileges required"
        )
    return user
...
def require_role(allowed_roles: list[UserRole]) -> Callable:
    ...
    async def role_checker(current_user: dict = Depends(get_current_user)) -> dict:
        ...
        if user_role is None:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"Invalid user role: {user_role_str}"
            )
...
def require_any_admin() -> Callable:
    ...
    return require_role(
        [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.CUSTOMER_SUPPORT, UserRole.STATION_MANAGER]
    )
