...
class AILeadManager:
    ...
    async def analyze_social_engagement(self, post: SocialMediaPost) -> dict[str, Any]:
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}], model="gpt-4", temperature=0.3
            )
    ...
    async def score_lead_with_ai(
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}], model="gpt-4", temperature=0.2
            )
...
class SocialMediaAI:
    ...
    async def create_promotional_content(self, campaign_type: str, target_audience: str) -> str:
        ...
        try:
            response = await self.openai.chat_completion(
                messages=[{"role": "user", "content": prompt}], model="gpt-4", temperature=0.8
            )
