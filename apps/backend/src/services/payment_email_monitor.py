...
class PaymentEmailParser:
    ...
    def parse_stripe_email(subject: str, body: str) -> Optional[Dict]:
        ...
        try:
            # Extract amount
            amount_match = re.search(PaymentEmailParser.PATTERNS["stripe"]["amount"], body, re.IGNORECASE)
            if not amount_match:
                return None
            
            # Extract transaction ID
            tx_match = re.search(PaymentEmailParser.PATTERNS["stripe"]["transaction_id"], body, re.IGNORECASE)
            ...
            logger.error(f"Error parsing Stripe email: {e}")
            return None
    ...
    def parse_venmo_email(subject: str, body: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not amount_match:
                ...
                amount_match = re.search(PaymentEmailParser.PATTERNS["venmo"]["amount"], body, re.IGNORECASE)
                if not amount_match:
            ...
            username_match = re.search(PaymentEmailParser.PATTERNS["venmo"]["username"], body_text, re.IGNORECASE)
            ...
            if sender_from_subject:
                ...
                sender_body = re.search(r"(?:from|by)\s+([\w\s]+?)\s+(?:sent|@|paid)", body_text, re.IGNORECASE)
                if sender_body:
            ...
            phone_match = re.search(r'(?:note|message|memo)[\s:]+(\d{10})', body_text, re.IGNORECASE)
            if not phone_match:
            ...
            result = {
                ...
                "customer_phone": phone_match.group(1) if phone_match else None,  # Customer's phone from note
                "status": "pending",  # Venmo requires manual confirmation
                "parsed_at": datetime.utcnow().isoformat(),
            }
            
            logger.info(f"Parsed Venmo email: amount=${result['amount']}, sender={result['sender_name']}, username={result.get('sender_username', 'N/A')}, phone={result.get('customer_phone', 'N/A')}")
            return result
    ...
    @staticmethod
    def parse_zelle_email(subject: str, body: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            body_amount = re.search(PaymentEmailParser.PATTERNS["zelle"]["amount"], body_text, re.IGNORECASE)
            ...
            if sender_from_subject:
                ...
                # Try to extract from body
                sender_body = re.search(PaymentEmailParser.PATTERNS["zelle"]["sender"], body_text, re.IGNORECASE)
                if sender_body:
                    sender_name = sender_body.group(1).strip()
            ...
            phone_match = re.search(r'(?:memo|note|message)[\s:]+(\d{10})', body_text, re.IGNORECASE)
            if not phone_match:
                # Fallback: any 10-digit number in the body
                phone_match = re.search(r'\b(\d{10})\b', body_text)
            ...
            result = {
                ...
                "customer_phone": phone_match.group(1) if phone_match else None,  # Customer's phone from memo
                "status": "pending",  # Zelle requires manual confirmation
                "parsed_at": datetime.utcnow().isoformat(),
            }
            
            logger.info(f"Parsed Zelle email: amount=${result['amount']}, sender={result['sender_name']}, phone={result.get('customer_phone', 'N/A')}")
            return result
            
        except Exception as e:
            logger.error(f"Error parsing Zelle email: {e}")
            import traceback
            traceback.print_exc()
            return None
    
    @staticmethod
    def parse_bofa_email(subject: str, body: str) -> Optional[Dict]:
        """Parse Bank of America payment notification email"""
        try:
            # Extract amount
            amount_match = re.search(PaymentEmailParser.PATTERNS["bank_of_america"]["amount"], body, re.IGNORECASE)
            if not amount_match:
                return None
            
            # Extract payment type
            type_match = re.search(PaymentEmailParser.PATTERNS["bank_of_america"]["type"], body, re.IGNORECASE)
            ...
            logger.error(f"Error parsing Bank of America email: {e}")
            return None
    
    @staticmethod
    def detect_and_parse(subject: str, from_email: str, body: str) -> Optional[Dict]:
        """
        ...
        # Stripe
        if "stripe" in from_lower or re.search(PaymentEmailParser.PATTERNS["stripe"]["subject"], subject_lower):
        ...
        if "bankofamerica" in from_lower or "bofa" in from_lower or "ealerts.bankofamerica" in from_lower:
            ...
            for email_id in email_ids:
                try:
                    ...
                    if parsed:
                        ...
                        logger.info(f"✅ Parsed payment email: {parsed['provider']} - ${parsed['amount']}")
...
if __name__ == "__main__":
    ...
    print(f"\n📧 Found {len(payment_notifications)} payment notification(s):\n")
