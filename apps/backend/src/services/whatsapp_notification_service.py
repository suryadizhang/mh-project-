...
class WhatsAppNotificationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.sms_from = os.getenv('TWILIO_SMS_NUMBER') or os.getenv('RC_SMS_FROM', '+19167408768')
        ...
        try:
            ...
            if self.client:
                ...
                if whatsapp_result["success"]:
                    ...
                    logger.info(f"WhatsApp sent to {formatted_phone}: {whatsapp_result['message_sid']}")
                else:
                    # Fallback to SMS
                    logger.warning(f"WhatsApp failed, falling back to SMS: {whatsapp_result['error']}")
    ...
    async def _send_ringcentral_sms(self, to: str, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"RingCentral SMS fallback failed: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def _send_to_admin_group(self, message: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Admin group notification failed: {e}")
            return False
    ...
    async def send_manual_review_alert(
        ...
        try:
            ...
            logger.error(f"Manual review alert failed: {e}")
            return {"success": False, "error": str(e)}
