...
class RingCentralVoiceAI:
    ...
    def __init__(self):
        ...
        self.greeting_message = (
            "Hello! Thank you for calling My Hibachi Chef. How can I help you today?"
        )
    ...
    async def generate_ai_reply(
        ...
        try:
            ...
            nlp_result = await asyncio.to_thread(self.nlp_service.extract_booking_details, message)

            intent = nlp_result.get("intent", "unknown")
            confidence = nlp_result.get("confidence", 0.0)

            # 2. Detect tone for response matching
            tone_result = await asyncio.to_thread(self.nlp_service.detect_tone_enhanced, message)
            ...
            if intent in ["booking", "reservation", "inquiry"]:
                booking_extraction = await self.customer_booking_ai._extract_booking_details(
                    message
                )
                booking_data = {
            ...
            response = await self.ai_pipeline.generate_response(
                user_message=message, context=ai_context
            )

            # 5. Determine escalation
            should_escalate = (
                confidence < self.escalation_threshold
                or intent in ["complaint", "escalation", "manager"]
                or "speak to manager" in message.lower()
                or "human" in message.lower()
            )
    ...
    async def transcribe_and_respond(
        ...
        try:
            ...
            response_audio = await self.speech_service.synthesize_speech(
                ai_reply.reply, voice_model=self.default_voice
            )
    ...
    async def finalize_call(
        ...
        try:
            ...
            summary = {
                ...
                "duration": (
                    datetime.now(timezone.utc) - conversation["started_at"]
                ).total_seconds(),
    ...
    async def _synthesize_greeting(self) -> bytes:
        ...
        try:
            audio = await self.speech_service.synthesize_speech(
                self.greeting_message, voice_model=self.default_voice
            )
            return audio
        except Exception as e:
