...
class DeepgramSTTBridge:
    ...
    def start(self, event_loop: asyncio.AbstractEventLoop):
        ...
        self.thread = threading.Thread(
            target=self._run_deepgram_thread, name=f"Deepgram-STT-{id(self)}", daemon=True
        )
        self.thread.start()

        # Start transcript processor
        asyncio.run_coroutine_threadsafe(self._process_transcripts(), self.loop)
        ...
        try:
            ...
            with self.deepgram_client.listen.v1.connect(
                ...
                recv_thread = threading.Thread(
                    target=self._receive_transcripts, args=(connection,), daemon=True
                )
                recv_thread.start()
                        ...
                        if alternatives and len(alternatives) > 0:
                            ...
                            if transcript_text.strip():  # Only process non-empty
                                result = TranscriptResult(
                                    ...
                                    confidence=(
                                        alt.confidence if hasattr(alt, "confidence") else 0.0
                                    ),
                                    timestamp=datetime.now(timezone.utc),
                                    speech_final=(
                                        message.speech_final
                                        if hasattr(message, "speech_final")
                                        else False
                                    ),
                                )
    ...
    async def _process_transcripts(self):
        ...
        while self.is_running:
            try:
                ...
                result = await asyncio.wait_for(self.transcript_queue.get(), timeout=0.1)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        uptime = (
            (datetime.now(timezone.utc) - self.started_at).total_seconds() if self.started_at else 0
        )
