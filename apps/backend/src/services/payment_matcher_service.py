...
class PaymentMatcher:
    ...
    def find_matching_payment(
        ...
        try:
            ...
            tolerance = PaymentMatcher.EXPANDED_AMOUNT_TOLERANCE if use_fuzzy_matching else PaymentMatcher.AMOUNT_TOLERANCE
            ...
            time_window = PaymentMatcher.EXTENDED_TIME_WINDOW_HOURS if use_fuzzy_matching else PaymentMatcher.TIME_WINDOW_HOURS
            ...
            logger.error(f"Error finding matching payment: {e}")
            return None
    ...
    def _find_by_phone_number(
        ...
        try:
            ...
            query = db.query(Payment).join(Booking).filter(
                and_(
                    ...
                    or_(
                        ...
                        Booking.customer_phone.like(f"%{normalized_phone[-4:]}%")  # Match last 4 digits
                    )
                )
            ).order_by(Payment.created_at.desc())
            ...
            logger.error(f"Error finding payment by phone: {e}")
            return None
    
    @staticmethod
    def _pick_closest_booking(
        potential_matches: List[Payment],
        ...
        for payment in potential_matches:
            ...
            if sender_name and booking.customer_name:
                ...
                # First name match (compare first word)
                    ...
                    score += 75
                    logger.debug(f"✅ First name match: '{sender_parts[0]}' = '{customer_parts[0]}'")
                
                # Last name match (compare last word)
                elif sender_parts and customer_parts and sender_parts[-1] == customer_parts[-1]:
                    score += 75
                    logger.debug(f"✅ Last name match: '{sender_parts[-1]}' = '{customer_parts[-1]}'")
                ...
                # Partial match (any word in sender name matches any word in customer name)
                    ...
                    score += 50
                    logger.debug(f"✅ Partial name match: sender={sender_parts}, customer={customer_parts}")
            ...
            if sender_phone and booking.customer_phone:
                ...
                normalized_customer = ''.join(c for c in booking.customer_phone if c.isdigit())[-10:]
                ...
                if alt_payer:
                    ...
                    if sender_name and alt_payer.get("name"):
                        ...
                        if sender_name == alt_name:
                            ...
                            logger.debug(f"✅ Alternative payer exact match: '{sender_name}' = '{alt_name}'")
                        
                        # First name match
                        elif sender_parts and alt_parts and sender_parts[0] == alt_parts[0]:
                            score += 125
                            logger.debug(f"✅ Alternative payer first name match: '{sender_parts[0]}'")
                        
                        # Last name match
                        elif sender_parts and alt_parts and sender_parts[-1] == alt_parts[-1]:
                            score += 125
                            logger.debug(f"✅ Alternative payer last name match: '{sender_parts[-1]}'")
                        
                        # Partial match
                        elif any(sp in alt_parts for sp in sender_parts if len(sp) > 2):
                            score += 100
                            logger.debug(f"✅ Alternative payer partial name match")
                    ...
                    if sender_phone and alt_payer.get("phone"):
                        normalized_alt = ''.join(c for c in alt_payer.get("phone", "") if c.isdigit())[-10:]
                ...
                try:
                    ...
                    asyncio.create_task(
                        notify_payment(
                            ...
                            balance_due=float(booking.balance_due_cents / 100.0) if hasattr(booking, 'balance_due_cents') and booking.balance_due_cents else 0.0
                        )
                    )
                    logger.info(f"📱 WhatsApp payment notification queued for {booking.customer_phone}")
                except Exception as e:
...
class PaymentEmailService:
    ...
    def process_payment_emails(
        ...
        try:
            ...
            logger.error(f"Error in payment email processing: {e}")
            results["errors"].append({"error": str(e)})
            return results
    
    def get_unmatched_notifications(
        self,
        since_date: Optional[datetime] = None
    ) -> List[Dict]:
        """
