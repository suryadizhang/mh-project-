...
class RingCentralSMSService:
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"RingCentral authentication failed: {response.status_code} - {response.text}"
                )
                return False

        except Exception as e:
            logger.exception(f"RingCentral authentication error: {e!s}")
            return False
    ...
    async def _authenticate_with_jwt(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"RingCentral JWT authentication failed: {response.status_code} - {response.text}"
                )
                return False

        except Exception as e:
            logger.exception(f"RingCentral JWT authentication error: {e!s}")
            return False

    async def _ensure_authenticated(self) -> bool:
        """Ensure we have a valid access token."""
        if not self.access_token or (
            self.token_expires_at and datetime.now(timezone.utc) >= self.token_expires_at
        ):
    ...
    async def send_sms(
        ...
        try:
            sms_data = {"to": [{"phoneNumber": to_number}], "text": message}
            ...
            error_msg = f"SMS send error: {e!s}"
    ...
    async def handle_webhook_notification(self, payload: dict[str, Any]) -> list[SMSMessage]:
        ...
        try:
            ...
            logger.exception(f"Webhook processing error: {e!s}")
    ...
    async def get_account_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self._client.get(
                f"{self.server_url}/restapi/v1.0/account/~", headers=headers
            )
