...
# Models
from models.knowledge_base import MenuItem, FAQItem, BusinessRule, SyncHistory
...
class KnowledgeSyncService:
    ...
    def _parse_menu_from_tsx(self) -> List[Dict]:
        ...
        base_section_match = re.search(r'Base Proteins.*?(?=Premium Upgrades|Additional Enhancements)', content, re.DOTALL)
        ...
        premium_section_match = re.search(r'Premium Upgrades.*?(?=Additional Enhancements|Included Items)', content, re.DOTALL)
        ...
        enhancements_section_match = re.search(r'Additional Enhancements.*?(?=$|<footer)', content, re.DOTALL)
    ...
    def _categorize_protein(self, name: str) -> str:
        ...
        if 'chicken' in name_lower:
            ...
            return 'beef'
        elif any(x in name_lower for x in ['shrimp', 'lobster', 'scallop', 'salmon', 'calamari', 'fish']):
            return 'seafood'
    ...
    def _parse_faqs_from_ts(self) -> List[Dict]:
        ...
        faqs_array_match = re.search(r'export const faqs: FaqItem\[\] = \[(.*?)\](?=\s*export|\s*$)', content, re.DOTALL)
        ...
        for obj_str in faq_objects:
            ...
            answer_match = re.search(r"answer:\s*['\"](.+?)['\"](?=,|\s*category:)", obj_str, re.DOTALL)
            if answer_match:
    ...
    def _parse_terms_from_tsx(self) -> List[Dict]:
        ...
        cancellation_match = re.search(r'<h2>4\. Cancellation Policy</h2>(.*?)(?=<h2>|</section>)', content, re.DOTALL)
        if cancellation_match:
            ...
            if '7+ Days Before Event' in section:
                rules.append({
                    ...
                    'value': {'days_before': 7, 'refund_percentage': 100, 'includes_deposit': True},
                    'is_active': True,
                    'version': 1
                })
            ...
            if 'Less than 7 Days' in section:
                rules.append({
                    ...
                    'value': {'days_before': 7, 'deposit_refundable': False, 'balance_refundable': False},
                    'is_active': True,
                    'version': 1
                })
        
        # Parse rescheduling policy
        rescheduling_match = re.search(r'<h2>5\. Rescheduling Policy</h2>(.*?)(?=<h2>|</section>)', content, re.DOTALL)
        if rescheduling_match:
            ...
            if 'One free reschedule' in section and '48 hours' in section:
                rules.append({
                    ...
                    'value': {'first_reschedule_free': True, 'time_window_hours': 48, 'subsequent_fee': 100},
                    'is_active': True,
                    'version': 1
                })
        ...
        # Parse payment terms
        payment_match = re.search(r'<h2>3\. Booking and Payment Terms</h2>(.*?)(?=<h2>|</section>)', content, re.DOTALL)
        if payment_match:
            ...
            if '$100 refundable deposit' in section or '$100 deposit' in section:
                rules.append({
                    ...
                    'value': {'deposit_amount': 100, 'refundable': True, 'conditions': 'if canceled 7+ days before event'},
                    'is_active': True,
                    'version': 1
                })
            ...
            if minimum_match:
                ...
                rules.append({
                    ...
                    'value': {'minimum_amount': minimum_amount, 'approximate_guests': 10, 'currency': 'USD'},
                    'is_active': True,
                    'version': 1
                })
        
        # Parse travel fees (may be in multiple sections)
        travel_match = re.search(r'first\s+(\d+)\s+miles.*?\$(\d+)\s*(?:per\s*)?mile', content, re.IGNORECASE | re.DOTALL)
        if travel_match:
            ...
            rules.append({
                ...
                'value': {'free_miles': free_miles, 'per_mile_rate': per_mile_rate, 'flexible': True},
                'is_active': True,
                'version': 1
            })
    ...
    def _items_differ(self, file_data: Dict, db_data: Dict) -> bool:
        ...
        compare_keys = ['name', 'description', 'price', 'category', 'question', 'answer', 'title', 'content']
