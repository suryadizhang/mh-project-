...
class PaymentEmailScheduler:
    ...
    def __init__(self, check_interval_minutes: int = 5, use_imap_idle: bool = True):
        ...
        self.run_count = 0
        self.error_count = 0
        self.idle_connection: Optional[imaplib.IMAP4_SSL] = None
    ...
    def cleanup_old_notifications_job(self):
        ...
        try:
            ...
            try:
                ...
                from models.payment_notification import PaymentNotification, PaymentNotificationStatus
                ...
                old_read_notifications = db.query(PaymentNotification).filter(
                    and_(
                        PaymentNotification.created_at < cutoff_date_read,
                        PaymentNotification.is_processed == True,
                        PaymentNotification.is_read == True,
                ...
                old_unread_notifications = db.query(PaymentNotification).filter(
                    and_(
            ...
            try:
                if IMAP_CLIENT_AVAILABLE:
                    ...
                    with IMAPClient(host="imap.gmail.com", ssl=True, port=993) as client:
                        # Login
                        client.login(settings.GMAIL_USER, settings.GMAIL_APP_PASSWORD_IMAP or settings.GMAIL_APP_PASSWORD)
                        
                        # Select inbox
                        client.select_folder('INBOX')
                        
                        logger.info("✅ IMAP IDLE connected (imapclient) - waiting for new emails (real-time push mode)")
                            ...
                            if responses:
                                ...
                                for response in responses:
                                    if b'EXISTS' in response or b'RECENT' in response:
                                        logger.info("📬 New email detected via IMAP IDLE - processing immediately...")
                                ...
                                logger.debug("IMAP IDLE timeout (28 min) - reconnecting for keepalive...")
                                client.idle_done()
                                break  # Break to reconnect
                        
                else:
                    # Fallback to raw imaplib (less reliable)
                    logger.warning("Using fallback imaplib for IMAP IDLE - consider installing imapclient")
                    import imaplib
                    import ssl
                    ...
                    # Login
                    self.idle_connection.login(settings.GMAIL_USER, settings.GMAIL_APP_PASSWORD_IMAP or settings.GMAIL_APP_PASSWORD)
                    ...
                    logger.info("✅ IMAP IDLE connected (fallback) - waiting for new emails (push mode)")
                    ...
                    while self.is_running:
                        try:
                            ...
                            try:
                                ...
                                if b'EXISTS' in line:
                                    logger.info("📬 New email detected via IMAP IDLE (fallback) - processing...")
                            ...
                            logger.error(f"IMAP IDLE error in loop: {e}")
                            break
                    
            except Exception as e:
                logger.error(f"❌ IMAP IDLE connection error: {e}")
    ...
    def run_scheduler(self):
        ...
        if self.use_imap_idle:
            logger.info(f"  - Real-time email monitoring: IMAP IDLE (push notifications)")
    ...
    def start(self):
        ...
        if self.use_imap_idle:
            ...
            logger.info("✅ Payment email service started: Real-time push (IMAP IDLE) + fallback polling")
        else:
            logger.info(f"✅ Payment email service started: Polling every {self.check_interval_minutes} minutes")
...
_scheduler: Optional[PaymentEmailScheduler] = None
...
def get_scheduler() -> PaymentEmailScheduler:
    ...
    if _scheduler is None:
        ...
        check_interval = getattr(settings, "PAYMENT_EMAIL_CHECK_INTERVAL_MINUTES", 30)  # 30-min fallback
        use_imap_idle = getattr(settings, "PAYMENT_EMAIL_USE_IMAP_IDLE", True)  # Real-time push enabled
        _scheduler = PaymentEmailScheduler(
            check_interval_minutes=check_interval,
            use_imap_idle=use_imap_idle
        )
    ...
    try:
        ...
        if not settings.GMAIL_USER or not settings.GMAIL_APP_PASSWORD:
            logger.warning(
                ...
                "payment email monitoring disabled"
            )
            return
        ...
        logger.error(f"Failed to start payment email scheduler: {e}")
...
async def on_shutdown():
    ...
    try:
        ...
        logger.error(f"Error stopping payment email scheduler: {e}")
