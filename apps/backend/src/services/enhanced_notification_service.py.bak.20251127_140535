...
class GroupAwareNotificationService:
    ...
    async def send_to_groups(
        ...
        recipients = await self.group_service.get_recipients_for_event(
            event_type=event_type, station_id=station_id, priority=priority
        )
        ...
        logger.info(
            f"📧 Sending {event_type.value} notification to {len(recipients)} group members"
        )
    ...
    async def _send_to_recipient(self, recipient: dict[str, Any], message: str) -> dict[str, Any]:
        ...
        if recipient["receive_whatsapp"]:
            ...
            try:
                ...
                logger.exception(f"❌ SMS failed for {name}: {e}")
    ...
    async def _send_to_customer(
        ...
        if self.base_service._is_quiet_hours():
            logger.info("🌙 Quiet hours - skipping customer notification")
            return {"success": False, "reason": "quiet_hours"}

        # Reformat message for customer (friendly format)
        customer_message = self._format_customer_message(
            event_type=event_type, customer_name=customer_name, admin_message=message
        )

        try:
            await self.base_service._send_whatsapp(
                to_number=customer_phone, message=customer_message
            )
            logger.info(f"✅ Customer notification sent to {customer_name}")
            return {"success": True, "channel": "whatsapp"}
        except Exception as e:
            logger.exception(f"❌ Customer notification failed: {e}")
            return {"success": False, "error": str(e)}

    def _format_customer_message(
        self, event_type: NotificationEventType, customer_name: str, admin_message: str
    ) -> str:
