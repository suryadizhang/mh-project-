...
class BookingService:
    ...
    async def get_dashboard_stats(
        ...
        # Average party size
        avg_party_size = (
            sum(b.party_size for b in bookings) / total_bookings if total_bookings > 0 else 0
        )
    ...
    async def get_booking_by_id(self, booking_id: UUID) -> Booking:
        ...
        if not booking:
            raise NotFoundException(resource="Booking", identifier=str(booking_id))
    ...
    @invalidate_cache("booking:*")
    async def cancel_booking(self, booking_id: UUID, reason: str | None = None) -> Booking:
        """
        Cancel a booking
    ...
    async def release_hold(
        ...
        logger.info(f"🔓 Hold released on booking {booking_id} by {staff_member}")
        return updated_booking
