        ...
        for idx, media_file in enumerate(media_files):
            try:
                ...
                if is_video:
                    ...
                    thumbnail_url = (
                        result["eager"][0]["secure_url"] if result.get("eager") else video_url
                    )
                    ...
                    thumbnail_url = (
                        result["eager"][0]["secure_url"] if result.get("eager") else image_url
                    )
    ...
    def _validate_media(self, media_file: UploadFile, is_video: bool):
        ...
        if is_video:
            ...
            if not media_file.content_type or not media_file.content_type.startswith("image/"):
                raise HTTPException(
                    status_code=400, detail=f"File {media_file.filename} is not an image"
                )
...
class LocalImageService:
        ...
        for idx, image_file in enumerate(images):
            try:
                ...
                if len(contents) > self.max_file_size:
                    raise HTTPException(
                        status_code=400, detail=f"Image {image_file.filename} exceeds 10MB"
                    )
    ...
    def _validate_image(self, image_file: UploadFile):
        ...
        if not image_file.content_type or not image_file.content_type.startswith("image/"):
            raise HTTPException(
                status_code=400, detail=f"File {image_file.filename} is not an image"
            )
