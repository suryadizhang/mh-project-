...
async def update_station_notification_group(
    ...
    new_station_location: Optional[str] = None
...
async def deactivate_station_notification_group(
    ...
    logger.info(f"   Members will no longer receive notifications")
...
async def reactivate_station_notification_group(
    ...
    logger.info(f"   Group: {group.name}")
    logger.info(f"   Members will resume receiving notifications")
    
    return group


async def delete_station_notification_group(
    db: AsyncSession,
    station_id: UUID
) -> bool:
    """
    ...
    logger.info(f"   All members and event subscriptions removed")
