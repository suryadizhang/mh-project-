...
router = APIRouter(prefix="/api/v1/payments/email-notifications", tags=["Payment Email Notifications"])
...
class ProcessEmailsRequest(BaseModel):
    ...
    since_hours: int = Field(default=24, ge=1, le=168, description="Process emails from last N hours")
...
async def process_payment_emails(
    ...
    **Returns:**
    ...
    try:
        ...
        logger.error(f"Error processing payment emails: {e}")
...
async def get_unmatched_notifications(
    ...
    **Returns:**
    ...
    try:
        ...
        logger.error(f"Error fetching unmatched notifications: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if request.confirm_payment:
            ...
            return ManualMatchResponse(
                ...
                message=f"Payment confirmed and matched to email notification",
            )
        else:
            return ManualMatchResponse(
                success=True,
