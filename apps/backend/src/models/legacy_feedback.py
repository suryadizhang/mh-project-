...
class CustomerReview(Base):
    ...
    coupon_id = Column(
        PostgresUUID(as_uuid=True), ForeignKey("feedback.discount_coupons.id"), nullable=True
    )
    ...
    escalations = relationship(
        "ReviewEscalation", back_populates="review", cascade="all, delete-orphan"
    )
...
class DiscountCoupon(Base):
    ...
    review_id = Column(
        PostgresUUID(as_uuid=True), ForeignKey("feedback.customer_reviews.id"), nullable=True
    )
    ...
    used_in_booking_id = Column(
        PostgresUUID(as_uuid=True), ForeignKey("core.bookings.id"), nullable=True
    )

    # Issue reason
    issue_reason = Column(
        String(100), nullable=False
    )  # 'negative_review', 'complaint_resolution', 'promotion'
    issued_by = Column(
        PostgresUUID(as_uuid=True), nullable=True
    )  # admin user or NULL for auto-issued
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "discount_type IN ('percentage', 'fixed_amount')", name="check_discount_type_valid"
        ),
        CheckConstraint(
    ...
    def mark_used(self, booking_id: str | None = None) -> None:
        """Mark coupon as used."""
        self.times_used += 1
...
class ReviewEscalation(Base):
    ...
    id = Column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid4)
    review_id = Column(
        PostgresUUID(as_uuid=True), ForeignKey("feedback.customer_reviews.id"), nullable=False
    )

    # Escalation details
    escalation_type = Column(
        String(50), nullable=False
    )  # 'admin_notification', 'ai_agent', 'manual'
    priority = Column(
...
__all__ = ["CustomerReview", "DiscountCoupon", "ReviewEscalation"]
