...
from sqlalchemy import (
    ...
    ForeignKey, Boolean, Numeric, JSON, Index
)
...
class CateringBooking(BaseModel):
    ...
    # Relationships
    payments = relationship("CateringPayment", back_populates="booking", cascade="all, delete-orphan")
    notifications = relationship("PaymentNotification", back_populates="booking", cascade="all, delete-orphan")
...
class CateringPayment(BaseModel):
    ...
    # Foreign Keys
    booking_id = Column(Integer, ForeignKey("catering_bookings.id", ondelete="CASCADE"), nullable=False, index=True)
    notification_id = Column(Integer, ForeignKey("payment_notifications.id", ondelete="SET NULL"), nullable=True, index=True)
    
    # Payment Details
    amount = Column(Numeric(10, 2), nullable=False)
    payment_method = Column(SQLEnum(PaymentProvider), nullable=False, index=True)
    status = Column(String(50), default="pending", index=True)  # pending, confirmed, refunded, failed
...
class PaymentNotification(BaseModel):
    ...
    status = Column(SQLEnum(PaymentNotificationStatus), default=PaymentNotificationStatus.DETECTED, nullable=False, index=True)
    match_score = Column(Integer, default=0)  # Confidence score (0-225)
    match_details = Column(JSON)  # Details about why/how it matched
    
    # Foreign Keys
    booking_id = Column(Integer, ForeignKey("catering_bookings.id", ondelete="SET NULL"), nullable=True, index=True)
    payment_id = Column(Integer, ForeignKey("catering_payments.id", ondelete="SET NULL"), nullable=True, index=True)
