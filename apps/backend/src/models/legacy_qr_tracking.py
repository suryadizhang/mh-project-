...
class QRCode(Base):
    ...
    created_by: UUID | None = Column(
        PG_UUID(as_uuid=True), ForeignKey("core.users.id", ondelete="SET NULL"), nullable=True
    )
...
class QRScan(Base):
    ...
    scanned_at: datetime = Column(DateTime(timezone=True), default=datetime.utcnow, index=True)

    # Device Information
    ip_address: str | None = Column(String(45), nullable=True)
    user_agent: str | None = Column(Text, nullable=True)
    device_type: str | None = Column(String(50), nullable=True)  # mobile, tablet, desktop
