...
class SocialAccount(Base):
    ...
    connected_by = Column(
        UUID(as_uuid=True), nullable=False, comment="User who connected this account"
    )
    ...
    platform_metadata = Column(
        JSONB, nullable=True, comment="Platform-specific settings and capabilities"
    )
...
class SocialIdentity(Base):
    ...
    confidence_score = Column(
        Float, default=1.0, nullable=False, comment="Confidence in customer mapping"
    )
...
class SocialThread(Base):
    ...
    account_id = Column(
        UUID(as_uuid=True),
        ForeignKey("core.social_accounts.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    social_identity_id = Column(
        UUID(as_uuid=True),
        ForeignKey("core.social_identities.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class SocialMessage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    thread_id = Column(
        UUID(as_uuid=True), ForeignKey("core.social_threads.id", ondelete="CASCADE"), nullable=False
    )
    ...
    intent_tags = Column(
        ARRAY(String(50)), nullable=True, comment="Detected intents: booking, complaint, etc"
    )
    is_public = Column(
        Boolean, default=False, nullable=False, comment="Public comment vs private DM"
    )
    requires_approval = Column(
        Boolean, default=False, nullable=False, comment="AI response needs human approval"
    )
    approved_by = Column(UUID(as_uuid=True), nullable=True, comment="Who approved the response")
    ai_generated = Column(Boolean, default=False, nullable=False)
    parent_message_id = Column(
        UUID(as_uuid=True), ForeignKey("core.social_messages.id", ondelete="CASCADE"), nullable=True
    )
    platform_metadata = Column(JSONB, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    sent_at = Column(
        DateTime(timezone=True), nullable=True, comment="When actually posted to platform"
    )
...
class Review(Base):
    ...
    thread_id = Column(
        UUID(as_uuid=True), ForeignKey("core.social_threads.id", ondelete="SET NULL"), nullable=True
    )
    author_handle = Column(String(100), nullable=True)
    author_name = Column(String(255), nullable=True)
