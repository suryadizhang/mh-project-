...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def _get_redis_client(self) -> aioredis.Redis:
        ...
        if self.redis_client is None:
            try:
                ...
                logger.error(f"❌ Redis connection failed for rate limiter: {e}")
                raise HTTPException(
    ...
    def _get_rate_limit_for_role(self, role: Optional[str]) -> int:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if is_login_endpoint:
                ...
                lockout_check = await self._track_login_attempt(redis, identifier, success=False)
                ...
                if lockout_check["locked"]:
                    ...
                    return JSONResponse(
                        ...
                        headers={
                            ...
                            "X-RateLimit-Remaining-Minutes": str(lockout_check["remaining_minutes"])
                        }
                    )
...
async def check_login_lockout(identifier: str, redis_url: Optional[str] = None) -> dict:
    """
