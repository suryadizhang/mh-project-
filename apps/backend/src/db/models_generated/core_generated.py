...
from sqlalchemy import ARRAY, Boolean, CheckConstraint, Date, DateTime, Double, Enum, ForeignKeyConstraint, Index, Integer, JSON, PrimaryKeyConstraint, String, Text, Time, UniqueConstraint, Uuid, text
...
class Chefs(Base):
    __tablename__ = 'chefs'
    __table_args__ = (
        PrimaryKeyConstraint('id', name='chefs_pkey'),
        {'schema': 'core'}
    )
...
class SocialAccounts(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, server_default=text('gen_random_uuid()'))
    platform: Mapped[str] = mapped_column(Enum('instagram', 'facebook', 'google_business', 'yelp', 'tiktok', 'twitter', name='social_platform'), nullable=False)
    page_id: Mapped[str] = mapped_column(String(255), nullable=False, comment='Platform-specific page/business ID')
    page_name: Mapped[str] = mapped_column(String(255), nullable=False, comment='Display name of the business page')
    connected_by: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, comment='User who connected this account')
    connected_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    ...
    handle: Mapped[Optional[str]] = mapped_column(String(100), comment='@username or handle if applicable')
    profile_url: Mapped[Optional[str]] = mapped_column(Text)
    avatar_url: Mapped[Optional[str]] = mapped_column(Text)
    token_ref: Mapped[Optional[str]] = mapped_column(Text, comment='Encrypted reference to access tokens')
    last_sync_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(True))
    metadata_: Mapped[Optional[dict]] = mapped_column('metadata', JSONB, comment='Platform-specific settings and capabilities')
    deleted_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(True))

    social_threads: Mapped[list['SocialThreads']] = relationship('SocialThreads', back_populates='account')
...
class SocialIdentities(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, server_default=text('gen_random_uuid()'))
    platform: Mapped[str] = mapped_column(Enum('instagram', 'facebook', 'google_business', 'yelp', 'tiktok', 'twitter', name='social_platform'), nullable=False)
    handle: Mapped[str] = mapped_column(String(100), nullable=False, comment='Social handle without @ prefix')
    confidence_score: Mapped[float] = mapped_column(Double(53), nullable=False, comment='Confidence in customer mapping')
    verification_status: Mapped[str] = mapped_column(String(50), nullable=False)
    first_seen_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    ...
    social_threads: Mapped[list['SocialThreads']] = relationship('SocialThreads', back_populates='social_identity')
...
class Stations(Base):
    ...
    message_threads: Mapped[list['MessageThreads']] = relationship('MessageThreads', back_populates='station')
...
class Customers(Base):
    ...
    message_threads: Mapped[list['MessageThreads']] = relationship('MessageThreads', back_populates='customer')
    social_threads: Mapped[list['SocialThreads']] = relationship('SocialThreads', back_populates='customer')
...
class Bookings(Base):
    ...
    status: Mapped[str] = mapped_column(Enum('new', 'deposit_pending', 'confirmed', 'completed', 'cancelled', 'no_show', name='booking_status'), nullable=False)
...
class MessageThreads(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['customer_id'], ['core.customers.id'], ondelete='CASCADE', name='message_threads_customer_id_fkey'),
        ForeignKeyConstraint(['station_id'], ['identity.stations.id'], ondelete='RESTRICT', name='fk_message_threads_station'),
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, server_default=text('gen_random_uuid()'))
    customer_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    channel: Mapped[str] = mapped_column(Enum('web_chat', 'sms', 'email', 'phone', 'admin', name='message_channel'), nullable=False)
    status: Mapped[str] = mapped_column(Enum('active', 'waiting', 'resolved', 'escalated', 'closed', 'urgent', name='thread_status'), nullable=False)
    ai_mode: Mapped[bool] = mapped_column(Boolean, nullable=False)
    last_message_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('now()'))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('now()'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('now()'))
    station_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
...
class SocialThreads(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['account_id'], ['core.social_accounts.id'], ondelete='CASCADE', name='social_threads_account_id_fkey'),
        ForeignKeyConstraint(['customer_id'], ['core.customers.id'], ondelete='SET NULL', name='social_threads_customer_id_fkey'),
        ForeignKeyConstraint(['social_identity_id'], ['core.social_identities.id'], ondelete='SET NULL', name='social_threads_social_identity_id_fkey'),
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, server_default=text('gen_random_uuid()'))
    platform: Mapped[str] = mapped_column(Enum('instagram', 'facebook', 'google_business', 'yelp', 'tiktok', 'twitter', name='social_platform'), nullable=False)
    thread_ref: Mapped[str] = mapped_column(String(255), nullable=False, comment='Platform-specific thread/conversation ID')
    account_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, comment='Our social account')
    status: Mapped[str] = mapped_column(Enum('open', 'pending', 'resolved', 'snoozed', 'escalated', name='social_thread_status'), nullable=False)
    ...
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    customer_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Linked customer if known')
    lead_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Generated lead if applicable')
    social_identity_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Other party social identity')
    subject: Mapped[Optional[str]] = mapped_column(String(255), comment='Thread subject or first message preview')
    context_url: Mapped[Optional[str]] = mapped_column(Text, comment='Link to original post/content')
    ...
    account: Mapped['SocialAccounts'] = relationship('SocialAccounts', back_populates='social_threads')
    customer: Mapped[Optional['Customers']] = relationship('Customers', back_populates='social_threads')
    social_identity: Mapped[Optional['SocialIdentities']] = relationship('SocialIdentities', back_populates='social_threads')
    reviews: Mapped[list['Reviews']] = relationship('Reviews', back_populates='thread')
    social_messages: Mapped[list['SocialMessages']] = relationship('SocialMessages', back_populates='thread')


class Messages(Base):
    __tablename__ = 'messages'
    __table_args__ = (
        CheckConstraint("direction::text = ANY (ARRAY['inbound'::character varying, 'outbound'::character varying]::text[])", name='check_message_direction'),
        CheckConstraint("sender_type::text = ANY (ARRAY['customer'::character varying, 'agent'::character varying, 'ai'::character varying, 'system'::character varying]::text[])", name='check_sender_type'),
        ForeignKeyConstraint(['thread_id'], ['core.message_threads.id'], ondelete='CASCADE', name='messages_thread_id_fkey'),
...
class Reviews(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("sentiment_score >= '-1.0'::numeric::double precision AND sentiment_score <= 1.0::double precision", name='ck_reviews_sentiment_range'),
        ForeignKeyConstraint(['account_id'], ['core.social_accounts.id'], ondelete='CASCADE', name='reviews_account_id_fkey'),
        ForeignKeyConstraint(['customer_id'], ['core.customers.id'], ondelete='SET NULL', name='reviews_customer_id_fkey'),
        ForeignKeyConstraint(['thread_id'], ['core.social_threads.id'], ondelete='SET NULL', name='reviews_thread_id_fkey'),
    ...
    review_ref: Mapped[str] = mapped_column(String(255), nullable=False, comment='Platform-specific review ID')
    status: Mapped[str] = mapped_column(Enum('new', 'acknowledged', 'responded', 'escalated', 'closed', name='review_status'), nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    thread_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Associated thread if replies exist')
    author_handle: Mapped[Optional[str]] = mapped_column(String(100))
    author_name: Mapped[Optional[str]] = mapped_column(String(255))
    rating: Mapped[Optional[int]] = mapped_column(Integer, comment='1-5 star rating where applicable')
    ...
    booking_ref: Mapped[Optional[str]] = mapped_column(String(50), comment='Reference to related booking if found')
    assigned_to: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid)
    escalation_reason: Mapped[Optional[str]] = mapped_column(String(255))
    response_due_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(True), comment='SLA deadline')
    ...
    thread: Mapped[Optional['SocialThreads']] = relationship('SocialThreads', back_populates='reviews')


class SocialMessages(Base):
    __tablename__ = 'social_messages'
    __table_args__ = (
        CheckConstraint("sentiment_score >= '-1.0'::numeric::double precision AND sentiment_score <= 1.0::double precision", name='ck_social_messages_sentiment_range'),
        ForeignKeyConstraint(['parent_message_id'], ['core.social_messages.id'], ondelete='CASCADE', name='social_messages_parent_message_id_fkey'),
        ForeignKeyConstraint(['thread_id'], ['core.social_threads.id'], ondelete='CASCADE', name='social_messages_thread_id_fkey'),
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, server_default=text('gen_random_uuid()'))
    thread_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    direction: Mapped[str] = mapped_column(Enum('in', 'out', name='social_message_direction'), nullable=False)
    kind: Mapped[str] = mapped_column(Enum('dm', 'comment', 'review', 'reply', 'mention', 'story_reply', name='social_message_kind'), nullable=False)
    is_public: Mapped[bool] = mapped_column(Boolean, nullable=False, comment='Public comment vs private DM')
    requires_approval: Mapped[bool] = mapped_column(Boolean, nullable=False, comment='AI response needs human approval')
    ai_generated: Mapped[bool] = mapped_column(Boolean, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
    message_ref: Mapped[Optional[str]] = mapped_column(String(255), comment='Platform-specific message ID')
    author_handle: Mapped[Optional[str]] = mapped_column(String(100))
    author_name: Mapped[Optional[str]] = mapped_column(String(255))
    body: Mapped[Optional[str]] = mapped_column(Text)
    media: Mapped[Optional[dict]] = mapped_column(JSONB, comment='Attachments, images, videos')
    sentiment_score: Mapped[Optional[float]] = mapped_column(Double(53), comment='AI sentiment analysis -1 to 1')
    intent_tags: Mapped[Optional[list[str]]] = mapped_column(ARRAY(String(length=50)), comment='Detected intents: booking, complaint, etc')
    approved_by: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Who approved the response')
    parent_message_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='Reply to this message')
    metadata_: Mapped[Optional[dict]] = mapped_column('metadata', JSONB)
    sent_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(True), comment='When actually posted to platform')
    ...
    parent_message: Mapped[Optional['SocialMessages']] = relationship('SocialMessages', remote_side=[id], back_populates='parent_message_reverse')
    parent_message_reverse: Mapped[list['SocialMessages']] = relationship('SocialMessages', remote_side=[parent_message_id], back_populates='parent_message')
    thread: Mapped['SocialThreads'] = relationship('SocialThreads', back_populates='social_messages')
