...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('category', postgresql.ENUM('policy', 'pricing', 'menu', 'travel', 'payment', 'cancellation', 'terms', 'guidelines', name='rule_category', create_type=False), nullable=False),
    ...
    op.create_table(
        'training_data',
        sa.Column('id', sa.String(36), primary_key=True),
        sa.Column('example_type', sa.String(100), nullable=False),  # greeting, pricing, upsell, objection, etc.
        sa.Column('tone', postgresql.ENUM('formal', 'casual', 'direct', 'warm', 'anxious', name='tone_type', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(36), primary_key=True),
        sa.Column('rule_name', sa.String(200), nullable=False),
        sa.Column('trigger_type', postgresql.ENUM('guest_count', 'event_type', 'date', 'location', 'budget', name='upsell_trigger_type', create_type=False), nullable=False),
        sa.Column('trigger_condition', sa.JSON, nullable=False),  # e.g., {"guest_count": {"min": 10}}
    ...
    op.create_table(
        ...
        sa.Column('offer_type', sa.String(100), nullable=False),  # discount, bundle, special_menu, etc.
        ...
        sa.Column('conditions', sa.JSON, nullable=True),  # e.g., {"min_guests": 15, "days": ["friday", "saturday"]}
        sa.Column('status', postgresql.ENUM('draft', 'active', 'expired', 'scheduled', name='offer_status', create_type=False), default='draft'),
    ...
    op.create_table(
        ...
        sa.Column('detected_tone', postgresql.ENUM('formal', 'casual', 'direct', 'warm', 'anxious', name='tone_type', create_type=False), nullable=False),
        sa.Column('confidence_score', sa.Numeric(5, 2), nullable=True),  # 0-100%
        sa.Column('interaction_channel', sa.String(50), nullable=False),  # email, sms, voice, instagram, etc.
    ...
    op.create_index('idx_business_rules_keywords', 'business_rules', ['keywords'], postgresql_using='gin', schema='public')
    op.create_index('idx_business_rules_effective', 'business_rules', ['effective_from', 'effective_until'], schema='public')
    ...
    op.create_index('idx_faq_keywords', 'faq_items', ['keywords'], postgresql_using='gin', schema='public')
    ...
    op.create_index('idx_offers_dates', 'seasonal_offers', ['valid_from', 'valid_until'], schema='public')
    ...
    op.create_index('idx_calendar_datetime', 'availability_calendar', ['date', 'time_slot'], schema='public')
    op.create_index('idx_calendar_available', 'availability_calendar', ['is_available'], schema='public')
    op.create_index('idx_calendar_station', 'availability_calendar', ['station_id'], schema='public')
    
    # customer_tone_preferences indexes
    op.create_index('idx_tone_customer', 'customer_tone_preferences', ['customer_id'], schema='public')
    op.create_index('idx_tone_detected', 'customer_tone_preferences', ['detected_tone'], schema='public')
    op.create_index('idx_tone_channel', 'customer_tone_preferences', ['interaction_channel'], schema='public')
    op.create_index('idx_tone_date', 'customer_tone_preferences', ['interaction_date'], schema='public')
    
    # Composite indexes for common queries
    op.create_index('idx_business_rules_lookup', 'business_rules', ['category', 'is_active', 'station_id'], schema='public')
    op.create_index('idx_faq_lookup', 'faq_items', ['category', 'is_active', 'priority'], schema='public')
    op.create_index('idx_training_lookup', 'training_data', ['tone', 'example_type', 'is_active'], schema='public')
    op.create_index('idx_tone_history', 'customer_tone_preferences', ['customer_id', 'interaction_date'], schema='public')
    
    # Foreign key constraint (customer_tone_preferences -> customers)
    op.create_foreign_key(
        'fk_tone_customer',
        'customer_tone_preferences', 'customers',
        ['customer_id'], ['id'],
        ondelete='CASCADE',
