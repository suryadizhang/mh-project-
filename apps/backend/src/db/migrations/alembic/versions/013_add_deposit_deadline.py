...
def upgrade():
    ...
    op.add_column("bookings", sa.Column("customer_deposit_deadline", sa.DateTime(), nullable=True))

    # Add internal enforcement deadline (24 hours - grace period)
    op.add_column("bookings", sa.Column("internal_deadline", sa.DateTime(), nullable=True))

    # Keep old deposit_deadline for backward compatibility
    op.add_column("bookings", sa.Column("deposit_deadline", sa.DateTime(), nullable=True))

    # Add manual deposit confirmation tracking
    op.add_column("bookings", sa.Column("deposit_confirmed_at", sa.DateTime(), nullable=True))
    op.add_column("bookings", sa.Column("deposit_confirmed_by", sa.String(255), nullable=True))
    ...
    op.add_column("bookings", sa.Column("held_by", sa.String(255), nullable=True))
    op.add_column("bookings", sa.Column("held_at", sa.DateTime(), nullable=True))
    op.add_column("bookings", sa.Column("hold_reason", sa.Text(), nullable=True))
...
def downgrade():
    ...
    # Drop indexes
    op.drop_index("ix_bookings_internal_deadline", table_name="bookings")
    op.drop_index("ix_bookings_customer_deposit_deadline", table_name="bookings")

    # Drop hold columns
    op.drop_column("bookings", "hold_reason")
    op.drop_column("bookings", "held_at")
    op.drop_column("bookings", "held_by")
    op.drop_column("bookings", "hold_on_request")

    # Drop manual confirmation columns
    op.drop_column("bookings", "deposit_confirmed_by")
    op.drop_column("bookings", "deposit_confirmed_at")

    # Drop deadline columns
    op.drop_column("bookings", "deposit_deadline")
    op.drop_column("bookings", "internal_deadline")
    op.drop_column("bookings", "customer_deposit_deadline")
