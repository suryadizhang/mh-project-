...
class AdminInvitation(Base):
    ...
    station_id: Mapped[Optional[UUID]] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("identity.stations.id"), nullable=True, index=True
    )
...
class GoogleOAuthAccount(Base):
    ...
    provider_account_id: Mapped[str] = mapped_column(String(255), nullable=False)
    provider_account_email: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
...
class AdminAccessLog(Base):
    ...
    # Context
    user_id: Mapped[Optional[UUID]] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("identity.users.id"), nullable=True, index=True
    )
    oauth_account_id: Mapped[Optional[UUID]] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("identity.oauth_accounts.id"), nullable=True
    )
    email: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
