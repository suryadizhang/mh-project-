...
# MIGRATED: from models.base → ..base_class (2 levels up from db/models/)
...
class AddonCategory(str, Enum):
    """Addon item categories"""

    PROTEIN_UPGRADES = "protein_upgrades"
...
class BusinessRule(Base):
    ...
    category = Column(PgEnum(RuleCategory, name="rule_category", create_type=False), nullable=False)
...
class TrainingData(Base):
    ...
    tone = Column(PgEnum(ToneType, name="tone_type", create_type=False), nullable=False)
...
class UpsellRule(Base):
    ...
    rule_name = Column(String(200), nullable=False)
    trigger_type = Column(
        PgEnum(UpsellTriggerType, name="upsell_trigger_type", create_type=False), nullable=False
    )
...
class SeasonalOffer(Base):
    ...
    status = Column(
        PgEnum(OfferStatus, name="offer_status", create_type=False), default=OfferStatus.DRAFT
    )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "discount_percentage": (
                str(self.discount_percentage) if self.discount_percentage else None
            ),
...
class CustomerTonePreference(Base):
    ...
    id = Column(String(36), primary_key=True)
    customer_id = Column(
        String(36), ForeignKey("core.customers.id", ondelete="CASCADE"), nullable=False
    )  # Fixed: customers table is in core schema
    detected_tone = Column(PgEnum(ToneType, name="tone_type", create_type=False), nullable=False)
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "detected_tone": (
                self.detected_tone.value
                if isinstance(self.detected_tone, Enum)
                else self.detected_tone
            ),
...
class MenuItem(Base):
    ...
    Example:
      ...
      tags: ["seafood", "shellfish", "grilled", "gluten_free", "contains_shellfish"]
    """

    __tablename__ = "menu_items"
    __table_args__ = (
        Index("idx_menu_items_category", "category"),
        Index("idx_menu_items_subcategory", "subcategory"),
        Index("idx_menu_items_tags", "tags", postgresql_using="gin"),
    ...
    category = Column(PgEnum(MenuCategory, name="menu_category", create_type=False), nullable=False)
    subcategory = Column(
        String(100), nullable=True
    )  # poultry, fish, shellfish, beef, pork, vegetarian, tofu
    tags = Column(
        JSONB, default=list
    )  # ["gluten_free", "contains_shellfish", "grilled", "premium"]
    base_price = Column(Numeric(10, 2), nullable=True)
    is_premium = Column(Boolean, default=False)
    is_available = Column(Boolean, default=True)
    dietary_info = Column(ARRAY(String), nullable=True)  # Legacy - consider migrating to tags
...
class PricingTier(Base):
    ...
    id = Column(String(36), primary_key=True)
    tier_level = Column(
        PgEnum(PricingTierLevel, name="pricing_tier_level", create_type=False), nullable=False
    )
    name = Column(String(200), nullable=False)
    description = Column(Text, nullable=True)
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            "id": self.id,
            "tier_level": (
                self.tier_level.value if isinstance(self.tier_level, Enum) else self.tier_level
            ),
            "name": self.name,
            "description": self.description,
...
class AddonItem(Base):
    """
    Addon items (protein upgrades, enhancements, equipment, etc.)

    Allows dynamic addon pricing management without code deployment
    """

    __tablename__ = "addon_items"
    __table_args__ = (
        Index("idx_addon_items_category", "category"),
        Index("idx_addon_items_active", "is_active"),
        Index("idx_addon_items_station", "station_id"),
        Index("idx_addon_items_lookup", "category", "is_active", "display_order"),
        {"schema": "public", "extend_existing": True},
    )
    ...
    description = Column(Text, nullable=True)
    category = Column(
        PgEnum(AddonCategory, name="addon_category", create_type=False), nullable=False
    )
...
class SyncHistory(Base):
    ...
    source_type = Column(
        PgEnum(SyncSourceType, name="sync_source_type", create_type=True), nullable=False
    )
    ...
    sync_type = Column(PgEnum(SyncType, name="sync_type", create_type=True), default=SyncType.AUTO)
    sync_status = Column(
        PgEnum(SyncStatus, name="sync_status", create_type=True), default=SyncStatus.SUCCESS
    )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            "id": self.id,
            "source_type": (
                self.source_type.value if isinstance(self.source_type, Enum) else self.source_type
            ),
            ...
            "sync_type": (
                self.sync_type.value if isinstance(self.sync_type, Enum) else self.sync_type
            ),
            "sync_status": (
                self.sync_status.value if isinstance(self.sync_status, Enum) else self.sync_status
            ),
