...
} from '../../../../libs/gsm-client/src/index';
...
class ConfigManager {
  ...
  async loadConfig(): Promise<BackendConfig> {
    ...
    try {
      ...
      this.config = {
        ...
        SESSION_ENCRYPTION_KEY:
          secrets.SESSION_ENCRYPTION_KEY || this.generateFallbackKey(),
        ...
        EMAIL_DEFAULT_FROM:
          secrets.EMAIL_DEFAULT_FROM || 'noreply@myhibachichef.com',
        ...
        FF_ENABLE_AI_BOOKING_V3: this.parseBoolean(
          secrets.FF_ENABLE_AI_BOOKING_V3,
          true
        ),
        FF_ENABLE_TRAVEL_FEE_V2: this.parseBoolean(
          secrets.FF_ENABLE_TRAVEL_FEE_V2,
          false
        ),
        FF_ENABLE_SMS_REMINDERS: this.parseBoolean(
          secrets.FF_ENABLE_SMS_REMINDERS,
          true
        ),
      ...
      console.log(
        `🚀 Feature flags: AI_V3=${this.config.FF_ENABLE_AI_BOOKING_V3}, Travel_V2=${this.config.FF_ENABLE_TRAVEL_FEE_V2}`
      );
  ...
  startConfigMonitoring(intervalMinutes: number = 5): void {
    console.log(
      `🔍 Starting config monitoring (every ${intervalMinutes} minutes)`
    );
  ...
  private generateFallbackKey(): string {
    return (
      ...
      'fallback-key-' + Math.random().toString(36)
    );
  }
  ...
  private loadFromEnv(): BackendConfig {
    ...
    return {
      ...
      SESSION_ENCRYPTION_KEY:
        process.env.SESSION_ENCRYPTION_KEY || 'dev-session-key',
      ...
      EMAIL_DEFAULT_FROM:
        process.env.EMAIL_DEFAULT_FROM || 'noreply@myhibachichef.com',
      ...
      // Feature Flags
      FF_ENABLE_AI_BOOKING_V3: this.parseBoolean(
        process.env.FF_ENABLE_AI_BOOKING_V3,
        true
      ),
      FF_ENABLE_TRAVEL_FEE_V2: this.parseBoolean(
        process.env.FF_ENABLE_TRAVEL_FEE_V2,
        false
      ),
      FF_ENABLE_SMS_REMINDERS: this.parseBoolean(
        process.env.FF_ENABLE_SMS_REMINDERS,
        true
      ),
 ...
 */
export const isAIBookingV3Enabled = () =>
  getFeatureFlag('FF_ENABLE_AI_BOOKING_V3');
export const isTravelFeeV2Enabled = () =>
  getFeatureFlag('FF_ENABLE_TRAVEL_FEE_V2');
export const isSMSRemindersEnabled = () =>
  getFeatureFlag('FF_ENABLE_SMS_REMINDERS');
