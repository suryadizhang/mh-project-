...
class MessageRouter:
    ...
    async def route_message(self, request: SendMessageRequest) -> MessageResponse:
        ...
        try:
            logger.info(
                f"Routing {request.channel} message to {request.phone_number or request.email_address}"
            )
            ...
            message.status = (
                MessageStatus.DELIVERED if result.get("success") else MessageStatus.FAILED
            )
            ...
            logger.exception(f"Message routing failed: {e!s}")
        ...
        try:
            ...
            if channel == MessageChannel.SMS and sender_info.get("phone_number"):
                tcpa_command = await self.tcpa_handler.handle_tcpa_command(
                    content.strip().upper(), sender_info["phone_number"], channel
                )
                if tcpa_command:
                    # TCPA command processed, send auto-response
                    return await self._send_tcpa_auto_response(
                        sender_info["phone_number"], tcpa_command, channel
                    )
            ...
            logger.exception(f"Inbound message processing failed: {e!s}")
            await self.db.rollback()
            raise
    ...
    async def _find_or_create_contact(
        self, sender_info: dict[str, Any], channel: MessageChannel
    ) -> UUID | None:
    ...
    async def _notify_websocket_clients(self, message: Message, event_type: str):
        ...
        try:
            ...
            query = select(WebSocketConnection).where(WebSocketConnection.is_active)
            result = await self.db.execute(query)
            ...
            logger.exception(f"WebSocket notification failed: {e!s}")

    async def _send_tcpa_auto_response(
        self, phone_number: str, command: str, channel: MessageChannel
    ) -> MessageResponse:
...
class TCPAHandler:
    ...
    async def check_tcpa_compliance(self, phone_number: str, channel: MessageChannel) -> bool:
        ...
        query = select(TCPAOptStatus).where(
            and_(TCPAOptStatus.phone_number == normalized_phone, TCPAOptStatus.channel == channel)
        )
    ...
    async def handle_tcpa_command(
        self, content: str, phone_number: str, channel: MessageChannel
    ) -> str | None:
    ...
    async def _update_tcpa_status(
        self, phone_number: str, channel: MessageChannel, status: TCPAStatus, method: str
    ):
        """Update TCPA opt status"""

        # Check for existing record
        query = select(TCPAOptStatus).where(
            and_(TCPAOptStatus.phone_number == phone_number, TCPAOptStatus.channel == channel)
        )
