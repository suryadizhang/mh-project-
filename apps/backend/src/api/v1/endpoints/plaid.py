...
async def check_account_balance(
    ...
    **Recommendation**: Only check balance for large transactions ($500+)
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to check account balance"
        )
    ...
    **Processing Time**: Instant (Real-Time Payment)
    ...
    try:
        ...
        result = await plaid_service.initiate_payment(
            ...
            description=request.description or f"My Hibachi payment for booking {request.booking_id}"
        )
        ...
        return InitiatePaymentResponse(
            **result,
            fees=fees,
            savings_vs_stripe=fees['savings_vs_stripe']
        )
