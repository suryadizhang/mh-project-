...
from models.payment_notification import (
    PaymentNotification, CateringBooking, CateringPayment,
    PaymentProvider, PaymentNotificationStatus
)
...
router = APIRouter(prefix="/api/v1/admin/payment-notifications", tags=["Payment Notifications - Admin"])
...
class CreateTestBookingRequest(BaseModel):
    ...
    alternative_payer_name: Optional[str] = None
    alternative_payer_phone: Optional[str] = None
    alternative_payer_venmo: Optional[str] = None
...
async def get_notification_stats(
    ...
    **Returns:**
    ...
    pending = db.query(PaymentNotification).filter(
        PaymentNotification.status == PaymentNotificationStatus.PENDING_MATCH
    ).count()
    matched = db.query(PaymentNotification).filter(
        PaymentNotification.status == PaymentNotificationStatus.MATCHED
    ).count()
    confirmed = db.query(PaymentNotification).filter(
        PaymentNotification.status == PaymentNotificationStatus.CONFIRMED
    ).count()
    manual_review = db.query(PaymentNotification).filter(
        PaymentNotification.requires_manual_review == True,
        PaymentNotification.is_processed == False
    ).count()
    errors = db.query(PaymentNotification).filter(
        PaymentNotification.status == PaymentNotificationStatus.ERROR
    ).count()
    
    # Provider breakdown
    stripe_count = db.query(PaymentNotification).filter(
        PaymentNotification.provider == PaymentProvider.STRIPE
    ).count()
    venmo_count = db.query(PaymentNotification).filter(
        PaymentNotification.provider == PaymentProvider.VENMO
    ).count()
    zelle_count = db.query(PaymentNotification).filter(
        PaymentNotification.provider == PaymentProvider.ZELLE
    ).count()
    
    # Recent activity
    last_24h = db.query(PaymentNotification).filter(
        PaymentNotification.received_at >= day_ago
    ).count()
    last_7d = db.query(PaymentNotification).filter(
        PaymentNotification.received_at >= week_ago
    ).count()
    
    # Matching performance
    avg_score = db.query(func.avg(PaymentNotification.match_score)).filter(
        PaymentNotification.match_score > 0
    ).scalar() or 0
    
    auto_matched = db.query(PaymentNotification).filter(
        PaymentNotification.status.in_([
            PaymentNotificationStatus.MATCHED,
            PaymentNotificationStatus.CONFIRMED
        ]),
        PaymentNotification.requires_manual_review == False
    ).count()
...
async def list_notifications(
    ...
    requires_review: Optional[bool] = Query(None, description="Filter needs manual review"),
    ...
    **Returns:**
    ...
    query = db.query(PaymentNotification).filter(
        PaymentNotification.received_at >= since_date
    )
...
async def get_notification_detail(
    ...
    **Returns:**
    ...
    """
    notif = db.query(PaymentNotification).filter(
...
async def create_test_booking(
    ...
    **Returns:**
    ...
    try:
        ...
        logger.info(f"Test booking created: ID={booking.id}, Customer={booking.customer_name}, Amount=${booking.total_amount}")
        ...
        db.rollback()
        logger.error(f"Failed to create test booking: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def manual_match_notification(
    ...
    **Returns:**
    ...
    try:
        # Get notification
        notif = db.query(PaymentNotification).filter(
            PaymentNotification.id == request.notification_id
        ).first()
        ...
        booking = db.query(CateringBooking).filter(
            CateringBooking.id == request.booking_id
        ).first()
        ...
        notif.status = PaymentNotificationStatus.CONFIRMED if request.confirm_payment else PaymentNotificationStatus.MATCHED
