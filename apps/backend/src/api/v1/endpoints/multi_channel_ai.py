...
from api.ai.endpoints.services.customer_booking_ai import get_customer_booking_ai
...
class MultiChannelInquiry(BaseModel):
    ...
    channel: str = Field(..., description="Communication channel: email, sms, instagram, facebook, phone_transcript, web_chat")
    customer_metadata: Optional[Dict[str, Any]] = Field(default=None, description="Additional customer metadata (if available)")
    ...
    **Example Use Cases**:
    ...
    try:
        ...
        response = await multi_channel_handler.process_multi_channel_inquiry(
            ...
            customer_booking_ai=customer_ai
        )
        ...
        logger.error(f"❌ Error processing {inquiry.channel} inquiry: {str(e)}")
        raise HTTPException(
...
@router.post("/inquiries/batch", response_model=List[ChannelResponse], status_code=status.HTTP_200_OK)
    ...
    **Rate Limit**: Max 10 inquiries per batch
    ...
    if len(inquiries) > 10:
        raise HTTPException(
            ...
            detail="Maximum 10 inquiries per batch request"
        )
    ...
    for inquiry in inquiries:
        try:
            ...
            logger.error(f"❌ Error processing inquiry in batch: {str(e)}")
            # Continue with other inquiries
            continue
    
    total_time = (time.time() - start_time) * 1000
    logger.info(f"✅ Batch processed {len(responses)}/{len(inquiries)} inquiries in {total_time:.2f}ms")
...
async def test_all_channels(
    ...
    **Use Case**: Training, quality assurance, A/B testing different responses
    ...
    return {
        "original_message": test_request.message,
        "responses_by_channel": responses
    }
...
async def analyze_inquiry(
    analysis_request: InquiryAnalysis,
    multi_channel_handler = Depends(get_multi_channel_handler)
):
    """
    ...
    try:
        ...
        return {
            ...
            "routing_recommendation": {
                ...
                "requires_human_review": details["sentiment"] == "negative" or details["urgency"] == "urgent",
                "estimated_response_model": "gpt-4" if details["inquiry_type"] in ["complaint", "quote"] else "gpt-3.5-turbo"
            }
        }
    except Exception as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to analyze inquiry: {str(e)}"
        )
