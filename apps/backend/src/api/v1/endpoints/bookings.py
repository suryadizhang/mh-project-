...
class BookingCreate(BookingBase):
    ...
    marketing_consent: bool = Field(
        default=False, description="Consent for marketing communications"
    )
...
class BookingUpdate(BaseModel):
    ...
    guest_count: int | None = Field(None, ge=2, le=20)
...
async def create_booking(
    booking_data: BookingCreate, response: Response, db: AsyncSession = Depends(get_db)
):
    """
    ...
    logger.info(
        f"New booking request created: {new_booking['id']} for {booking_data.customer_email}"
    )
...
async def list_bookings(
    ...
    search: str | None = Query(None, description="Search customer name or email"),
    ...
    logger.info(
        f"Admin {current_user['email']} listed bookings: {len(paginated_bookings)} of {total}"
    )
