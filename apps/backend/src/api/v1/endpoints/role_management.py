...
from models.role import Role, Permission, RoleType
...
async def list_roles(
    ...
    return [
        RoleResponse(
            ...
            permissions=[p.name.value if hasattr(p.name, 'value') else p.name for p in role.permissions]
        )
        for role in roles
    ]
...
async def assign_role_to_user(
    ...
    role_result = await db.execute(
        select(Role).where(Role.id == UUID(request.role_id))
    )
...
async def remove_role_from_user(
    ...
    if not role_to_remove:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User does not have role {role_id}"
        )
...
async def get_user_permissions(
    ...
    for role in user.roles:
        for permission in role.permissions:
            perm_name = permission.name.value if hasattr(permission.name, 'value') else permission.name
