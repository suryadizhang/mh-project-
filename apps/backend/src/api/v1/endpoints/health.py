...
async def check_stripe() -> ServiceHealthCheck:
    ...
    try:
        ...
        webhook_secret = getattr(settings, 'STRIPE_WEBHOOK_SECRET', None) or getattr(settings, 'stripe_webhook_secret', None)
...
def get_configuration_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "stripe_configured": bool(getattr(settings, 'STRIPE_SECRET_KEY', None) or getattr(settings, 'stripe_secret_key', None)),
            "email_configured": bool(getattr(settings, 'SMTP_USER', None) or getattr(settings, 'smtp_user', None))
        }
    except Exception as e:
    ...
    try:
        ...
        if isinstance(db_check, Exception):
            db_check = ServiceHealthCheck(
                status="unhealthy",
                details="Database check exception",
                error=str(db_check)
            )
        ...
        if isinstance(stripe_check, Exception):
            stripe_check = ServiceHealthCheck(
                status="degraded",
                details="Stripe check exception",
                error=str(stripe_check)
            )
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            SERVICE_HEALTH_GAUGE.labels(service='database').set(1 if db_check.status == "healthy" else 0)
            SERVICE_HEALTH_GAUGE.labels(service='redis').set(1 if redis_check.status == "healthy" else 0)
            SERVICE_HEALTH_GAUGE.labels(service='stripe').set(1 if stripe_check.status == "healthy" else 0)
        
        response = ReadinessResponse(
            status=overall_status,
            checks=checks,
            ready=is_ready,
            details="Database is critical for readiness" if not is_ready else "All critical services ready"
        )
        ...
        raise HTTPException(
            ...
            detail={
                "status": "not_ready",
                "error": str(e),
                "details": "Readiness check exception"
            }
        )
...
async def detailed_health_check(request: Request):
    ...
    try:
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            HEALTH_CHECK_DURATION.labels(endpoint='detailed', check_type='comprehensive').observe(duration)
