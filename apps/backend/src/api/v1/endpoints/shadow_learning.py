...
async def get_shadow_model_info():
    ...
    if model_info is None:
        raise HTTPException(
            status_code=503,
            detail="Llama-3 model not available. Run 'ollama pull llama3'"
        )
...
@router.get("/stats")
async def get_shadow_learning_stats(
    days: int = 7,
    ...
    stats["interpretation"] = {
        "readiness": "not_ready" if stats["total_pairs"] < 100 else (
            "ready" if stats["average_similarity"] >= 0.85 else "training"
        ),
        "quality_assessment": (
            "excellent" if stats["average_similarity"] >= 0.9 else
            "good" if stats["average_similarity"] >= 0.85 else
            "needs_improvement"
        ),
        "recommendation": (
            ...
            if stats["average_similarity"] >= 0.85 else
            "Continue collecting high-quality pairs"
        )
    }
...
async def get_training_data_preview(
    ...
    """
    pairs = await get_high_quality_pairs(
        db,
        ...
        "pairs": [
            {
                ...
                "teacher_response": pair.teacher_response[:200] + "..." if len(pair.teacher_response) > 200 else pair.teacher_response,
                "student_response": pair.student_response[:200] + "..." if len(pair.student_response) > 200 else pair.student_response,
    ...
    return {
        ...
        "next_steps": (
            ...
            [
                ...
                if not ollama_ready else None,
                f"Collect {100 - stats['total_pairs']} more training pairs"
                if not data_ready else None,
                "Improve response quality (current avg: {:.2f}, target: 0.85)".format(stats["average_similarity"])
                if not quality_ready else None
            ]
        )
    }
