...
async def list_threads(
    ...
    channel: Optional[MessageChannel] = Query(None, description="Filter by channel"),
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.email} listed threads - page {page}, found {total} total")
...
async def get_thread(
    ...
    try:
        ...
        return ThreadWithMessages(
            **thread,
            messages=[MessageResponse(**m) for m in thread_messages]
        )
...
async def send_message(
    ...
    try:
        ...
        thread["last_message_preview"] = message.content[:50] + ("..." if len(message.content) > 50 else "")
        thread["updated_at"] = datetime.now()
        ...
        logger.error(f"Error sending message in thread {thread_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def generate_ai_reply(
    ...
    try:
        ...
        logger.error(f"Error generating AI reply for thread {thread_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
