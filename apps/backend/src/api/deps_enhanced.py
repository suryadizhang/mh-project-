...
def get_booking_repository(
    ...
    try:
        ...
        logger.exception(f"Failed to resolve booking repository: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Booking service unavailable"
        )
...
def get_customer_repository(
    ...
    try:
        ...
        logger.exception(f"Failed to resolve customer repository: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Customer service unavailable"
        )


def get_database_session(container: DependencyInjectionContainer = Depends(get_di_container)):
    """Get database session from DI container"""
    try:
        return container.resolve("database_session")
    except Exception as e:
        logger.exception(f"Failed to resolve database session: {e}")
        raise HTTPException(
...
async def get_admin_user(current_user: AuthenticatedUser = Depends(get_current_user)) -> AdminUser:
    """Require admin user"""
...
async def get_business_context(
    ...
    try:
        ...
        return {"business_info": {}, "quiet_hours": False, "timezone": "America/Los_Angeles"}
...
async def require_api_key(x_api_key: str = Header(..., description="API Key")) -> str:
...
async def get_pagination_params(
    ...
    if per_page > max_per_page:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Per page must be <= {max_per_page}"
        )

    return PaginationParams(page=page, per_page=per_page, offset=(page - 1) * per_page)
...
def with_database_transaction():
    ...
    def _with_transaction(session=Depends(get_database_session)):
        ...
        class TransactionManager:
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                try:
                    ...
                    logger.exception(f"Transaction management error: {e}")
                    self.session.rollback()
                finally:
                    self.session.close()
