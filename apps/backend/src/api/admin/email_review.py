...
class PendingEmailResponse(BaseModel):
    ...
    # Quote details
    estimated_quote: Optional[float]
...
class EmailApprovalRequest(BaseModel):
    ...
    email_id: str
    edited_response: Optional[str] = None
    add_cc: Optional[List[EmailStr]] = None
    add_bcc: Optional[List[EmailStr]] = None
...
async def get_pending_emails(
    ...
    **Returns**: List of pending emails sorted by priority and received time
    ...
    if min_quote:
        filtered_emails = [e for e in filtered_emails if e.estimated_quote and e.estimated_quote >= min_quote]
...
async def approve_and_send_email(
    ...
    **Parameters**:
    ...
    return {
        ...
        "message": "Email approved and sent successfully" if email.status == EmailStatus.SENT else "Email approved and scheduled"
    }
...
async def get_email_stats():
    ...
    **Returns**:
    ...
    stats = {
        ...
        "by_priority": {
            "urgent": len([e for e in all_emails if e.priority == Priority.URGENT and e.status == EmailStatus.PENDING]),
            "high": len([e for e in all_emails if e.priority == Priority.HIGH and e.status == EmailStatus.PENDING]),
            "normal": len([e for e in all_emails if e.priority == Priority.NORMAL and e.status == EmailStatus.PENDING]),
            "low": len([e for e in all_emails if e.priority == Priority.LOW and e.status == EmailStatus.PENDING])
        },
        "by_inquiry_type": {},
        "total_processed": len([e for e in all_emails if e.status in [EmailStatus.SENT, EmailStatus.REJECTED]]),
        "approval_rate": 0.0,
        "average_quote": 0.0
    }
...
async def add_email_to_review_queue(
    ...
    logger.info(f"Added email {email_id} to review queue: {customer_name} - {pending_email.inquiry_type}")
