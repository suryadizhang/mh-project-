...
class ApproveReviewRequest(BaseModel):
    ...
    comment: Optional[str] = Field(None, description="Optional comment from admin")
...
async def get_pending_reviews(
    ...
    try:
        ...
        # Get total count
        count_query = select(func.count()).select_from(CustomerReviewBlogPost).where(
            CustomerReviewBlogPost.status == 'pending'
        )
        ...
        for review in reviews:
            reviews_data.append({
                ...
                'has_external_reviews': bool(review.google_review_url or review.yelp_review_url),
                'media_count': len(review.get_images()),
                'image_count': len([img for img in review.get_images() if img.get('resource_type') == 'image']),
                'video_count': len([img for img in review.get_images() if img.get('resource_type') == 'video'])
            })
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching pending reviews: {str(e)}")
...
async def get_approval_log(
    ...
    try:
        ...
        log_query = select(ReviewApprovalLog).where(
            ReviewApprovalLog.review_id == review_id
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching approval log: {str(e)}")
...
async def get_moderation_stats(
    ...
    try:
        # Count by status
        pending_query = select(func.count()).select_from(CustomerReviewBlogPost).where(
            CustomerReviewBlogPost.status == 'pending'
        )
        approved_query = select(func.count()).select_from(CustomerReviewBlogPost).where(
            CustomerReviewBlogPost.status == 'approved'
        )
        rejected_query = select(func.count()).select_from(CustomerReviewBlogPost).where(
            CustomerReviewBlogPost.status == 'rejected'
        )
        ...
        return {
            ...
            'data': {
                ...
                'approval_rate': round(approved_count / (approved_count + rejected_count) * 100, 2) if (approved_count + rejected_count) > 0 else 0
            }
        }
...
async def hold_review(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Error holding review: {str(e)}")
