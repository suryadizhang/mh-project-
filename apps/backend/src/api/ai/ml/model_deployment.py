...
class ModelDeployment:
    ...
    async def deploy_shadow(
        ...
        self.logger.info(
            f"🚀 Shadow deployment: {new_model_id}",
            extra=deployment
        )
    ...
    async def compare_performance(
        ...
        try:
            ...
            self.logger.error(f"Error comparing performance: {str(e)}", exc_info=True)
            raise
    ...
    def _calculate_metrics(self, messages: List[Any]) -> Dict[str, float]:
        ...
        escalated = sum(
            1 for m in messages 
            if m.metadata and m.metadata.get("escalated")
        )
        ...
        if winner == "candidate":
            improvement = (
                (self._composite_score_simple(candidate_metrics) /
                 self._composite_score_simple(base_metrics) - 1) * 100
            )
    ...
    async def promote_to_production(
        ...
        self.logger.info(
            f"✅ PROMOTED: {model_id} → Production",
            extra=promotion
        )
    ...
    async def rollback(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"⏮️  ROLLED BACK: Removed {old_candidate}",
            extra=rollback
        )
...
# Global singleton
_deployment_manager: Optional[ModelDeployment] = None
