...
class ModelFineTuner:
    ...
    def __init__(
        self, api_key: str | None = None, min_examples: int = 200, max_examples: int = 10000
    ):
        """
        ...
        self.logger.info(f"ModelFineTuner initialized: " f"min={min_examples}, max={max_examples}")
    ...
    async def start_fine_tune(
        ...
        try:
            ...
            self.logger.info(f"✅ Fine-tune job started: {fine_tune_job.id}", extra=result)

            return result

        except Exception as e:
            self.logger.error(f"❌ Error starting fine-tune: {e!s}", exc_info=True)
            raise

    async def monitor_fine_tune(self, job_id: str, poll_interval: int = 60) -> dict[str, Any]:
        """
        ...
        try:
            while True:
                ...
                if job.status == "succeeded":
                    ...
                    self.logger.info(
                        f"✅ Fine-tuning complete! Model: {job.fine_tuned_model}", extra=result
                    )
            ...
            self.logger.error(f"Error monitoring fine-tune: {e!s}", exc_info=True)
    ...
    async def get_fine_tune_status(self, job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "fine_tuned_model": (
                    job.fine_tuned_model if hasattr(job, "fine_tuned_model") else None
                ),
                "trained_tokens": job.trained_tokens if hasattr(job, "trained_tokens") else 0,
            }

        except Exception as e:
            self.logger.error(f"Error getting job status: {e!s}", exc_info=True)
            raise
    ...
    async def list_fine_tune_jobs(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error listing jobs: {e!s}", exc_info=True)
            raise
    ...
    async def cancel_fine_tune(self, job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"job_id": job.id, "status": job.status, "cancelled": job.status == "cancelled"}

        except Exception as e:
            self.logger.error(f"Error cancelling job: {e!s}", exc_info=True)
            raise


# Global singleton
_fine_tuner: ModelFineTuner | None = None
