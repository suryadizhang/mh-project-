...
class PIIScrubber:
    ...
    def scrub(
        ...
        for pii_type, config in self.patterns.items():
            ...
            if matches:
                ...
                if config["risk_level"] == "high":
                    ...
                    self.logger.warning(
                        f"High-risk PII detected: {pii_type}",
        ...
        self.logger.debug(
            f"Scrubbed text: found {len(pii_found)} PII types",
    ...
    def batch_scrub(
        ...
        for i, text in enumerate(texts):
            try:
                ...
                self.logger.error(
                    f"Error scrubbing text {i}: {str(e)}",
                    exc_info=True
                )
                results.append({
        ...
        total_pii = sum(
            sum(r.get("pii_count", {}).values()) for r in results
        )
        
        self.logger.info(
            f"Batch scrubbed {len(texts)} texts, found {total_pii} PII instances"
        )
