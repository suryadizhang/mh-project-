...
class TrainingDatasetBuilder:
        ...
        try:
            ...
            for pair in training_pairs:
                ...
                if not question_safe["is_safe_for_training"] or not answer_safe["is_safe_for_training"]:
                    skipped_pii += 1
                    self.logger.debug(
                        f"Skipped training pair due to PII risk",
                        extra={
                ...
                if len(dataset) >= self.max_examples:
                    self.logger.warning(
                        f"Reached max examples ({self.max_examples}), stopping"
                    )
                    break
            ...
            if not validation["is_valid"]:
                raise ValueError(
                    f"Dataset validation failed: {validation['issues']}"
                )
            ...
            self.logger.info(
                f"✅ Dataset built successfully: {len(dataset)} examples",
                extra=result
            )
            ...
            self.logger.error(f"Error building dataset: {str(e)}", exc_info=True)
            raise
    ...
    def _validate_dataset(self, dataset: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(dataset) < min_size:
            issues.append(
                f"Dataset too small: {len(dataset)} examples (need {min_size})"
            )
        ...
        if duplicates > len(dataset) * 0.1:  # >10% duplicates
            issues.append(
                f"Too many duplicates: {duplicates} ({duplicates/len(dataset)*100:.1f}%)"
            )
    ...
    async def export_for_evaluation(
        ...
        try:
            ...
            self.logger.error(f"Error exporting evaluation data: {str(e)}", exc_info=True)
            raise


# Global singleton
_dataset_builder: Optional[TrainingDatasetBuilder] = None
