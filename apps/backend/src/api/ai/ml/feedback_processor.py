...
class FeedbackProcessor:
    ...
    async def process_feedback(
        ...
        try:
            ...
            if quality_score >= self.min_quality_score:
                promoted = await self._promote_to_training(db, message_id, quality_score)
    ...
    async def _promote_to_training(
        self, db: AsyncSession, message_id: str, quality_score: float
    ) -> bool:
        """
        ...
        try:
            ...
            logger.exception(f"Error promoting to training: {e}")
            return False

    async def get_feedback_analytics(self, db: AsyncSession, days: int = 30) -> dict[str, Any]:
        """
    ...
    async def get_training_promotion_stats(
        ...
        try:
            ...
            logger.exception(f"Error generating promotion stats: {e}")
            return {"error": str(e)}

    async def trigger_kb_update_if_needed(self, db: AsyncSession) -> bool:
        """
        ...
        try:
            ...
            if pending_count >= 50:
                ...
                logger.info(f"Triggering KB update: {pending_count} pending examples")
                return True
            else:
                logger.info(f"No KB update needed: only {pending_count} pending examples")
                return False

        except Exception as e:
            logger.exception(f"Error checking KB update trigger: {e}")
            return False
