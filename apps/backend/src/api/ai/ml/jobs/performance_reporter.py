...
async def weekly_performance_reporter(db: AsyncSession):
    ...
    try:
        ...
        current_metrics = await _calculate_weekly_metrics(db, current_week_start, now)
        ...
        report = {
            ...
            "period": {"start": current_week_start.isoformat(), "end": now.isoformat()},
...
def _generate_summary(current: dict[str, Any], previous: dict[str, Any]) -> str:
    """
...
def _format_channel_breakdown(channels: dict[str, int]) -> str:
    ...
    return "\n".join([f"  - {channel}: {count}" for channel, count in sorted_channels[:3]])
