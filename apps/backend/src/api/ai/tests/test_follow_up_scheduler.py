...
class TestFollowUpScheduler(unittest.TestCase):
    ...
    def test_03_emotion_based_template_selection(self):
        ...
        async def run_test():
            ...
            assert (
                high_job.template_id == "post_event_high_emotion"
            ), f"Expected post_event_high_emotion, got {high_job.template_id}"
            assert (
                low_job.template_id == "post_event_low_emotion"
            ), f"Expected post_event_low_emotion, got {low_job.template_id}"
    ...
    def test_05_duplicate_prevention(self):
        ...
        async def run_test():
            ...
            job_id_1 = await self.scheduler.schedule_post_event_followup(
                conversation_id=f"{self.test_conv_id}_dup", user_id=user_id, event_date=event_date
            )
            ...
            job_id_2 = await self.scheduler.schedule_post_event_followup(
                conversation_id=f"{self.test_conv_id}_dup2", user_id=user_id, event_date=event_date
            )
    ...
    def test_07_list_scheduled_followups(self):
        ...
        async def run_test():
            ...
            pending_jobs = await self.scheduler.get_scheduled_followups(
                user_id=user_id, status=FollowUpStatus.PENDING
            )
            logger.info(f"Found {len(pending_jobs)} pending jobs")
            assert (
                len(pending_jobs) >= 2
            ), f"Should have at least 2 pending, got {len(pending_jobs)}"
    ...
    def test_08_timezone_handling(self):
        ...
        async def run_test():
            ...
            scheduler_pst = FollowUpScheduler(
                self.memory, self.emotion_service, timezone="US/Pacific"
            )
    ...
    def test_10_message_template_rendering(self):
        ...
        async def run_test():
            ...
            await self.scheduler.schedule_post_event_followup(
                conversation_id=conv_id, user_id=user_id, event_date=event_date
            )
