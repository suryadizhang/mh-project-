...
class TestSchedulerIntegration(unittest.TestCase):
    ...
    def test_02_booking_confirmation_followup(self):
        ...
        async def run_test():
            ...
            logger.info(f"✅ PASSED: Booking confirmation triggered follow-up (template: {job.template_id})")
    ...
    def test_03_emotion_based_template_selection(self):
        ...
        async def run_test():
            ...
            job_id_high = await self.orchestrator.schedule_post_event_followup(
                conversation_id=conv_high,
                user_id=user_high,
                event_date=event_date
            )
            ...
            job_id_low = await self.orchestrator.schedule_post_event_followup(
                conversation_id=conv_low,
                user_id=user_low,
                event_date=event_date
            )
    ...
    def test_04_inactive_user_detection(self):
        ...
        async def run_test():
            ...
            # Find our test user
            test_user = next(
                (u for u in inactive_users if u["user_id"] == user_id),
                None
            )
    ...
    def test_05_message_callback_integration(self):
        ...
        async def run_test():
            ...
            from api.ai.scheduler.follow_up_scheduler import ScheduledFollowUp, FollowUpStatus
            from core.database import get_db_context
