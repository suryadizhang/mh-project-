...
from api.ai.reasoning.complexity_router import ComplexityLevel, ComplexityRouter
from api.ai.reasoning.react_agent import ReActAgent, ReActConfig, ReActResult, ReActStep
...
async def test_complexity_router_complex_query(complexity_router):
    ...
    for query in queries:
        level = await complexity_router.classify(query)
        assert level == ComplexityLevel.MULTI_AGENT, f"Expected MULTI_AGENT for: {query}"
...
async def test_complexity_router_with_context(complexity_router):
    ...
    assert level1 in [ComplexityLevel.CACHE, ComplexityLevel.REACT, ComplexityLevel.HUMAN]

    # High escalation history + negative sentiment → HUMAN
    query_negative = "I have another issue and I'm very unhappy"
    level2 = await complexity_router.classify(query_negative, {"escalation_count": 3})
