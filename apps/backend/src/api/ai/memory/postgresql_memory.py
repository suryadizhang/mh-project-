...
from sqlalchemy import (
    ...
    Index, ForeignKey, select, func, and_, or_, desc
)
...
class AIMessage(Base):
    ...
    conversation_id = Column(String(100), ForeignKey("ai_conversations.id", ondelete="CASCADE"), nullable=False, index=True)
...
class PostgreSQLMemory(MemoryBackend):
    ...
    async def get_escalated_conversations(
        ...
        try:
            ...
            logger.error(f"Failed to get escalated conversations: {e}")
        ...
        try:
            async with get_db_context() as db:
                ...
                result = await db.execute(
                    select(func.count())
                    .select_from(AIConversation)
                    .where(and_(conv_filter, AIConversation.is_active == True))
                )
                ...
                result = await db.execute(
                    select(func.count())
                    .select_from(AIConversation)
                    .where(and_(conv_filter, AIConversation.escalated == True))
                )
    ...
    async def _update_emotion_stats_background(
        ...
        try:
            async with get_db_context() as db:
                ...
                conversation = await db.get(AIConversation, conversation_id)
                if not conversation:
                    logger.warning(f"Conversation {conversation_id} not found for emotion stats update")
                    return
                ...
                logger.debug(f"Updated emotion stats for conversation {conversation_id} in background")
                
        except Exception as e:
            logger.error(f"Failed to update emotion stats in background for {conversation_id}: {e}")
