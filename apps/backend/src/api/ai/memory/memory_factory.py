...
async def create_memory_backend(
    backend_type: Optional[MemoryBackendType] = None,
    **kwargs
) -> MemoryBackend:
    """
    ...
    if backend_type is None:
        ...
        try:
            ...
            logger.error(f"Invalid MEMORY_BACKEND: {backend_str}, using default: {DEFAULT_BACKEND.value}")
...
_global_memory_backend: Optional[MemoryBackend] = None
...
async def test_memory_backend(backend_type: Optional[MemoryBackendType] = None) -> bool:
    """
    ...
    try:
        ...
        logger.error(f"Memory backend test failed: {e}")
        return False
