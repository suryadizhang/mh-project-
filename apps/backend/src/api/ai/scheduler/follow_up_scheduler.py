...
from core.database import get_db_context, Base
...
class ScheduledFollowUp(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_scheduled_followups_duplicate_check', 'user_id', 'trigger_type', 'status', 'scheduled_at'),
    )
...
class FollowUpTemplate(BaseModel):
    ...
    content: str
    emotion_condition: Optional[str] = None  # high/neutral/low
    variables: List[str] = Field(default_factory=list)  # {booking_id}, {event_date}, etc.
...
class FollowUpScheduler:
            ...
            async with get_db_context() as db:
                ...
                for job in pending_jobs:
                    self.scheduler.add_job(
                        ...
                        trigger=DateTrigger(run_date=job.scheduled_at, timezone=self.timezone),
                        args=[job.id],
                        id=job.id,
    ...
    async def schedule_post_event_followup(
        ...
        try:
            ...
            if await self._has_pending_followup(user_id, FollowUpTriggerType.POST_EVENT, event_date):
                logger.warning(f"Post-event follow-up already scheduled for user {user_id}")
                return None
            ...
            async with get_db_context() as db:
                followup = ScheduledFollowUp(
                    ...
                    trigger_data={
                        ...
                        "followup_delay_hours": followup_delay.total_seconds() / 3600
                    },
                    scheduled_at=scheduled_at,
                    status=FollowUpStatus.PENDING.value,
                    template_id=template.id,
                    message_content=self._render_template(template, {
            ...
            self.scheduler.add_job(
                self._execute_followup,
                trigger=DateTrigger(run_date=scheduled_at, timezone=self.timezone),
                args=[job_id],
                id=job_id,
                replace_existing=True
            )
            
            logger.info(f"Scheduled post-event follow-up: {job_id} at {scheduled_at}")
            return job_id
            
        except Exception as e:
            logger.error(f"Failed to schedule post-event follow-up: {e}")
            raise
    ...
    async def schedule_reengagement(
        ...
        try:
            ...
            # Check for duplicates
            if await self._has_pending_followup(user_id, FollowUpTriggerType.RE_ENGAGEMENT):
                logger.warning(f"Re-engagement already scheduled for user {user_id}")
                return None
            
            # Select template (check if past customer)
            conversations = await self.memory.get_user_conversations(user_id, include_inactive=True)
            has_past_bookings = len(conversations) > 1
            
            template = (FOLLOW_UP_TEMPLATES["reengagement_past_customer"] 
            ...
            logger.error(f"Failed to schedule re-engagement: {e}")
            raise
            ...
            async with get_db_context() as db:
                ...
                for f in followups:
                    jobs.append(FollowUpJob(
                        ...
                        message_preview=f.message_content[:100] if f.message_content else None
                    ))
    ...
    async def _execute_followup(self, job_id: str) -> None:
        ...
        try:
            ...
            async with get_db_context() as db:
                ...
                if not followup or followup.status != FollowUpStatus.PENDING.value:
                    logger.warning(f"Follow-up {job_id} not found or not pending")
                    return
                ...
                if self.orchestrator_callback:
                    try:
                        await self.orchestrator_callback(
                            ...
                            conversation_id=followup.conversation_id or f"followup_{followup.id}",
                        ...
                        logger.error(f"Failed to send message for follow-up {job_id}: {e}")
                        raise  # Re-raise to mark as failed
                else:
                    # No callback - just log (useful for testing)
                    logger.info(f"No orchestrator callback - follow-up {job_id} marked as executed (testing mode)")
            ...
            logger.error(f"Failed to execute follow-up {job_id}: {e}")
    ...
    async def _has_pending_followup(
        ...
        try:
            ...
            logger.error(f"Failed to check for pending follow-up: {e}")
            return False
    ...
    async def _select_template_by_emotion(
        ...
        try:
            ...
            logger.error(f"Failed to select template by emotion: {e}")
            # Fallback to neutral
            if trigger_type == FollowUpTriggerType.POST_EVENT:
                return FOLLOW_UP_TEMPLATES["post_event_neutral_emotion"]
            else:
    ...
    def _render_template(self, template: FollowUpTemplate, variables: Dict[str, str]) -> str:
