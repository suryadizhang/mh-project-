...
class FollowUpScheduler:
    ...
    async def get_scheduled_followups(
        self, user_id: str | None = None, status: FollowUpStatus | None = None, limit: int = 50
    ) -> list[FollowUpJob]:
        """
            ...
            async with get_db_context() as db:
                ...
                if self.orchestrator_callback:
                    try:
                        ...
                        logger.exception(f"Failed to send message for follow-up {job_id}: {e}")
    ...
    async def _has_pending_followup(
        self, user_id: str, trigger_type: FollowUpTriggerType, event_date: datetime | None = None
    ) -> bool:
    ...
    async def _select_template_by_emotion(
        self, trigger_type: FollowUpTriggerType, emotion_history: list[dict[str, Any]]
    ) -> FollowUpTemplate:
