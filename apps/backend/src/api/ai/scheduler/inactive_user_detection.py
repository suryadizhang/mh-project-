...
async def detect_inactive_users(
    ...
    try:
        ...
        async with get_db_context() as db:
            ...
            logger.info(f"Found {len(inactive_users)} inactive users (threshold: {inactive_threshold_days} days)")
            return inactive_users
            
    except Exception as e:
        logger.error(f"Failed to detect inactive users: {e}")
        return []
    ...
    try:
        ...
        for user in inactive_users:
            try:
                ...
                logger.error(f"Failed to schedule re-engagement for user {user['user_id']}: {e}")
        
        logger.info(f"Scheduled {scheduled_count}/{len(inactive_users)} re-engagement campaigns")
        return scheduled_count
        
    except Exception as e:
        logger.error(f"Failed to schedule re-engagement campaigns: {e}")
        return 0
...
async def run_daily_reengagement_check(scheduler):
    ...
    try:
        ...
        logger.error(f"Daily re-engagement check failed: {e}")
