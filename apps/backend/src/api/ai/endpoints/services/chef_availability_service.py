...
class ChefAvailabilityService:
    ...
    def __init__(self, db: Session, station_id: str | None = None):
        """
    ...
    def get_available_slots(
        self, target_date: date, location_zone: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for slot_time in self.STANDARD_SLOTS:
            ...
            slot_info = {
                ...
                "booking_ids": (
                    self._get_booking_ids(target_date, slot_time) if current_bookings > 0 else []
                ),
            }
    ...
    def check_slot_available(
        self, target_date: date, slot_time: str, location_zone: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "display_time": self._format_time_display(slot_time),
            "reason": (
                self._get_unavailable_reason(chef_capacity, current_bookings)
    ...
    def find_next_available_slots(
        self, start_date: date, count: int = 5, location_zone: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def suggest_alternative_dates(
        self, requested_date: date, requested_time: str, alternatives_count: int = 3
    ) -> dict[str, Any]:
        """
        ...
        if slot_check["available"]:
            return {"requested_available": True, "slot": slot_check, "alternatives": []}
        ...
        if len(alternatives) < alternatives_count:
            next_available = self.find_next_available_slots(
                requested_date + timedelta(days=1), count=alternatives_count - len(alternatives)
            )
    ...
    def _get_bookings_count(self, target_date: date, slot_time: str) -> int:
        ...
        try:
            ...
            target_datetime = datetime.combine(
                target_date, datetime.strptime(slot_time, "%H:%M").time()
            )
            ...
            query = select(func.count(Booking.id)).where(
                and_(
                    ...
                    or_(Booking.status == "confirmed", Booking.status == "deposit_pending"),
                    Booking.cancelled_at is None,
                )
            )
            ...
            logger.exception(f"Error counting bookings: {e}")
            return 0
    ...
    def _get_booking_ids(self, target_date: date, slot_time: str) -> list[str]:
        ...
        try:
            ...
            logger.exception(f"Error fetching booking IDs: {e}")
            return []
...
# Singleton instance
_availability_service_instance: ChefAvailabilityService | None = None


def get_availability_service(db: Session, station_id: str | None = None) -> ChefAvailabilityService:
    """
