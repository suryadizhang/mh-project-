...
class ProductionKnowledgeBaseService:
    ...
    def _load_business_knowledge(self):
        ...
        logger.info(
            f"KnowledgeBase: Loaded {len(business_knowledge)} business knowledge chunks (business + admin system)"
        )
    ...
    async def enrich_with_database_data(self, db: AsyncSession, query: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if any(
                word in query.lower()
                for word in ["recent", "latest", "booking", "reservation", "event"]
            ):
            ...
            if any(
                word in query.lower()
                for word in ["review", "rating", "feedback", "testimonial", "customer"]
            ):
                ...
                try:
                    result = await db.execute(
                        select(func.count(CustomerReviewBlogPost.id)).where(
            ...
            logger.exception(f"Error enriching with database data: {e}")

        return dynamic_knowledge

    async def search_knowledge_base(
        self, query: str, limit: int = 10, min_score: float = 0.3, db: AsyncSession | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for chunk_id, chunk_data in self.chunks_cache.items():
            ...
            base_score = (
                (keyword_matches * 2 + content_matches) / (len(query_words) * 2)
                if query_words
                else 0
            )
    ...
    async def update_knowledge_chunk(
        self, chunk_id: str, content: str | None = None, metadata: dict[str, Any] | None = None
    ):
        ...
        if chunk_id in self.chunks_cache:
            ...
            for keyword in keywords:
                if keyword in self.keywords_index:
                    self.keywords_index[keyword] = [
                        cid for cid in self.keywords_index[keyword] if cid != chunk_id
                    ]
