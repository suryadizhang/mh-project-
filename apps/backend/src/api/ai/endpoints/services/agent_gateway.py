...
class StationAwareAgentGatewayService:
    ...
    async def validate_agent_for_station_user(
        self, 
        agent: str, 
        user_station_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _get_agent_capabilities(self, agent_type: AgentType) -> Dict[str, bool]:
        ...
        config = self.agent_configs[agent_type]
        all_capabilities = [
            "booking_management", "user_management", "staff_management",
            "financial_oversight", "system_configuration", "analytics_access",
            "restaurant_operations", "customer_support", "escalation_management",
            "cross_station_access"
        ]
        
        return {
            cap: cap in config["capabilities"] for cap in all_capabilities
        }
    ...
    async def process_station_aware_request(
        ...
        try:
            ...
            accessible_stations = user_station_context.get("accessible_stations", [current_station_id])
            ...
            escalation_check = await self._check_escalation_triggers(
                message,
                agent_type,
                agent_config["escalation_triggers"]
            )
            ...
            logger.error(f"Station-aware agent gateway processing error: {e}")
    ...
    async def _generate_agent_suggestions_with_station(
        ...
        station_role = user_station_context.get("station_role", "customer_support")
        ...
        suggestions_map = {
            ...
            ] + (["View cross-station analytics", "Compare station performance"] if is_cross_station else []),
            AgentType.STAFF: [
            ...
            ] + (["Compare with other stations", "Cross-station analysis"] if is_cross_station else [])
        }
    ...
    async def get_agent_details(self, agent_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "status": "active",
                "prompt_template": await self.prompt_registry.get_agent_prompt_template(agent_type.value),
                "tool_permissions": await self.tool_registry.get_agent_permissions(agent_type.value)
            }
