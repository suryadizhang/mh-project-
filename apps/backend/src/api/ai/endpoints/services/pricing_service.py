...
# Import models
from api.app.models.booking_models import MenuItem, AddonItem
...
class PricingService:
    ...
    def __init__(self, db: Optional[Session] = None, station_id: Optional[str] = None):
        """
    ...
    def _load_station_location(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to load station location: {e}, using env fallback")
            self._load_station_from_env()
    ...
    def calculate_travel_distance(
        ...
        destination_zipcode: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not self._google_maps_api_key:
            ...
            return self._estimate_travel_from_zip(destination_zipcode) if destination_zipcode else {
...
Your appreciation means the world to our chefs! 🙏✨
    ...
    def _get_minimum_filler_options(self, shortfall_amount: float) -> List[Dict[str, Any]]:
        ...
        upgrade_options = [
            ...
            {"name": "Filet Mignon Upgrade", "price": self.get_upgrade_price("filet_mignon"), "category": "protein_upgrade", "per_person": True},
            {"name": "Lobster Tail Upgrade", "price": self.get_upgrade_price("lobster_tail"), "category": "protein_upgrade", "per_person": True},
            
            # Additional items (+$10 each from your menu)
            {"name": "3rd Protein", "price": self.get_addon_price("third_protein"), "category": "protein", "per_person": False},
            {"name": "Gyoza", "price": self.get_addon_price("gyoza"), "category": "appetizer", "per_person": False},
            
            # Additional enhancements (+$5 each from your menu)
            {"name": "Yakisoba Noodles", "price": self.get_addon_price("yakisoba_noodles"), "category": "side", "per_person": False},
            {"name": "Extra Fried Rice", "price": self.get_addon_price("extra_fried_rice"), "category": "side", "per_person": False},
            {"name": "Extra Vegetables", "price": self.get_addon_price("extra_vegetables"), "category": "side", "per_person": False},
            {"name": "Edamame", "price": self.get_addon_price("edamame"), "category": "appetizer", "per_person": False},
        ]
    ...
    def calculate_party_quote(
        ...
        protein_selections: Optional[Dict[str, int]] = None,  # NEW: Protein breakdown
        upgrades: Optional[Dict[str, int]] = None,  # DEPRECATED: Use protein_selections
        addons: Optional[List[str]] = None,
        customer_address: Optional[str] = None,
        customer_zipcode: Optional[str] = None,
        travel_miles: Optional[float] = None  # Deprecated: use customer_address instead
    ) -> Dict[str, Any]:
        """
        ...
        response = {
            ...
            "actual_charge_before_travel": float(actual_charge_before_travel),  # What customer pays (min $550)
            ...
            ) if not meets_minimum else None,
            ...
            "travel_policy": self.get_travel_fee_explanation(detailed=True) if not travel_info or travel_info.get("requires_full_address") else None
        }
...
def get_pricing_service(db: Optional[Session] = None, station_id: Optional[str] = None) -> PricingService:
    """
...
if __name__ == "__main__":
    ...
    print("=== CORRECTED DEBBIE'S QUOTE (REAL PRICING) ===")
    print(f"Adults (14 × ${quote['breakdown']['adults']['unit_price']}): ${quote['breakdown']['adults']['total']}")
    print(f"Children (2 × ${quote['breakdown']['children']['unit_price']}): ${quote['breakdown']['children']['total']}")
    print(f"Filet Mignon Upgrade (10 × ${quote['breakdown']['upgrades']['filet_mignon']['unit_price']}): ${quote['breakdown']['upgrades']['filet_mignon']['total']}")
    print(f"Travel Fee: ${quote['breakdown']['travel_fee']}")
    if quote['breakdown']['travel_info']:
        print(f"  Distance: {quote['breakdown']['travel_info'].get('distance_miles')} miles")
        print(f"  Drive Time: {quote['breakdown']['travel_info'].get('drive_time')}")
    print(f"\nGRAND TOTAL: ${quote['total']}")
    print(f"Meets Minimum: {quote['meets_minimum']}")
