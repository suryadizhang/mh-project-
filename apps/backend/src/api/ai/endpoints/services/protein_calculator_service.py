...
class ProteinCalculatorService:
    ...
    def calculate_protein_costs(self, guest_count: int, protein_selections: dict[str, int]) -> dict:
        """
        ...
        for protein_key, quantity in protein_selections.items():
            ...
            if protein_lower in self.PREMIUM_UPGRADES:
                ...
                logger.info(
                    f"Premium protein: {premium_info['name']} × {quantity} = ${upgrade_price_per * quantity}"
                )
    ...
    def _generate_explanation(
        ...
        if breakdown:
            ...
            for item in breakdown:
                if item.is_free:
                    lines.append(
                        f"   • {item.protein_name}: {item.quantity} portions (FREE base protein)"
                    )
        ...
        if total > 0:
            ...
            lines.append("   • **No additional protein charges** ✅")
    ...
    def _generate_summary(self, breakdown: list[ProteinBreakdown]) -> str:
        ...
        for item in breakdown:
            if item.quantity > 0:
                if item.is_upgrade:
                    lines.append(
                        f"{item.quantity}× {item.protein_name} " f"(+${item.upgrade_price} each)"
                    )
                else:
    ...
    def validate_protein_selection(
        self, guest_count: int, protein_selections: dict[str, int]
    ) -> tuple[bool, str | None]:
        """
        ...
        for protein_key in protein_selections:
            protein_lower = protein_key.lower().replace(" ", "_")
    ...
    def get_available_proteins(self) -> dict:
        ...
        return {
            "free_proteins": [
                {"key": key, "name": name, "price": 0} for key, name in self.FREE_PROTEINS.items()
            ],
            "premium_upgrades": [
