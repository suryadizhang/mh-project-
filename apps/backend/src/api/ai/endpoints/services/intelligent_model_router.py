...
class IntelligentModelRouter:
    ...
    def calculate_query_complexity(
        ...
        simple_matches = sum(
            1 for pattern in self.simple_patterns if re.search(pattern, message_lower)
        )
        if simple_matches > 0:
        ...
        complex_matches = sum(
            1 for pattern in self.complex_patterns if re.search(pattern, message_lower)
        )
    ...
    def select_model(
        ...
        logger.info(f"Model selected: {model} (score: {score:.1f}, reason: {reason})")
    ...
    def get_savings_report(self, requests_data: list) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_cost_per_request": (
                round(total_actual_cost / len(requests_data), 6) if requests_data else 0
            ),
        }
