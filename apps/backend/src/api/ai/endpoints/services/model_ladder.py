...
class ModelLadderService:
    ...
    async def _analyze_complexity(self, message: str, context: dict[str, Any] | None = None) -> str:
        """Analyze message complexity."""
        try:
            ...
            complex_score = sum(
                1 for pattern in self.complexity_patterns["complex"] if pattern in message_lower
            )

            # Check for medium patterns
            medium_score = sum(
                1 for pattern in self.complexity_patterns["medium"] if pattern in message_lower
            )

            # Check for simple patterns
            simple_score = sum(
                1 for pattern in self.complexity_patterns["simple"] if pattern in message_lower
            )
    ...
    async def should_retry_with_higher_model(
        self, current_model: str, confidence: float, agent: str, error_type: str | None = None
    ) -> tuple[bool, str | None]:
        """
    ...
    def _get_next_tier_model(self, current: ModelTier, max_allowed: ModelTier) -> ModelTier | None:
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            test_selection = await self.select_model(message="test message", agent="customer")
