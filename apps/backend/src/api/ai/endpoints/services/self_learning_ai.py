...
class SelfLearningAI:
    ...
    async def record_interaction(
        ...
        """
        try:
            from api.ai.endpoints.models import Message, Conversation, MessageRole
    ...
    async def process_user_feedback(
        ...
        try:
            ...
            learning_insights = await self._analyze_feedback(
                db, message, feedback
            )
    ...
    async def generate_knowledge_base_updates(
        ...
        try:
            ...
            logger.error(f"Error generating KB updates: {e}")
            return []
    ...
    async def adapt_system_prompt(
        ...
        try:
            ...
            logger.error(f"Error adapting system prompt: {e}")
            ...
            return {
                **self.learning_stats,
                "database_metrics": {
                    ...
                    "feedback_rate": round(feedback_count / total_count * 100, 1) if total_count > 0 else 0
                },
    ...
    def _calculate_learning_health(self) -> str:
        ...
        feedback_rate = (
            self.learning_stats["feedback_received"] / 
            self.learning_stats["total_interactions"] 
            if self.learning_stats["total_interactions"] > 0 else 0
        )
