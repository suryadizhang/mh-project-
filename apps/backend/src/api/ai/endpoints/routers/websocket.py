...
from api.ai.endpoints.websocket_manager import (
    MessageType,
    WebSocketMessage,
    websocket_manager,
)
...
async def websocket_chat_endpoint(
    ...
    user_id: str | None = Query("admin"),
    channel: str | None = Query("admin"),
    user_role: str | None = Query("customer"),
):
    """
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.info(
                    f"Received WebSocket message: {message_type} in conversation {conversation_id}"
                )
                    ...
                    if user_name and user_phone:
                        try:
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Created/updated lead {lead.id} from chat for {user_name}"
                                )
                                ...
                                await db.rollback()
                                logger.exception(
                                    f"Error creating lead or subscribing to newsletter: {lead_error}"
                                )
                            ...
                            logger.exception(f"Error getting database session: {db_error}")
                ...
                logger.exception(f"Invalid JSON received: {data}")
                await websocket_manager.send_error_message(
                    conversation_id, "Invalid message format. Please send valid JSON."
                )
            except Exception as e:
                logger.exception(f"Error processing message: {e}")
                await websocket_manager.send_error_message(
                    conversation_id, f"Error processing message: {e!s}"
                )
    ...
    try:
        ...
        await websocket_manager.send_ai_response(
            ...
            metadata={
                ...
                "channel": channel,
                "ai_service": (
                    "customer_booking" if user_role == UserRole.CUSTOMER else "admin_management"
                ),
            },
        )
        ...
        logger.exception(f"Error handling role-based chat message: {e}")
        ...
        # Send error message
        await websocket_manager.send_error_message(
            conversation_id, "Sorry, I'm having trouble processing your message right now."
        )
...
async def broadcast_message(conversation_id: str, message: dict):
    """
