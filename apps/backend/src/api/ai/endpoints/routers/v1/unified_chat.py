...
from api.ai.endpoints.auth import (
    StationContext,
    get_station_context_optional,
    has_agent_access,
)
...
class UnifiedChatRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "context": {"location": "San Francisco", "party_size": 12},
                "stream": False,
            }
        }
    )
    ...
    model_override: str | None = Field(None, description="Force specific model selection")
...
class UnifiedChatResponse(BaseModel):
    ...
    actions: list[dict[str, Any]] | None = Field(None, description="Available actions")
    escalation: dict[str, Any] | None = Field(None, description="Escalation information if needed")

    # Agent-aware metadata
    agent_capabilities: dict[str, bool] = Field(..., description="Current agent capabilities")
    tools_used: list[str] = []
    cost: dict[str, float] | None = Field(None, description="Request cost breakdown")

    # Station-aware metadata
    station_context: dict[str, Any] | None = Field(None, description="Station context information")
...
def detect_agent_from_request(
    x_agent: str | None = Header(None, description="Agent identifier"),
...
async def unified_chat(
    ...
    try:
        ...
        if station_context:
            ...
            permission_level = (
                "station_scoped" if not station_context.is_super_admin else "super_admin"
            )
        ...
        logger.exception(f"Unified chat error: {e}")
        await monitoring.log_error(
            message_id=message_id, agent=agent, error=str(e), context=request.context
        )
        raise HTTPException(
...
async def unified_api_health():
    ...
    try:
        ...
        health_status = {
            ...
            "features": {"station_aware": True, "rbac_enabled": True, "multi_tenant": True},
        }
