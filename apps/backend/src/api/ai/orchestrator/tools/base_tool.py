    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "error": None,
                "metadata": {"execution_time_ms": 45, "tool_name": "calculate_party_quote"},
            }
        }
...
class BaseTool(ABC):
    ...
    async def execute_with_logging(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            # Log execution start
            self.logger.info(f"Executing tool: {self.name}", extra={"parameters": kwargs})
            ...
            self.logger.info(
                ...
                extra={"execution_time_ms": execution_time_ms, "success": result.success},
            )
