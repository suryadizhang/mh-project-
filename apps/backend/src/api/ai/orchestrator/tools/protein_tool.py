    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if guest_count < 1:
                return ToolResult(
                    success=False,
                    error="guest_count must be at least 1"
                )
            
            if not protein_selections:
                return ToolResult(
                    success=False,
                    error="protein_selections cannot be empty"
                )
            ...
            # FREE proteins note
            free_proteins = [p for p in result["breakdown"] if p["is_free"] and p["upgrade_price"] == 0]
            if free_proteins:
            ...
            upgrade_proteins = [p for p in result["breakdown"] if p["is_upgrade"] and not p["is_third_protein"]]
            ...
            if result["third_protein_cost"] > 0:
                ...
                notes.append(f"3rd protein charge: ${result['third_protein_cost']:.2f} (+$10 per portion beyond 2 per guest, {extra_portions} extra portions)")
            
            # Educational note
            if sum(protein_selections.values()) <= guest_count * 2:
                free_count = sum(1 for p in result["breakdown"] if p["is_free"] and p["upgrade_price"] == 0)
                if free_count > 0:
            ...
            logger.info(
                f"Protein tool executed successfully",
                extra={
                    "guest_count": guest_count,
            ...
            return ToolResult(
                ...
                metadata={
                    ...
                    "includes_third_protein_logic": True
                }
            )
            
        except Exception as e:
            logger.error(f"Protein tool execution failed: {str(e)}", exc_info=True)
            return ToolResult(
                success=False,
                error=f"Failed to calculate protein costs: {str(e)}",
