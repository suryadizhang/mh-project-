...
from ...monitoring.pricing import get_model_pricing, calculate_cost
...
class OpenAIProvider:
    ...
    async def complete(
        ...
        if stream:
            raise InvalidRequestError(
                "Use complete_stream() for streaming responses",
                provider="openai"
            )
    ...
    async def embed(
        ...
        try:
            ...
            logger.info(f"OpenAI embeddings: {len(texts)} texts, {response.usage.total_tokens} tokens")
    ...
    def get_default_model(self, capability: ModelCapability) -> str:
        ...
        elif capability == ModelCapability.STREAMING:
            return self._default_chat_model
