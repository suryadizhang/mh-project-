...
async def log_tutor_pair(
    ...
    teacher_tokens: Optional[int] = None,
    student_tokens: Optional[int] = None,
    teacher_cost_usd: Optional[float] = None,
    teacher_response_time_ms: Optional[int] = None,
    student_response_time_ms: Optional[int] = None,
...
async def get_high_quality_pairs(
    ...
    query = select(AITutorPair).where(
        AITutorPair.similarity_score >= min_similarity
    )
    ...
    logger.info(
        f"Retrieved {len(pairs)} high-quality pairs "
...
async def get_tutor_pair_stats(
    db: AsyncSession,
    days: int = 7
) -> Dict[str, Any]:
    """
    ...
    # Average similarity
    avg_similarity_query = select(
        func.avg(AITutorPair.similarity_score)
    ).where(
    ...
    avg_teacher_time_query = select(
        func.avg(AITutorPair.teacher_response_time_ms)
    ).where(
    ...
    avg_student_time_query = select(
        func.avg(AITutorPair.student_response_time_ms)
    ).where(
    ...
    return {
        ...
        "high_quality_percentage": (
            round((high_quality_count / total_pairs) * 100, 1)
        ...
        "student_speedup": (
            round(avg_teacher_time / avg_student_time, 2)
            if avg_student_time > 0 else 0.0
        )
    }
