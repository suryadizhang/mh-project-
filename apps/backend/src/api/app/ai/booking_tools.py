...
class AIBookingRequest(BaseModel):
    ...
    def validate_date(self, v):
        ...
        if v < min_date.date():
            raise ValueError(
                f"Booking must be at least {AIBookingToolsConfig.MIN_ADVANCE_BOOKING_HOURS} hours in advance"
            )
        if v > max_date:
            raise ValueError(
                f"Booking cannot be more than {AIBookingToolsConfig.MAX_ADVANCE_BOOKING_DAYS} days in advance"
            )
...
class AIBookingTools:
    ...
    async def check_availability(
        self, preferred_date: date, party_size: int, preferred_slots: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            async with get_db_session() as db:
                # Validate inputs
                if (
                    party_size < self.config.MIN_PARTY_SIZE
                    or party_size > self.config.MAX_PARTY_SIZE
                ):
                ...
                today = date.today()
                min_booking_date = today + timedelta(
                    hours=self.config.MIN_ADVANCE_BOOKING_HOURS / 24
                )
                ...
                if preferred_date.weekday() == 0:  # Monday
                    raise BookingValidationError(
                        "Restaurant is closed on Mondays", "RESTAURANT_CLOSED"
                    )
                ...
                if not result.success:
                    raise BookingValidationError(
                        result.error or "Failed to check availability", "AVAILABILITY_CHECK_FAILED"
                    )
                ...
                if preferred_slots:
                    available_slots = [
                        slot for slot in available_slots if slot.get("time") in preferred_slots
                    ]
    ...
    async def create_booking(self, booking_request: AIBookingRequest) -> dict[str, Any]:
        ...
        try:
            async with get_db_session() as db:
                # Get AI system user
                await self.get_ai_system_user(db)
                ...
                if not result.success:
                    raise BookingValidationError(
                        result.error or "Failed to create booking", "BOOKING_CREATION_FAILED"
                    )
                ...
                logger.info(
                    f"AI successfully created booking {booking_id} for {booking_request.customer_email}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to queue follow-up scheduling for booking {booking_id}: {e}"
                    )
                ...
                return {
                    ...
                    "next_steps": {
                        ...
                        "can_modify_until": str(
                            booking_request.preferred_date - timedelta(hours=24)
                        ),
                    },
                    "ai_context": {
                        "conversation_id": booking_request.ai_conversation_id,
            ...
            logger.exception(f"Error creating AI booking: {e}")
            raise BookingValidationError(
        ...
        try:
            async with get_db_session() as db:
                ...
                if booking_id:
                    try:
                        ...
                        query = GetBookingQuery(
                            booking_id=booking_uuid, include_payments=True, include_messages=True
                        )
                        result = await query_bus.execute(query, db)

                        if not result.success:
                            raise BookingValidationError(
                                f"Booking not found with ID: {booking_id}", "BOOKING_NOT_FOUND"
                            )

                        return {"success": True, "booking": result.data}
                    except ValueError:
                        raise BookingValidationError(
                            "Invalid booking ID format", "INVALID_BOOKING_ID"
                        )
                ...
                if customer_email or customer_phone:
                    ...
                    if not result.success:
                        raise BookingValidationError(
                            "Failed to search for bookings", "SEARCH_FAILED"
                        )
                    ...
                    if not bookings:
                        ...
                        raise BookingValidationError(
                            f"No bookings found for {identifier}", "NO_BOOKINGS_FOUND"
                        )
            ...
            logger.exception(f"Error getting booking details: {e}")
            raise BookingValidationError(
        ...
        try:
            async with get_db_session() as db:
                ...
                try:
                    ...
                    raise BookingValidationError("Invalid booking ID format", "INVALID_BOOKING_ID")

                # Get current booking details
                current_booking_result = await self.get_booking_details(booking_id=booking_id)
                if not current_booking_result["success"]:
                    raise BookingValidationError(
                        f"Booking not found: {booking_id}", "BOOKING_NOT_FOUND"
                    )

                current_booking_result["booking"]
                ...
                for field, value in updates.items():
                    if field not in allowed_fields:
                        raise BookingValidationError(
                            f"Field '{field}' cannot be updated via AI", "FIELD_NOT_UPDATABLE"
                        )
                    ...
                    if field == "date" and value:
                        ...
                        if new_date.weekday() == 0:  # Monday
                            raise BookingValidationError(
                                "Restaurant is closed on Mondays", "INVALID_DATE"
                            )
                        ...
                        if value not in self.config.AVAILABLE_SLOTS:
                            raise BookingValidationError(
                                f"Invalid time slot: {value}", "INVALID_SLOT"
                            )
                ...
                if not validated_updates:
                    raise BookingValidationError("No valid updates provided", "NO_UPDATES")
                ...
                if not result.success:
                    raise BookingValidationError(
                        result.error or "Failed to update booking", "UPDATE_FAILED"
                    )
            ...
            logger.exception(f"Error updating AI booking: {e}")
            raise BookingValidationError(
        ...
        try:
            async with get_db_session() as db:
                ...
                if refund_requested:
                    ...
                    hours_until_booking = (
                        datetime.combine(booking_date, datetime.min.time()) - datetime.now()
                    ).total_seconds() / 3600
                ...
                if not result.success:
                    raise BookingValidationError(
                        result.error or "Failed to cancel booking", "CANCELLATION_FAILED"
                    )

                logger.info(
                    f"AI successfully cancelled booking {booking_id} with refund ${refund_amount_cents/100:.2f}"
                )
            ...
            logger.exception(f"Error cancelling AI booking: {e}")
            raise BookingValidationError(
