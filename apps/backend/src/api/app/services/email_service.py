...
class EmailService:
    ...
    def __init__(self):
        ...
        if self.enabled:
            logger.info(
                f"Email service initialized: {self.smtp_host}:{self.smtp_port} (User: {self.smtp_user})"
            )
        else:
    ...
    def _send_email(
        ...
        while attempt <= max_retries:
            try:
                ...
                logger.exception(f"❌ SMTP Authentication failed: {e} (Check IONOS credentials)")
...
async def send_booking_confirmation(booking_data: dict, background_tasks=None):
    ...
    try:
        ...
        email_service._send_email(
            to_emails=[booking_data["email"]], subject=subject, body=plain_text, html_body=html_body
        )
...
async def send_admin_notification(
    ...
    try:
        ...
        email_service._send_email(
            to_emails=recipient_emails, subject=subject, body=plain_text, html_body=html_body
        )
...
async def send_booking_cancellation_email(booking_data: dict, reason: str, background_tasks=None):
    ...
    try:
        subject = "Booking Cancelled - My Hibachi Chef"
...
async def send_newsletter(
    recipients: list[dict], subject: str, message: str, background_tasks=None
):
    """
    ...
    try:
        ...
        for i in range(0, len(emails), batch_size):
            ...
            try:
                ...
                logger.info(
                    f"Newsletter batch sent: {len(batch)} emails ({total_sent}/{len(emails)} total)"
                )
                ...
                logger.exception(f"Failed to send newsletter batch {i//batch_size + 1}: {e}")
