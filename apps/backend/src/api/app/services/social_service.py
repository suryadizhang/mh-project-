...
class SocialService:
    ...
    async def _process_instagram_message(self, account: SocialAccount, messaging: dict[str, Any]) -> None:
        ...
        if is_incoming and message_data.get("text"):
            ...
            if newsletter_service.is_stop_command(message_text):
                ...
                await self.send_instagram_message(thread.id, response_msg, {"is_system_message": True})
            ...
            if newsletter_service.is_start_command(message_text):
                ...
                logger.info(f"Processed START command from Instagram @{instagram_handle}: {success}")
                return  # Don't process with AI
    ...
    async def _process_facebook_message(self, account: SocialAccount, messaging: dict[str, Any]) -> None:
        ...
        if is_incoming and message_data.get("text"):
            ...
            if newsletter_service.is_stop_command(message_text):
                ...
                await self.send_facebook_message(thread.id, response_msg, {"is_system_message": True})
    ...
    async def _process_facebook_change(self, account: SocialAccount, change: dict[str, Any]) -> None:
    ...
    async def _process_facebook_comment(self, account: SocialAccount, comment_data: dict[str, Any]) -> None:
    ...
    async def send_facebook_message(self, thread_id: UUID, message_body: str, metadata: dict[str, Any] = None) -> bool:
