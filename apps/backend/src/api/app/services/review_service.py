...
class ReviewService:
    ...
    async def send_review_sms(
        ...
        try:
            ...
            logger.error(f"Error sending review SMS: {e}")
            return False
        ...
        try:
            ...
            if rating in ['okay', 'could_be_better']:
                if not complaint_text:
                    return {"success": False, "error": "Complaint text required for negative reviews"}
    ...
    async def track_external_review(
        ...
        try:
            ...
            await self.db.rollback()
            logger.error(f"Error tracking external review: {e}")
            return False
    ...
    async def issue_coupon_after_ai_interaction(
        ...
        try:
            ...
            logger.error(f"Error issuing coupon after AI interaction: {e}")
            await self.db.rollback()
            return None
    ...
    async def _issue_coupon(
        ...
        try:
            ...
            logger.error(f"Error issuing coupon: {e}")
            return None

    async def get_review_by_id(self, review_id: UUID) -> Optional[CustomerReview]:
