...
async def generate_station_code(
    ...
    result = await db.execute(
        select(func.max(Station.code))
        .where(Station.code.like(f"{prefix}-%"))
    )
...
async def _test_code_generator(db: AsyncSession):
    ...
    print("Testing Station Code Generator:")
    for city, state, expected_pattern in test_cases:
        code = await generate_station_code(db, city, state)
        is_valid = await validate_station_code_format(code)
        print(f"  {city}, {state} -> {code} (valid: {is_valid})")
