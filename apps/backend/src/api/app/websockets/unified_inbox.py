...
async def authenticate_websocket(websocket: WebSocket, token: str) -> str | None:
    ...
    try:
        ...
        logger.exception(f"WebSocket authentication error: {e}")
        return None
...
async def send_active_threads(websocket: WebSocket):
    ...
    try:
        ...
        logger.exception(f"Error sending active threads: {e}")
...
async def mark_thread_read(websocket: WebSocket, thread_id: str):
    ...
    try:
        ...
        logger.exception(f"Error marking thread as read: {e}")
...
async def websocket_endpoint(
    ...
    await manager.connect(
        websocket, user_id, {"client_type": client_type, "client_version": client_version}
    )
    ...
    try:
        ...
        logger.exception("Invalid JSON received from WebSocket client")
        await manager.send_personal_message(
            {
                "type": "error",
...
async def broadcast_new_message(thread_data: dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"Broadcasted new message: {thread_data.get('platform')} - {thread_data.get('sender')}"
        )

    except Exception as e:
        logger.exception(f"Error broadcasting new message: {e}")
...
async def broadcast_lead_update(lead_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error broadcasting lead update: {e}")
...
async def broadcast_system_alert(alert_data: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error broadcasting system alert: {e}")
