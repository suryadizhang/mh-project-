...
from api.app.models.lead_newsletter import ThreadStatus
...
class UnifiedMessage:
    ...
    # Participants
    from_contact: str | None = None  # Phone, email, handle, etc.
...
class UnifiedMessageRouter:
    ...
    async def process_message(
        self, message: UnifiedMessage, db: Session, background_processing: bool = True
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            await self._process_by_channel(message, db)
            ...
            await self._store_message_event(message, thread_info, db)
            ...
            processing_result = {
                ...
                "new_thread": thread_info.created_at
                and thread_info.created_at > datetime.now().replace(second=0, microsecond=0),
            ...
            if background_processing and not command_result:
                asyncio.create_task(
                    self._background_ai_processing(message, lead_info, thread_info, db)
                )
    ...
    async def _process_by_channel(
        self, message: UnifiedMessage, db: Session
    ) -> dict[str, Any] | None:
    ...
    async def _handle_sms_message(self, message: UnifiedMessage, db: Session) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error handling SMS message: {e}")
            return {"channel_processed": False, "error": str(e)}

    async def _handle_instagram_message(
        self, message: UnifiedMessage, db: Session
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error handling Instagram message: {e}")
            return {"channel_processed": False, "error": str(e)}
    ...
    async def _handle_facebook_message(
        ...
        try:
            ...
            message.platform_metadata.update(
                {
                    ...
                    "has_attachments": (
                        len(message.attachments) > 0 if message.attachments else False
                    ),
                }
            )
            ...
            logger.exception(f"Error handling Facebook message: {e}")
            return {"channel_processed": False, "error": str(e)}

    async def _handle_google_business_message(
        self, message: UnifiedMessage, db: Session
    ) -> dict[str, Any]:
        ...
        try:
            ...
            message.platform_metadata.update(
                {
                    ...
                    "business_location": (
                        message.platform_metadata.get("location")
                        if message.platform_metadata
                        else None
                    ),
                }
            )
            ...
            logger.exception(f"Error handling Google Business message: {e}")
            return {"channel_processed": False, "error": str(e)}
    ...
    async def _handle_stripe_message(self, message: UnifiedMessage, db: Session) -> dict[str, Any]:
        ...
        try:
            ...
            message.platform_metadata.update(
                {
                    ...
                    "event_type": (
                        message.platform_metadata.get("event_type")
                        if message.platform_metadata
                        else "unknown"
                    ),
                }
            ...
            logger.exception(f"Error handling Stripe message: {e}")
            return {"channel_processed": False, "error": str(e)}
    ...
    async def _handle_plaid_message(self, message: UnifiedMessage, db: Session) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error handling Plaid message: {e}")
            return {"channel_processed": False, "error": str(e)}
    ...
    async def _handle_email_message(self, message: UnifiedMessage, db: Session) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error handling email message: {e}")
            return {"channel_processed": False, "error": str(e)}
    ...
    async def _handle_phone_message(self, message: UnifiedMessage, db: Session) -> dict[str, Any]:
        ...
        try:
            ...
            message.platform_metadata.update(
                {
                    ...
                    "from_number": from_number,
                    "call_duration": (
                        message.platform_metadata.get("duration")
                        if message.platform_metadata
                        else None
                    ),
                    "call_result": (
                        message.platform_metadata.get("result")
                        if message.platform_metadata
                        else None
                    ),
                }
            ...
            logger.exception(f"Error handling phone message: {e}")
            return {"channel_processed": False, "error": str(e)}

    def _normalize_phone_number(self, phone: str | None) -> str | None:
    ...
    async def _process_lead(self, message: UnifiedMessage, db: Session) -> dict[str, Any] | None:
        """Find or create lead for message."""
        # Implementation would go here
        return {"lead_id": "1", "created": False}

    async def _process_thread(
        self, message: UnifiedMessage, lead_info: dict[str, Any] | None, db: Session
    ):
    ...
    async def _broadcast_message_update(
        self, processing_result: dict[str, Any], message: UnifiedMessage, thread_info
    ):
        """Broadcast real-time message updates via WebSocket."""
        # Implementation would go here

    async def _background_ai_processing(
        self, message: UnifiedMessage, lead_info: dict[str, Any] | None, thread_info, db: Session
    ):
...
_message_router: UnifiedMessageRouter | None = None
...
async def process_ringcentral_sms(
    ...
    try:
        ...
        logger.exception(f"Error processing RingCentral SMS: {e}")
        raise
