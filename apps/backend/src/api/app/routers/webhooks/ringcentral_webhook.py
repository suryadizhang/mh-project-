...
def verify_ringcentral_signature(
    payload: bytes, 
    signature: str, 
    secret: str
) -> bool:
...
async def ringcentral_voice_webhook(
    ...
    try:
        ...
        logger.error(f"Error processing RingCentral voice webhook: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def process_inbound_sms(
    ...
    try:
        ...
        logger.error(f"Error processing inbound SMS {message_id}: {e}")
...
async def broadcast_message_update(processing_result: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(f"Error preparing message update: {e}")
