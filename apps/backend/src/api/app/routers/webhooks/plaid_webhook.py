...
from ...models.lead_newsletter import Lead, LeadSource, LeadStatus, SocialThread, ThreadStatus
...
async def process_transaction_event(
    ...
    try:
        ...
        if requires_attention:
            ...
            thread.status = ThreadStatus.URGENT if webhook_code == "ITEM_ERROR" else ThreadStatus.OPEN
        ...
        logger.error(f"Error processing Plaid transaction event: {e}")
        db.rollback()
        return None


async def process_item_event(
    webhook_data: Dict[str, Any],
    db: Session
) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if webhook_code == "ITEM_ERROR":
            ...
            message_content = f"🚨 Account connection error: {error.get('error_message', 'Unknown error')}"
        ...
        logger.error(f"Error processing Plaid item event: {e}")
        db.rollback()
        return None


def generate_plaid_message(webhook_type: str, webhook_code: str, webhook_data: Dict[str, Any]) -> str:
...
async def analyze_transaction_event(
    ...
    try:
        ...
        logger.error(f"Error in transaction event AI analysis: {e}")


async def handle_item_alert(
    thread_id: str,
    webhook_code: str,
    webhook_data: Dict[str, Any]
):
    ...
    try:
        ...
        logger.error(f"Error handling item alert: {e}")
...
async def broadcast_thread_update(thread_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(f"Error broadcasting Plaid thread update: {e}")
