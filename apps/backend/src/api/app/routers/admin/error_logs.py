...
async def list_error_logs(
    ...
    level: Optional[str] = Query(None, description="Filter by log level (ERROR, WARNING, etc.)"),
...
async def resolve_error(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to resolve error log: {str(e)}"
        )
...
async def get_error_statistics(
    ...
    try:
        ...
        errors_by_level = {level: count for level, count in level_result.all()}
        ...
        errors_by_type = {error_type: count for error_type, count in type_result.all()}
        ...
        errors_by_path = {path: count for path, count in path_result.all()}
        ...
        avg_result = await db.execute(
            select(func.avg(ErrorLog.response_time_ms))
            .where(and_(
                ErrorLog.timestamp >= time_window,
                ErrorLog.response_time_ms.isnot(None)
            ))
        )
...
async def export_error_logs_csv(
    ...
    try:
        ...
        # Write header
        writer.writerow([
            "ID", "Timestamp", "Level", "Error Type", "Error Message",
            "Method", "Path", "Client IP", "User ID", "User Role",
            "Status Code", "Response Time (ms)", "Resolved"
        ])
