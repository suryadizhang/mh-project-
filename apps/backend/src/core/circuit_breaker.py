...
async def retry_with_backoff(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"Retry attempt {attempt + 1}/{max_retries} failed: {e}. " f"Retrying in {delay}s"
            )
...
class ExternalServiceClient:
    ...
    def __init__(self):
        self.stripe_breaker = get_circuit_breaker(
            "stripe", failure_threshold=5, recovery_timeout=60
        )
        self.ringcentral_breaker = get_circuit_breaker(
            "ringcentral", failure_threshold=3, recovery_timeout=30
        )

    async def charge_customer(self, amount: float) -> dict:
        """Charge customer with circuit breaker protection"""
        return await self.stripe_breaker.call(self._do_stripe_charge, amount)
    ...
    async def send_sms(self, phone: str, message: str) -> dict:
        ...
        return await self.ringcentral_breaker.call(self._do_send_sms, phone, message)
