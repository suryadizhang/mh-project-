...
class ComplianceConfig(BaseModel):
    ...
    tcpa_opt_out_keywords: list[str] = [
        "STOP",
        "UNSUBSCRIBE",
        "CANCEL",
        "END",
    ...
    retention_booking_records: int = (
        2555  # 7 years in days (tax/business records)
    )
...
class ConsentRecord(BaseModel):
    ...
    consent_source: str = Field(
        ..., description="Source where consent was captured"
    )
    consent_ip_address: str | None = None
    consent_user_agent: str | None = None
    consent_text: str = Field(
        ..., description="Exact consent text shown to user"
    )

    # Method tracking
    consent_method: str = Field(
        ..., description="checkbox, auto_subscribe, double_optin"
    )
...
class ComplianceValidator:
    ...
    def validate_sms_consent(
        ...
        if consent_method == "auto_subscribe":
            if "STOP" not in consent_text.upper():
                return (
                    False,
                    "Auto-subscribe must include STOP opt-out instructions",
                )
        ...
        missing = [
            elem for elem in required_elements if elem not in consent_lower
        ]
        if missing:
            return (
                False,
                f"Missing required TCPA disclosures: {', '.join(missing)}",
            )
        ...
        if self.config.consent_ip_tracking and not ip_address:
            return (
                False,
                "IP address tracking required for consent audit trail",
            )
    ...
    def validate_email_consent(
        ...
        if not has_physical_address:
            return (
                False,
                "CAN-SPAM requires physical business address in emails",
            )
...
<div style="border-top: 1px solid #ddd; padding: 20px; margin-top: 30px; font-size: 12px; color: #666; text-align: center;">
    ...
    <p style="margin-top: 5px; font-size: 11px;">
        Copyright {datetime.now(timezone.utc).year} {self.config.business_name}. All rights reserved.
    </p>
</div>
"""
    ...
    def check_marketing_frequency_limit(
        self, contact: str, channel: str, messages_sent_period: int
    ) -> tuple[bool, str]:
        """
        ...
        if messages_sent_period >= max_allowed:
            return (
                False,
                f"Exceeded {max_allowed} {channel} messages per {period}",
            )

        return (
            True,
            f"Within frequency limit ({messages_sent_period}/{max_allowed} per {period})",
        )
    ...
    def verify_unsubscribe_token(
        self, email: str, token: str, secret_key: str
    ) -> bool:
        """
