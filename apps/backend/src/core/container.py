...
class DependencyInjectionContainer:
    ...
    def register_singleton(
        ...
        self._services[service_name] = ServiceConfig(
            service_class=implementation, singleton=True, dependencies=dependencies or {}
        )
    ...
    def register_transient(
        ...
        service_name = self._get_service_name(interface)
        self._services[service_name] = ServiceConfig(
            service_class=implementation, singleton=False, dependencies=dependencies or {}
        )
    ...
    def register_factory(
        ...
        logger.debug(
            f"Registered factory: {service_name} -> {factory_class.__name__}.{factory_method}"
        )
        return self
        ...
        for dep_name, dep_service in config.dependencies.items():
            if dep_service in self._services or dep_service in self._instances:
                ...
                try:
                    ...
                    logger.exception(f"Failed to resolve dependency {dep_service}: {e}")
    ...
    def _get_constructor_dependencies(
        self, service_class: type, explicit_deps: dict[str, Any]
    ) -> dict[str, Any]:
...
def singleton(dependencies: dict[str, str] | None = None):
...
def transient(dependencies: dict[str, str] | None = None):
...
def service_factory(
    factory_method: str, singleton: bool = True, dependencies: dict[str, str] | None = None
):
