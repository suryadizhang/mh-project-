...
class BaseService:
    ...
    async def _handle_error(self, error: Exception, context: str) -> None:
        ...
        """
        self.logger.error(
            f"Error in {self.__class__.__name__}.{context}: {str(error)}",
            exc_info=True,
            extra={
                "service": self.__class__.__name__,
                "context": context,
...
class EventTrackingMixin:
    ...
    async def track_event(
        ...
        try:
            ...
            if hasattr(self, 'logger'):
                self.logger.error(f"Failed to track event: {str(e)}")
...
class CacheMixin:
    ...
    async def invalidate_cached(self, key: str) -> bool:
        ...
        try:
            ...
            if hasattr(self, 'logger'):
                self.logger.error(f"Cache invalidation failed for key {key}: {str(e)}")
            return False
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            if hasattr(self, 'logger'):
                self.logger.error(f"Cache pattern invalidation failed for {pattern}: {str(e)}")
            return 0
