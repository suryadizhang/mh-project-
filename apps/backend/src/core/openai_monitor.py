...
class OpenAIKeyMonitor:
    ...
    async def validate_key(self, force: bool = False) -> APIKeyHealthStatus:
        ...
        if not force and self._last_check:
            time_since_check = datetime.now() - self._last_check.last_checked
            if time_since_check < self._check_interval:
                logger.debug(f"Using cached validation result (checked {time_since_check.seconds}s ago)")
                return self._last_check
        
        logger.info("Validating OpenAI API key...")
        
        key_prefix = self.api_key[:20] + "..." if len(self.api_key) > 20 else self.api_key
        ...
        try:
            ...
            logger.info(f"✅ API key is valid ({models_count} models available)")
    ...
    async def startup_validation(self) -> APIKeyHealthStatus:
        ...
        if not status.is_valid:
            ...
            logger.info(f"✅ OpenAI API key validated successfully on startup")
            logger.info(f"   Key Type: {status.key_type}")
            logger.info(f"   Models Available: {status.available_models_count}")
            logger.info(f"   Test Call: {'✓' if status.test_call_successful else '✗'}")
