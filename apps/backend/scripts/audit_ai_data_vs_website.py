...
class DataAudit:
    ...
    def load_faqs_data(self):
        ...
        faq_path = Path(__file__).parent.parent.parent / "customer" / "src" / "data" / "faqsData.ts"
        ...
        if "$55 per adult" in content or "$55/adult" in content:
            ...
            self.warnings.append("⚠️  Adult pricing $55 not found in faqsData.ts")

        if "add-on option" in content.lower():
            self.verified.append("✅ 'add-on options' terminology found in faqsData.ts")
        else:
            self.warnings.append("⚠️  'add-on options' terminology not found in faqsData.ts")
    ...
    async def audit_business_rules(self, website_data):
        ...
        if adult_price == website_data["adult_base"]:
            self.verified.append(f"✅ Adult pricing: ${adult_price} matches website")
        else:
            self.issues.append(
        ...
        if child_price == website_data["child_base"]:
            self.verified.append(f"✅ Child pricing: ${child_price} matches website")
        else:
            self.issues.append(
        ...
        if minimum == website_data["party_minimum"]:
            self.verified.append(f"✅ Party minimum: ${minimum} matches website")
        else:
            self.issues.append(
        ...
        for addon_name, addon_price in website_data["add_ons"].items():
            ...
            if exists:
                ...
                if pitch and f"${addon_price}" in pitch:
                    self.verified.append(f"✅ {addon_name}: ${addon_price} correctly priced")
                ...
                self.warnings.append(f"⚠️  {addon_name}: Not found in upsell_rules table")
    ...
    async def audit_training_examples(self):
        ...
        if real_pricing_count >= 1:
            ...
            self.warnings.append("⚠️  No training examples with real pricing ($2,750 for 50 guests)")
    ...
    async def audit_faq_items(self, website_data):
        ...
        for faq in faqs:
            ...
            if "party package" in question or "party package" in answer:
                if "party package" not in answer or "$12" in answer or "$22" in answer:
                    self.issues.append(f"❌ FAKE PACKAGE in FAQ: {faq['question'][:50]}...")
        ...
        for topic1, topic2 in key_topics:
            ...
            if exists:
                ...
                self.warnings.append(f"⚠️  No FAQ found for: {topic1} + {topic2}")
            ...
            if table_exists:
                ...
                try:
                    ...
                    if fake_bookings:
                        ...
                        self.verified.append("✅ No fake/test bookings found in database")
                except Exception as e:
                    self.warnings.append(f"⚠️  Could not check bookings data: {e}")
            else:
                self.warnings.append(
