...
export interface MockAnalytics {
  ...
  topRevenueCustomers: Array<{ name: string; email: string; totalSpent: number }>;
}
...
const SAMPLE_NAMES = [
  ...
  'David Kim', 'Emily Brown', 'James Wilson', 'Maria Garcia',
  'Robert Taylor', 'Jennifer Martinez', 'William Anderson', 'Jessica Lee',
  'Michael Thomas', 'Ashley Jackson', 'Christopher White', 'Amanda Harris'
];
...
function generateBookings(count: number = 50): MockBooking[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    bookings.push({
      ...
      billingAddress: Math.random() > 0.3 ? address : SAMPLE_ADDRESSES[Math.floor(Math.random() * SAMPLE_ADDRESSES.length)],
      totalAmount,
      depositPaid: status !== 'pending',
      specialRequests: Math.random() > 0.7 ? 'Outdoor setup preferred' : undefined,
...
function generateLeads(count: number = 30): MockLead[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    leads.push({
      ...
      quality,
      createdAt: createdDate.toISOString(),
      lastContactedAt: status !== 'new' ? subDays(new Date(), Math.floor(Math.random() * 5)).toISOString() : undefined,
 ...
 */
function generateAnalytics(bookings: MockBooking[], leads: MockLead[]): MockAnalytics {
  ...
  const uniqueCustomers = new Set(recentBookings.map(b => b.customerEmail)).size;
...
class MockDataService {
   ...
   */
  async updateBookingStatus(id: string, status: MockBooking['status']): Promise<MockBooking | null> {
    await new Promise(resolve => setTimeout(resolve, 300));
   ...
   * Get all leads
   */
  async getLeads(filters?: { status?: string; quality?: string }): Promise<MockLead[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
