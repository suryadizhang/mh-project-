...
class EmailScheduler {
  ...
  private calculateEventEndTime(eventDate: string, eventTime: string): Date {
    ...
    const eventEndTime = new Date(
      `${eventDate}T${endHour.toString().padStart(2, '0')}:00:00`
    );
  ...
  async processPendingEmails(): Promise<void> {
    ...
    console.log(
      `[EMAIL SCHEDULER] Processing ${pendingEmails.length} pending emails`
    );
    ...
    for (const scheduledEmail of pendingEmails) {
      try {
        ...
        if (scheduledEmail.emailType === 'review_request') {
          ...
          success = await emailService.sendReviewRequest(
            scheduledEmail.bookingData
          );
          ...
          const { emailService } = await import('./email-service');
          success = await emailService.sendUpsellEmail(
            scheduledEmail.bookingData
          );
        }
  ...
  async triggerScheduledEmail(emailId: string): Promise<boolean> {
    const email = scheduledEmails.find(
      e => e.id === emailId && e.status === 'pending'
    );
    if (!email) {
      return false;
    }
    ...
    try {
      ...
      email.status = 'failed';
      console.error(
        `[EMAIL SCHEDULER ERROR] Manual trigger failed for ${emailId}:`,
        error
      );
      return false;
    }
  ...
  getSchedulerStats(): {
    ...
    scheduledEmails.forEach(email => {
      ...
      if (
        ...
        new Date(email.scheduledFor) <= next24Hours
      ) {
