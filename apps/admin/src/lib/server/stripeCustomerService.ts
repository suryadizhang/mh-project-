...
export class StripeCustomerService {
  ...
  static async updatePaymentPreferences(
    ...
    try {
      const response = await apiFetch(
        `/api/v1/customers/${stripeCustomerId}/preferences`,
        {
          method: 'PUT',
  ...
  static async trackPaymentPreference(
    customerId: string,
    paymentMethod: string
  ): Promise<void> {
    try {
      const response = await apiFetch(
        `/api/v1/customers/${customerId}/track-payment`,
        {
          method: 'POST',
      ...
      console.log(
        `[PAYMENT PREFERENCE TRACKED] Customer: ${customerId}, Method: ${paymentMethod}`
      );
    } catch (error) {
  ...
  static async createPortalSession(
    ...
    try {
      const response = await apiFetch(
        `/api/v1/customers/${stripeCustomerId}/portal`,
        {
          method: 'POST',
