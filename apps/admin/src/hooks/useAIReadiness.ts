...
interface UseAIReadinessReturn {
  ...
  enableAI: (reason: string, intents?: IntentType[], startPercentage?: number) => Promise<void>;
 ...
 */
export function useAIReadiness(options: UseAIReadinessOptions = {}): UseAIReadinessReturn {
  const {
    autoRefresh = true,
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const error = err instanceof Error ? err : new Error('Failed to fetch AI readiness data');
   ...
   */
  const updateTrafficSplit = useCallback(async (intent: IntentType, percentage: number) => {
    try {
      await AIReadinessService.updateTrafficSplit(intent, percentage);
      await refresh(); // Refresh data after update
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Failed to update traffic split');
      setError(error);
      if (onError) {
        onError(error);
      }
  ...
  const enableAI = useCallback(async (
    ...
    try {
      ...
      const error = err instanceof Error ? err : new Error('Failed to enable local AI');
      setError(error);
      if (onError) {
        onError(error);
      }
   ...
   * Disable local AI
   */
  const disableAI = useCallback(async (reason: string, emergency = false) => {
    try {
      await AIReadinessService.disableLocalAI({ reason, emergency });
      await refresh(); // Refresh data after disabling
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Failed to disable local AI');
      setError(error);
      if (onError) {
        onError(error);
      }
   ...
   * Rollback an intent
   */
  const rollbackIntent = useCallback(async (intent: IntentType, reason: string) => {
    try {
      await AIReadinessService.rollbackIntent(intent, reason);
      await refresh(); // Refresh data after rollback
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Failed to rollback intent');
      setError(error);
      if (onError) {
        onError(error);
      }
   ...
   */
  const getIntentReadiness = useCallback((intent: IntentType): IntentReadiness | null => {
 ...
 */
export function useShadowLearningStats(autoRefresh = true, refreshInterval = 30000) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch shadow stats'));
    } finally {
      setLoading(false);
    }
  }, []);
...
export function useOllamaHealth(autoRefresh = true, refreshInterval = 60000) {
  ...
  const checkHealth = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to check Ollama health'));
    } finally {
      setLoading(false);
    }
  }, []);
