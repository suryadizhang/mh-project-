...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if client_ip in self.blocked_ips:
            self.log_security_event("BLOCKED_IP", client_ip,
                                    "IP is in blocklist")
            return JSONResponse(
                status_code=403,
        ...
        if self.rate_limiter.is_rate_limited(
            ...
            self.log_security_event("RATE_LIMIT", client_ip,
                                    "Rate limit exceeded")
            return JSONResponse(
                status_code=429,
        ...
        if not self.validate_user_agent(user_agent):
            self.log_security_event("SUSPICIOUS_USER_AGENT", client_ip,
                                    f"UA: {user_agent}")
        ...
        if not self.validate_content_type(content_type):
            self.log_security_event("INVALID_CONTENT_TYPE", client_ip,
                                    f"CT: {content_type}")
            return JSONResponse(
                status_code=400,
        ...
        if not self.validate_request_content(request, body):
            self.log_security_event("ATTACK_PATTERN", client_ip,
