...
**Impact**: Double bookings possible **Location**:
`apps/backend/src/services/booking_service.py` **Fix Time**: 4 hours
**Fix**: Add database unique constraint + SELECT FOR UPDATE

### 2. Bug #14: Helper Method Error Handling

**Impact**: Crashes when invalid time format **Location**:
`apps/backend/src/services/booking_service.py` (\_slots_overlap) **Fix
Time**: 1 hour **Fix**: Add try-catch around time parsing

### 3. Integration Test Failures (18 tests)

**Impact**: Production reliability unknown **Location**:
`apps/backend/tests/integration/` **Fix Time**: 16 hours (2-3 days)
...
_Created: November 19, 2025_ _Priority: Start with Bug #13 and #14
TODAY_
