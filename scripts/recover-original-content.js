...
const BLOG_CONTENT_FILE = path.join(__dirname, '../apps/customer/src/data/blogContent.md');
...
CONTENT_MAPPING.forEach(mapping => {
  ...
  try {
    ...
    if (contentMarkerIndex !== -1) {
      section = section.substring(contentMarkerIndex + '**Content:**'.length).trim();
    }
    ...
    if (oldContent === 'No content available') {
      ...
      console.log(`⚠️  Warning: Content is not placeholder (${oldContent.substring(0, 50)}...)`);
    }
