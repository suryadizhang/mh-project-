...
function discoverRoutes() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { routes: cleanRoutes, timestamp: new Date().toISOString() },
      null,
      2
    )
  );
...
function createAssetBaseline() {
  ...
  fs.writeFileSync(
    `${BASELINE_DIR}/asset-baseline.json`,
...
async function main() {
  ...
  fs.writeFileSync(
    `${BASELINE_DIR}/baseline-summary.json`,
  ...
  console.log(
    `📊 Summary: ${routes.length} routes, ${
      apiRoutes.length
    } API routes, Build: ${buildSuccess ? 'SUCCESS' : 'FAILED'}`
  );
