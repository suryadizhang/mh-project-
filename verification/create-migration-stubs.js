...
function createMigrationStubs() {
  ...
  for (const route of routesToMigrate) {
    try {
      ...
      migrations.push({
        from: cleanPath,
        to:
          cleanPath.includes('/payments/') || cleanPath.includes('/stripe/')
      ...
      console.error(
        `❌ Failed to create stub for ${route.path}:`,
        error.message
      );
    }
  }
  ...
  fs.writeFileSync(
    './verification/migration-map.json',
...
function updateFrontendFetchCalls() {
  ...
  function updateFile(filePath) {
    try {
      ...
      for (const route of routesToMigrate) {
        const apiPath = route.path.replace(
...
function main() {
  ...
  console.log(
    `📊 Summary: ${migrationMap.migrations.length} routes migrated, ${updatedFiles.length} files updated`
  );
  console.log(
    `🔗 Next steps: Test functionality, update webhook configs, deploy backend`
  );
