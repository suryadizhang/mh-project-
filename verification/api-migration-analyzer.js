  ...
  try {
    ...
    if (hasBackendFunctionality || hasComplexLogic) {
      return {
        ...
        path: relativePath,
        reason: hasBackendFunctionality
          ? 'Contains backend keywords'
...
function generateMigrationPlan() {
  ...
  console.log(
    `✅ ALREADY MIGRATED (${routeCategories.ALREADY_MIGRATED.length}):`
  );
  ...
  routeCategories.UNKNOWN.forEach(route => {
    console.log(
      `  • ${route.path} - ${route.reason} (${route.size || 0} bytes)`
    );
  });
  ...
  allNeedingRewire.forEach(route => {
    ...
    const backendPath = apiPath
      .replace('/v1/', '/api/stripe/v1/')
      .replace('/api/', '/api/');
    console.log(
      `  fetch('${apiPath}') → fetch('${
  ...
  const analysis = {
    ...
    summary: {
      total: Object.values(routeCategories).reduce(
  ...
  fs.writeFileSync(
    './verification/api-migration-analysis.json',
