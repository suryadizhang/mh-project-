...
test.describe('Payment Processing', () => {
  ...
  test('processes Stripe payment successfully @payment @critical', async ({ page }) => {
    const amount = 100;
  ...
  test('shows Zelle payment instructions @payment @critical', async ({ page }) => {
...
test.describe('Payment Security', () => {
  ...
  test('sensitive data not exposed in DOM @payment @security', async ({ page }) => {
    await page.goto('/payment');
    
    // Complete payment
    await completeStripePayment(page, 100);
    
    // Check that card number is not in page content
