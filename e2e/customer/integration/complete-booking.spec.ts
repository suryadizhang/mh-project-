...
test.describe('Complete Booking Flow', () => {
  test('customer can create booking and complete payment @e2e @critical', async ({
    page,
  }) => {
    ...
    test.step('Create booking', async () => {
      ...
      test
        .info()
        .annotations.push({ type: 'bookingId', description: bookingId! });
    });
  ...
  test('handles invalid booking data gracefully @e2e', async ({ page }) => {
    ...
    // The booking page should be visible
    await expect(page.locator('h1').first()).toBeVisible();

    // Verify booking form is present (multi-step form)
