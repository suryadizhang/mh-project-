...
const CUSTOMER_URL =
  process.env.CUSTOMER_STAGING_URL || 'https://staging.myhibachichef.com';
...
test.describe('Customer Signing Page @customer @signing', () => {
  test('Signing page loads with valid token @critical', async ({ page }) => {
    ...
    const content = page.locator(
      'h1, h2, [data-testid="agreement-title"], [class*="agreement"]'
    );
  ...
  test('Signature pad is visible for valid pending hold @signing', async ({
    page,
  }) => {
    await page.goto(`${CUSTOMER_URL}/sign/${TEST_TOKEN}`);
    ...
    const signatureArea = page.locator(
      'canvas, [data-testid="signature-pad"], [class*="signature"]'
    );
  ...
  test('Submit button present for pending agreement @signing', async ({
    page,
  }) => {
    await page.goto(`${CUSTOMER_URL}/sign/${TEST_TOKEN}`);
    await page.waitForTimeout(3000);
    ...
    const submitButton = page
      .locator('button')
      .filter({ hasText: /sign|submit|agree|continue/i });
  ...
  test('Time remaining displayed for pending hold @customer', async ({
    page,
  }) => {
    await page.goto(`${CUSTOMER_URL}/sign/${TEST_TOKEN}`);
    await page.waitForTimeout(2000);
    ...
    const hasTimeInfo =
      pageContent?.toLowerCase().includes('expire') ||
