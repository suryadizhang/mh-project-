...
const ADMIN_URL =
  process.env.ADMIN_STAGING_URL || 'https://admin-staging.mysticdatanode.net';
...
test.describe('Admin Agreements Page @admin @agreements', () => {
  ...
  test('Pending holds tab shows slot holds @critical', async ({ page }) => {
    ...
    const holdsSection = page.locator(
      '[data-testid="holds-list"], table, [class*="hold"]'
    );
  ...
  test('Slot hold shows signing link and copy button @critical', async ({
    page,
  }) => {
    // Click on pending/holds tab
    const pendingTab = page
    ...
    const linkOrCopy = page.locator(
      '[data-testid="signing-link"], button:has-text("Copy"), [class*="copy"]'
    );
