...
test.describe('Admin Dashboard', () => {
  ...
  test('loads dashboard successfully @admin @smoke', async ({ page }) => {
    ...
    // Admin app uses root '/' as dashboard (no /dashboard route)
    // After login, user lands on stations page or root
    await expect(page).toHaveURL(/.*\/(stations)?$/);
    // Verify main content is visible (dashboard or stations page)
  ...
  test('displays booking statistics @admin', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="pending-bookings"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="completed-bookings"]')
    ).toBeVisible();
  ...
  test('can view booking details @admin @critical', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="booking-details-modal"]')
    ).toBeVisible();
  ...
  test('can add notes to booking @admin', async ({ page }) => {
    ...
    // Verify note added
    await expect(
      page.locator('text=Customer requested extra soy sauce')
    ).toBeVisible();
  });
});

test.describe('Admin Real-time Features', () => {
  test('receives real-time booking notifications @admin @realtime', async ({
    page,
  }) => {
    await loginAsAdmin(page);
