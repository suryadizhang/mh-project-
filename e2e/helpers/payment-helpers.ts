...
export async function completeStripePayment(page: Page, amount: number) {
  ...
  await stripeFrame
    ...
    .fill(testPayment.cards.visa.number);
  await stripeFrame
    .locator('[name="exp-date"]')
    .fill(testPayment.cards.visa.expiry);
...
export async function testPaymentFailure(page: Page) {
  ...
  await stripeFrame
    .locator('[name="cardnumber"]')
    .fill(testPayment.cards.declined.number);
  await stripeFrame
    .locator('[name="exp-date"]')
    .fill(testPayment.cards.declined.expiry);
  await stripeFrame
    .locator('[name="cvc"]')
    .fill(testPayment.cards.declined.cvc);
  await stripeFrame
    .locator('[name="postal"]')
    .fill(testPayment.cards.declined.zip);
