...
export const CheckoutSessionVerifyResponseSchema = z
  .object({
    id: z.string().startsWith('cs_').describe('Stripe Checkout Session ID'),
    status: z.enum(['open', 'complete', 'expired']).describe('Session status'),
    payment_status: z
    ...
    currency: z.string().length(3).toLowerCase().describe('Currency code'),
    customer_details: z
      .object({
    ...
    metadata: z.record(z.string()).optional().describe('Session metadata'),
    payment_intent: z
      .string()
      .startsWith('pi_')
      .optional()
  ...
  .describe(
    '⚠️ NOT IMPLEMENTED: Checkout session verification - endpoint missing'
  );

export type CheckoutSessionVerifyResponse = z.infer<
  typeof CheckoutSessionVerifyResponseSchema
>;
